...
- `preview/nav-frame-height/` — **⚠ 最新一頁：選單那三個玻璃框要多高，手機版三案（2026-08-11，未定案）**。
  ...
  > ⚠ **Ⓥ3 在 320 上 H ＝ 23.64 < 24，圓角上下接在一起、變成膠囊**，直段消失。
  >   375 以上仍是圓角矩形（375 上直段 1.80px）。挑 Ⓥ3 要知道 320 上是另一種形狀。
...
0.5 **那三個玻璃框要多高（手機版）** —— 2026-08-11 開的，三案在
   `preview/nav-frame-height/`（`?v=now|v1|v2|v3`），等使用者挑。
   Ⓥ1 2.65／Ⓥ2 2.36（＝主題與科別那排標記的比例）／Ⓥ3 2.15，數字與成因見第八節那一條。
   > **定案之後還有兩件跟著要做的：**
   > ① 套進 `index.html` 的手機 @media，並把切換條連同 `data-v` 一起刪。
   > ② `moveLamp()` 要一起搬 —— 框是 CSS 畫的、底是 JS 餵的，
   >   兩邊各算各的就會露出半個像素的錯位。提案頁把它抽成 `frameBox()`，
   >   讀不到 `--frame-em` 就退回原本的「外擴 1px」，電腦版因此完全不受影響。
   > **電腦版要不要跟著收，是還沒問過的一題**（這一輪使用者指定只做手機版）。
