...
| **看診時間本身** | **08:45–12:00／13:45–16:40／17:45–20:30**（2026-08-13 改，原本是 11:30／16:30／20:00）。依據是門口那面磁鐵板，**和 LINE 官方帳號上的那一組完全相同** —— 第十節記著的三邊不一致，網站原本是唯一的少數。休診日沒變（週六、週日）。底下那句同時換成**「國定假日與各科特別門診請來電確認。」**（使用者定的字，逐字，不要潤飾）—— 兒牙、口外這類特別門診的醫師不是每週都到，但**不要在表上標哪幾顆是隔週、不要列日期或當月排班**。⚠ **Google 商家檔案要同步**，三邊不一致會傷本地搜尋 | 第十節、`/history/clinic-schedule.html` |
...
### 資料不一致：網站這一側已經解掉（2026-08-13）

**原本的問題**（2026-08-10 從使用者的手機截圖看到的）：LINE 官方帳號上的門診時間
和網站不一樣，而「哪一組才對只有診所知道」。

**2026-08-13 由診所門口那面磁鐵門診表的照片裁決**：板子上是
**08:45–12:00／13:45–16:40／17:45–20:30**，和 LINE 那一組**完全相同** ——
網站原本那組（11:30／16:30／20:00）是唯一的少數。已改，JSON-LD 跟著更新。
...
1. **Google 商家檔案還沒同步**（只有使用者能改）。三邊不一致會直接傷到
   本地搜尋的實體辨識。
2. **LINE 上另外那組「營業時間」**（早班 08:35–12:05…）是第三組數字，
   和「門診時間」那組並存，也要一併處理。

> ⚠ 另外一件到現在都還沒確認：**板子上週六早診有兩顆磁鐵**（一般牙科＋齒顎矯正），
> 但使用者中途指示「六日拿掉」，所以表格只排週一到週五、網站維持「週六、週日休診」。
