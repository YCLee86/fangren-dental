...
| 牙周治療的藍 | **套色 `#4478b5`、白底的字 `#244369`**（Ⓧ·Ⓡ，2026-08-09）。從使用者給的 `#3496F2` 收到「白底黑底等效」那條線上。**白字只有 4.57:1，是全站最緊的一格，不能再亮** | [PALETTE.md](PALETTE.md) 第六之十四節 |
...
4. **齒顎矯正的藍要不要提亮、提到哪一階**（2026-08-09）。使用者說套色不夠亮，
   量下來它同時是七科裡最暗（L\* 36.3）與最灰（C\* 14.8）的一顆。三案 Ⓐ／Ⓑ／Ⓒ，
   提案頁 `preview/spec-ortho-bright/`，推導在 [PALETTE.md](PALETTE.md) 第六之十二節。
   **正式站與 `index.html` 一個字都還沒動。**
   > 這一件中途曾因為「牙周要拿 Ⓒ」而被判死（提亮就撞），**現在又活了** ——
   > 牙周最後搬到 h 273，矯正 h 248 旁邊空出來了。要重啟就直接看那一頁。
   >
   > **2026-08-09 又開了第二條路：矯正乾脆換掉藍。** 使用者要求把品牌 logo 還沒用到的
   > 兩顆（米色、稻草灰）拿進來，加上資料庫三個不撞的，做成 `preview/spec-ortho-palette/`
   > 八案（含上面的 Ⓑ 一起比）。推導在 [PALETTE.md](PALETTE.md) 第六之十五節。
   >
   > **⚠ 然後這一件整個結束了（同日）**：使用者從那八案挑中青綠，但**改給牙周**，
   > 並且**把牙周原本的藍 `#4478b5` / `#244369` 整組給矯正**。矯正因此從 L\* 36.3
   > 直接跳到 49.5，比提亮三案裡最激進的 Ⓒ 還亮 —— **提亮那一輪不必再選了**。
   > 剩下的唯一問題是牙周的青綠要幾階（`preview/spec-perio-teal/`，Ⓣ1／Ⓣ2／Ⓣ3，
   > 建議 Ⓣ2 單一值 `#2f7873`），推導在 [PALETTE.md](PALETTE.md) 第六之十六節。
5. **[PALETTE.md](PALETTE.md) 第六之八節的三組要重排**（2026-08-09 起）。
   顯微根管離開苔綠之後綠色段只剩一般牙科 151；牙周又從綠色段整個搬去藍色段（h 273），
   冷色端變成矯正 248／牙周 273／贋復 273／外科 320 四顆。整節還沒重寫。
6. **頁首與 HERO 之間要不要拉開、拉多開**（2026-08-09）。使用者的朋友回報
   「左上角 logo 的字跟 cover image 的字大小差不多，看起來會比較 noisy，可以增加 spacing」。
   實測後成立，**而且手機比電腦嚴重**：390×844 上詩的上緣（58.5px）比頁首那一塊的
   下緣（61.1px）還高，兩塊字是疊著的；1440×900 上也只有 63px。
   三案 Ⓐ 微幅／Ⓑ 大幅／Ⓒ 鬆（字距行距），提案頁 `preview/head-hero-space/`（`?sp=a|b|c|c2`），
   完整的量測與推導寫在那一頁 `<head>` 最後的註解裡。
   **只動距離，字級、顏色、照片裁切一個都沒碰；`index.html` 一個字都還沒動。**
   > **使用者已選 Ⓒ**（同日）。剩下的是**手機的距離**：Ⓒ 是 24vw（空隙 32.5px）、
   > Ⓒ＋ 是 28vw（48.1px），兩案在**電腦上完全相同**，只差手機。
   > 上限是 28vw 那一階：Ⓒ 的行距把詩撐高 19px，30vw 在 320／360 寬只剩 1～3px
   > 就壓到左邊矮房子的屋脊，Ⓑ 的 33vw 更是直接壓過去（四支寬度裡三支是負的）。
...
8. **牙周的青綠要幾階**（2026-08-09）：兩科對調本身使用者已經決定，
   只剩 Ⓣ1 兩階 `#317d78`／`#2a6d69`、Ⓣ2 單一值 `#2f7873`、Ⓣ3 `#317d78`／`#116f6a` 三選一。
   卡住的點是**全站連結色深綠松 `#214D48` 和青綠同色相**，字階壓下去會碰到它。
   提案頁 `preview/spec-perio-teal/`，建議 Ⓣ2。**選好就可以套進 `index.html`。**
...
**`#4478b5`／`#244369` 原本是牙周的，2026-08-09 起改屬齒顎矯正**，見第六之十四、十六節與
`preview/spec-perio-split/`、`preview/spec-perio-teal/`。）
