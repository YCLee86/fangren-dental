...
- `preview/head-blur-ipad/` — **頁首玻璃「招牌之前不要那麼糊」，iPad 這一輪定案 Ⓑ1（2026-08-11）**。
  ...
  > 是 `index.html` 的**完整複本**，差異清單寫在它 `<head>` 最前面。
- `preview/head-blur-mobile/` — **⚠ 提案中：同一件事的手機版，形狀三案 × 濃度三階（2026-08-11）**。
  iPad 定了 Ⓑ1 之後使用者要看手機版。網址可帶 `?m=now|m1|m2|m3`、`?L=L1|L2|L3` 與 `&mark=1`。
  預設 **Ⓜ1＋Ⓛ1 ＝ iPad 那條規則原封不動搬過來**的樣子。**正式站一個字都沒動。**
...
13. **頁首玻璃「招牌之前」要多輕**（2026-08-11 起）。**三處要分別決定，都定了才一起上線。**
    ...
    - **手機 ⚠ 提案中**，提案頁 `preview/head-blur-mobile/`，兩排切換條：
      形狀 **Ⓜ1 同 iPad／Ⓜ2 一路到底／Ⓜ3 轉場拉長**、
      濃度 **Ⓛ1 4px／Ⓛ2 3px／Ⓛ3 2px**（預設 Ⓜ1＋Ⓛ1）。
    - **電腦版 ⚠ 還沒問過。** 這一套是**通用規則**（照片的哪一列是算出來的），
      所以不特別擋的話電腦版也會跟著變輕。但電腦版的條子同時蓋到左右兩側的天空，
      細節量只有 0.34、本來就沒那麼糊 —— 要不要只在窄視窗才套，得問使用者。
    > **成因寫在第八節那兩條，不要重新目測** —— 是 `object-fit: cover` 在不同視窗
    > 比例上裁掉不同的地方，不是哪個裝置的模糊比較重（全部都是 6px）。
    > ⚠ **手機和 iPad 是兩件事，不要拿一個值套兩邊**：裁切方向相反（手機留左邊 89%、
    >   含天空；iPad 留中間 46%、純立面）、條子佔比 21% vs 5%、整張照片走完
    >   221px vs 1133px、DPR 3 vs 2。細節全部在第八節 `head-blur-mobile` 那一條。
