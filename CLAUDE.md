...
- `preview/head-blur-ipad/` — **頁首玻璃「招牌之前不要那麼糊」，iPad 這一輪定案 Ⓑ1（2026-08-11）**。
  ...
  網址可帶 `?blur=now|b1|b2|b3` 與 `&mark=1`（招牌標線）。**正式站一個字都沒動。**
  > **切換條先留著**，因為整件事還沒上線：手機版怎麼套是另一輪（見下一條），
  > 電腦版要不要跟著套也還沒問過。三處都定了才一起改 `index.html`。
...
- `preview/head-blur-mobile/` — **同一件事的手機版，定案 Ⓜ3＋Ⓛ1（2026-08-11）**：
  ...
  網址可帶 `?m=now|m1|m2|m3`、`?L=L1|L2|L3` 與 `&mark=1`。**正式站一個字都沒動。**
  ...
  > ⚠ **這個 0.45 可能會被下一輪的通則取代**（畫面上等於沒變）——
  >   見 `preview/head-blur-desktop/` 的 Ⓓ1。
  > 是 `index.html` 的**完整複本**，差異清單寫在它 `<head>` 最前面。
- `preview/head-blur-desktop/` — **⚠ 提案中：同一件事的電腦版，四案（2026-08-11，收尾這一輪）**。
  網址可帶 `?d=now|d1|d2|d3` 與 `&mark=1`，**預設 Ⓓ1**（推薦的那一案）。
  **正式站一個字都沒動。**
  ...
  > **四案**：現況／**Ⓓ1 一條規則**（4px、招牌之前 **110px** 起升，全站共用、
  > 沒有任何 @media，回頭套用到 iPad 是 f .706、手機 375 是 .407，差 10px 內）／
  > Ⓓ2 照抄手機的 f（4px、.45 起升，1440 上是 332px，等於三個尺寸三條規則）／
  > Ⓓ3 收少一階（5px，轉場同 Ⓓ1）。
  > ⚠ **Ⓓ1 是唯一不需要 @media 的一案**，和 `nav-frame-height` 那一輪同一個取向。
  > ⚠ Ⓓ1 的起升點是**反著算回來的**：先由 f=.799 換出招牌在哪個 scrollY，
  >   退 110px，再換回 f 當門檻，並夾在 f(scroll 0) 以上（視窗很矮時 110px 可能
  >   比招牌之前的全部行程還長）。所以那 110px 在每個視窗都真的是 110px。
  > 是 `index.html` 的**完整複本**，差異清單寫在它 `<head>` 最前面。
...
13. **頁首玻璃「招牌之前」要多輕**（2026-08-11 起）。**三處要分別決定，都定了才一起上線。**
    - **iPad ★ 已定案 Ⓑ1**（招牌之前 4px → 0.70~0.799 升回 6px → 招牌之後 6px）。
      提案頁 `preview/head-blur-ipad/`。落選 Ⓑ2 3px／Ⓑ3 2px。
    - **手機 ★ 已定案 Ⓜ3＋Ⓛ1**（平段 4px，**f 0.45 起升**，375 上轉場 97.8px）。
      提案頁 `preview/head-blur-mobile/`。落選 Ⓜ1 同 iPad（轉場只有 27.7px）、
      Ⓜ2 一路到底、Ⓛ2 3px／Ⓛ3 2px。
    - **電腦 ⚠ 提案中**，提案頁 `preview/head-blur-desktop/`，`?d=now|d1|d2|d3`，
      **預設也是推薦的 Ⓓ1**：平段 4px、轉場改用**捲動距離**表示 ——
      **招牌之前 110px 開始升**。這一案同時把上面兩個定案收進同一條規則
      （iPad 118px、手機 98~102px，誤差 10px 內），**全站共用、沒有任何 @media**。
      另兩案：Ⓓ2 照抄手機的 f（.45，1440 上 332px）、Ⓓ3 平段 5px。
    > ⚠ **先前說「電腦版本來就沒那麼糊、細節量只有 0.34」是錯的** ——
    > 那是捲動 0（壓在天空上）那一刻的值。整段量完電腦版接近 iPad，不像手機。
    > **手機讀起來比較不糊的主因不是左右裁切，是條子佔了 21% 的照片高**
    > （電腦 6.9%、iPad 5.2%），厚的條子會把大片天空一起平均進來。
    > 完整的數字與推導在第八節 `head-blur-desktop` 那一條，不要重新目測。
