...
- `preview/head-blur-ipad/` — **⚠ 提案中：頁首玻璃「招牌之前不要那麼糊」，三案（2026-08-11）**。
...
13. **頁首玻璃「招牌之前」要多輕**（2026-08-11 起，提案頁 `preview/head-blur-ipad/`）。
    使用者在 iPad 上回報玻璃太糊，捲到招牌附近就還好。三案等他挑：
    **Ⓑ1 4px／Ⓑ2 3px／Ⓑ3 2px**（招牌之後一律維持現況的 6px）。
    > **成因寫在第八節那一條，不要重新目測** —— 是 `object-fit: cover` 在瘦視窗上
    > 把左右的天空切掉、只留水泥立面與窗戶，不是 iPad 的模糊比較重。
    > 挑完之後要一起想的一件：這一套是**通用規則**（照片的哪一列是算出來的），
    > 所以手機與電腦版也會跟著變輕。電腦版本來就沒那麼糊（條子蓋到兩側天空），
    > 要不要只在**窄視窗**才套，是還沒問過的一題。
