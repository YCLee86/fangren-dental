...
>
> **資深 PM**（2026-09-21 從「PM」升級 —— 交件的形狀變了：精簡賅要、
> 問題／事件／解決方案三格一組、逐項編號、摘要表在最前面，見 TEAM.md 第一節）、
> 作家、事實查核師、工程師、資深美編／美感總監、資深使用者介面總監、
...
3. **「最後更新日期」由腳本維護，不要手改。** `tools/build.mjs` 用內容雜湊比對，
   ...
   > 「這一頁最後改於何時」。
   >
   > ### ✅ 2026-09-20：**文章頁又看得到 `updated` 了**（卡片沒有）
   >
   > 上面那句「不再出現在畫面上」**只剩卡片那一半還成立**。那天起
   > **十六篇的 HERO 圖下面各多一行「最後更新：2026/09/20」**
   > （`<p class="post-updated-line">`，貼著圖的下緣，借 `.post-hero figcaption`
   > 的規格）。起因是 SEO／GEO 體檢的 B5：Google 要的是**畫面與結構化資料兩邊
   > 都有、而且一致**。推導與五個落選的位置在 `/history/b5-updated.html`。
   >
   > **⚠ 首頁卡與延伸閱讀卡沒有動，仍然顯示 `published`** —— 使用者指定
   > 「首頁圖卡不用」，和上面那一段完全一致。文章頁因此同時看得到兩個日期：
   > 頂端那一行是上架日、圖下那一行是最後更新日。**不要把卡片也補上去。**
   >
   > ⚠ 那一行的字由 `tools/build.mjs` 寫（靠 `time.post-updated` 這個 class 找到），
   > **不要手改**；格式是 `2026/09/20`，不是頁尾那句在用的 `zhDate()`。
   > 它**不進內容雜湊**（`normalize()` 有一條把它換成佔位符），所以不會變成
   > 「寫進新日期 → 雜湊變了 → 下次又換成今天」的迴圈。
...
src/
  ...
  click.js              **點擊紀錄**的寫入與查詢（2026-09-20）。被 worker.js 匯入。
                        ...
                        軌跡是**另一條線、另一個資料庫**（src/paths.js），這一份本身仍然不帶代碼
  ...
  paths.js              **訪客軌跡**的寫入與報告資料（2026-09-26）。寫進**另一個資料庫** PATHS
                        （fangren-dental-paths），沒綁就安靜地不作用。識別碼只有分頁代碼
                        （sessionStorage、30 分鐘閒置換一組），**沒有 IP、UA、cookie**。
                        ⚠ 歸納不在這裡做（10 ms CPU），在 assets/path-summary.js。見 DECISIONS.md「訪客軌跡」
...
assets/
  ...
  path-summary.js       訪客軌跡的歸納（路徑、轉移表、五種訪客類型）。**只有 /admin/search/ 載它**。
                        ⚠ 規則改了要把 V 加一（src/paths.js 的 V 一起），舊的日子才會重算
...
admin/search/index.html **搜尋紀錄 ＋ 點擊紀錄 ＋ 來源紀錄 ＋ 訪客軌跡 ＋ 資料庫**（2026-09-20，網址 /admin/search/；
                        來源那一份 2026-09-25、軌跡與資料庫那兩份 2026-09-26 加的）。五份在同一頁，上面一排分頁切換，共用同一把密碼與同一排「期間」。
...
tools/
  ...
  doctor-photo-crop.mjs 醫師形象照 → assets/doctor-<slug>-400.jpg（圓頭像，顯示 76px）。
                        2026-09-19 上線，目前九位裡八位有照片（2026-09-23 謝耀慶、陳芷鈴、楊小瑩；2026-09-24 林晏妤、許馨文）。
                        ⚠ 廖立揚 2026-09-23 上線當天由使用者要求移除，FACES 裡那一段
                        註解掉留著（要放回來的五步寫在那一段裡），成品檔已刪。
  ...
  path-test.mjs         **訪客軌跡那條線的守門**（2026-09-26）。80 項（含「讀到哪裡」與「沒動作就停表」）：寫入的白名單、摘要的歸納、
...
> `published`（見第一節第 3 條），所以**首頁卡的日期與順序完全不受影響**。
> 會被誤傷的是三個機器讀的地方：`sitemap.xml` 的 `lastmod`、JSON-LD 的
> `dateModified`、頁尾那句「網站最後更新」。
>
> **⚠⚠ 2026-09-20 起它又變重了，而且比 2026-08-18 之前更重。**
> 上面那句「畫面上看不出差別了」**已經不成立** —— 文章頁的 HERO 圖下面
> 現在印著「最後更新：<日期>」（第一節第 3 條那個 2026-09-20 的補充）。
> 一次改到全部文章的 `<main>` 而不還原的話，**十六篇會一起印出當天的日期**，
> 那是讀者直接看得到的假話。動到全站共用結構之前先想這一條。
>
...
> **✅ 2026-09-20 又開又收三頁：`topic-stance/`、`topic-stance-endo/`、`topic-stance-prosth/`**
> （三個著陸頁的「那一句回應」），當天開、當天定案上線（使用者：「都選 C　定案上線」），
> 文字在 `/history/topic-stance.html`，產生器 `tools/stance-preview.mjs` 一併刪掉
> （一次性，**有進版控** —— `main` 中途被另一台推了兩次，快照要跟著重抓）。
> ⚠⚠ 這一輪**推翻了一條已定案的文案規則**（2026-08-19「lead 與 stance 一科只挑一種」），
> 定案的值與「那條規則現在怎麼寫」在 [DECISIONS.md](DECISIONS.md) 那張表最上面一列、
> 推導在 [COPY.md](COPY.md) 第九之二十一節。那一組多示範四件事：
> ・⚠⚠ **提案頁要換的是「整個元素」不是元素裡面** —— 這一輪切的是 `<ul>` ↔ `<dl>`，
>   而站上那條 `.tp-intro dl.tp-cases:last-of-type dd.tp-reply` 比的是**元素型別**；
>   把四個候選都留在 DOM 裡用 `display` 切，旁邊那個藏起來的 `dl` 會讓下緣差 6.7px，
>   **量到的就不是上線後的數字**。
> ・**要量「每一個候選各自會長到哪裡」，就在畫面外放一份影子** ——
>   複製 `.tp-intro`、同寬、`visibility: hidden`，在那裡換來換去量。
>   拿畫面上那一塊直接換，使用者會看到它跳。
> ・⚠ **守門「掃整頁等於沒掃」又差點中**（第九節第 4 條）：切換條那支腳本裡面
>   也寫著 `dd class="tp-reply"`，數整頁會數到兩個 —— 要先切出介紹那一塊再數。
> ・**候選按鈕的字要短到不折行**：第一版用「Ⓐ 不是一眼看得出」這種長標籤，
>   那一排折成兩列，切換條從 21% 長到 26%，而這一頁要判斷的正好是最上面那一塊。
...
### ⚠ 看診時間：**2026-09-20 已經收斂了**（原本三組數字）

> ## ✅ 2026-09-20：使用者把**站外**改成和網站同一組
>
> 「時間已經改好了」。確認過三件：
>
> 1. **統一成網站這一組**（08:45–11:30／13:45–16:30／17:45–20:00）——
>    **網站一個數字都沒有動**，是站外改過來的。
> 2. 已經改好的是 **Google 商家檔案** 與 **LINE「門診時間」**。
> 3. **週六早上沒有診，維持週六日休診**（這件從 2026-08-13 掛到今天才有答案）。
>
> **還沒收的兩個（都不影響搜尋）：**
>
> | | 還是舊的 | 要緊嗎 |
> | --- | --- | --- |
> | LINE「**營業時間**」 | 08:35–12:05（只有上午） | 那是 LINE 資料頁上的另一個欄位，爬得到，**有空再收** |
> | 門口的**磁鐵板** | 08:45–12:00／13:45–16:40／17:45–20:30 | 實體的牌子，**機器讀不到，完全不影響 SEO** —— 只影響站在門口的人 |

**原本的三組數字**（留著當紀錄，不要拿它回頭「訂正」任何東西）：

| | **網站 `#clinic`（現行，沒變）** | 門口的磁鐵板 ＝ 舊的 LINE「門診時間」 | LINE「營業時間」 |
| --- | --- | --- | --- |
| 上午 | **08:45–11:30** | 08:45–12:00 | 08:35–12:05 |
| 下午 | **13:45–16:30** | 13:45–16:40 | — |
| 晚上 | **17:45–20:00** | 17:45–20:00 ~~20:30~~ | — |

⚠⚠ **網站這一組是使用者明確指定的，不要「訂正」。** 2026-08-13 換版型時
我一度依板子把三個數字改成 12:00／16:40／20:30，**使用者當天就要求改回原本那組**。
2026-09-20 他把站外改過來，等於再確認了一次。
日後看到**門口的板子**和網站不一樣，那是預期的（板子還沒換），
**不要自己動網站，也不要先問** —— 問過了，答案在上面這一格。

> ✅ **週六早診那件結案了（2026-09-20）**：板子上週六早診有兩顆磁鐵
> （一般牙科＋齒顎矯正），但使用者 2026-08-13 指示「六日拿掉」，2026-09-20
> 再次確認**週六早上沒有診**。所以門診表只排週一到週五、JSON-LD 的週六日
> 是 `00:00–00:00`（＝休診），**這是對的，不是漏排**。
> ⚠ 這一條原本寫著「日後若確認週六早上有診，要補回週六那一欄」——
> **已經確認了，答案是沒有。不要再補。**
