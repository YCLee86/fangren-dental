...
- `preview/hero-contact-mobile/` — **⚠ 最新一頁：手機版要不要也放地址與電話（2026-08-10，未定案）**。
  位置使用者已指定：**瀏覽數下面、1983／9／6 上面**，所以三案只差排法
  （Ⓜ1 一行置中／Ⓜ2 一行撐開／Ⓜ3 兩行置中，`?m=m1|m2|m3`）。
...
| **窄帶右側的地址與電話** | **一行、中間 1.9rem；貼版心外緣、離照片下緣 15.18px；線條空心圖示（Lucide）；底線常駐；靜止 `.65`／hover `.95` —— 這條帶子上只准兩個白。只有電腦版，手機 `display: none`** | [PALETTE.md](PALETTE.md) 第六之十七節、`preview/hero-contact/` |
...
9. **⚠ 三格間距的 4rem「拉開」目前沒有生效**（2026-08-10 量到，還沒處理）。
   `index.html` 的 `.stats li { padding-inline: 4rem; }` 之後還有一條同權重、
   排在後面的 `.stats li { position: relative; padding-inline: 1.9rem; … }`，後者贏 ——
   **線上現在跑的是 2026-08-07 當時被淘汰的 1.9rem**（實測算出來 30.4px，
   三格方塊 330.1px 而不是 531.7px）。修法只要把 1.9rem 那一份的 `padding-inline`
   拿掉，但那會讓首頁窄帶明顯變寬，是看得見的改動，**要先問使用者**。
   詳見 [PALETTE.md](PALETTE.md) 第六之十七節第七小節。
