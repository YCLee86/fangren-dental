...
> **等上線中：`claude/light-dark-favicon-design-0j4pte` 分支**（2026-08-07）
>
> 新的 favicon（定稿幾何 ＋ 淺 `#4f716e`／深 `#4b7a60`，淺色版牙洞上移 2.05）
> 與五頁比較用的提案頁都在這條分支上，**刻意還沒併進 main**。
> 使用者要等自己在工作電腦上的其他改動處理完，再一起上線取代現有設計。
>
> 也就是說：**正式站現在還是舊的那顆手繪牙**（`#12656a` 圓角底框那個）。
> 要上線就是把這條分支併進 main 再 push，Cloudflare 幾分鐘後自動部署。
> 在那之前不要單獨把 `assets/favicon.svg` 推到 main。
...
- `preview/logo-favicon/` — favicon 深色版最早那組跨距大的五個候選（色相 147°、彩度 24%，只差亮度）。
  **顏色已於 2026-08-07 定案：淺 `#4f716e`、深 `#4b7a60`**（在 `-2way` 那頁選的），
  已經寫進 `assets/favicon.svg`。這幾頁留著當過程紀錄。
...
- `preview/logo-favicon-2way/` — **淺色五案 × 深色三案，兩邊都能換**（2026-08-07）。
  使用者覺得淺色偏深，所以往亮處做了四階：`#4f6361`（現況）`#4f6664` `#4f6a67` `#4f6d6b` `#4f716e`。
  **提亮的分寸來回三次才對**：彩度鎖死→褪色；彩度抬到底（C\*→24）→變成另一個色系；
  正解是用 OKLCH 鎖色相 188.9°、亮度與彩度一起抬，但**彩度比 C\*/L\* 只從 0.20 走到 0.29**。
  頁面底部留著三排對照帶（鎖死／適度／跟到底）。數字見 PALETTE 第六節。
...
- `preview/logo-favicon-bridge/` — **牙洞下緣的視錯覺與補償**（2026-08-07）。使用者覺得淺色版的洞
  比深色版更靠近嘴巴下緣。**量過：兩版路徑完全相同**，差別是抗鋸齒邊緣的感知位置
  （白底門檻 0.650、深底 0.332）。**使用者選 5 號（淺色版多上移 1.00）並套進
  `assets/favicon.svg`** —— 那支檔案現在是**兩條路徑**（淺 2.05／深 1.05），
  用同一個 `prefers-color-scheme` 切，**不要合成一條**。
  算出來最接近 0 的其實是 1.55，使用者比對後要 2.05（淺色略厚），照使用者的。
  數字見 PALETTE 第六節。

  > **`assets/favicon.svg` 已經換成定稿了**（2026-08-07）：定稿幾何（牙洞 1.6×、上移 1.05）
  > ＋ 淺 `#4f716e`／深 `#4b7a60`，深色靠 SVG 內嵌的 `@media (prefers-color-scheme: dark)` 切。
  > 舊的那顆手繪牙（`#12656a` 圓角底框那個）已經不在了。
