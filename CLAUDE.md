...
- `preview/doc-spec-tags/` — **⚠ 提案中：專科與專長都做成標記（2026-08-11，最新一頁）**。
  ...
  > 切換條：`?t=1|2`（**Ⓣ1 點到的才亮，預設，＝使用者要的**／Ⓣ2 專長永遠是一排標記，
  > 第一版，留著對照）、`?x=1|2`（兒童隱適美／兒童早期矯正算不算齒顎矯正，預設不算）、
  > `?v=1|0`（小字對字面中線／對基線，見下面那一條）、
  > **`?k=1|2`（命中的專長要多重：Ⓚ1 套色填滿，預設／Ⓚ2 白底字套色，2026-08-11
  > 使用者要求另做的一版）**。Ⓚ2 只動專長、**專科藥丸仍然填滿**，所以有一個層次：
  > 藥丸最重（他本科就是這個）、專長輕一階（他會做這件事）、沒命中的是純文字。
  > ⚠ Ⓚ2 的副作用：**只靠專長進來的那張卡整張會一顆填滿都沒有**（按牙周時的李柄輝，
  >   藥丸退成白底、專長也是白底）。關聯還講得出來，但份量比 Ⓚ1 輕很多。
  > **⚠ 小字往下坐，已修（Ⓥ1，2026-08-11）**：使用者回報「標籤過的字有點往下跑，
  > 專長和專科都是」。量出來兩個都成立，成因相同 —— 中文小字和大字並排時瀏覽器對的是
  > **基線**，眼睛讀的是**字面中線**。抬升量 ＝ `0.345 × (父層字級 − 自己字級) ÷ 自己字級`
  > （0.345em 是 Noto Sans TC 的中文字面中線，400px 下實測四組 0.336~0.359）。
  > 專科藥丸 `.158em`（1.82px）、專長標記 `.053em`（0.61px）。修完兩個都是 0.00px，
  > **十一個視窗量過行高與卡高一個值都沒變**（抬完仍在 strut 以內，line box 不長高）。
  > ⚠ **專科藥丸那 1.82px 是正式站本來就有的** —— `.doc-role` 沒寫 vertical-align。
  >   所以這一頁的藥丸比線上高 1.82px，**那是使用者要的，不是提案頁跑掉了**。
  > ⚠ 第一版我在專長標記上寫了 `vertical-align: -.12em`，**方向加反了**，
  >   實際低 1.99px（0.61 ＋ 1.38），那一行已經拿掉。
