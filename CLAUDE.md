...
13. **`backdrop-filter` 的那一層在「一步跳很遠」的捲動裡會漏畫一到兩幀**
   （2026-08-14，使用者回報「右上三個標籤切換的時候會有畫面閃跳的感覺」）。
   點頁首選單是 `scroll-behavior: smooth` 的錨點跳轉，背後的畫面整個換掉，
   WebKit 來不及重建那個合成層 —— 結果是**整條頁首消失 33~67ms**，
   白字落在紙色上等於憑空不見。iPad 直放的螢幕錄影拆成 30 fps：
   **三次點選單、三次都閃**，那幾幀頁首那條的平均亮度從 63.1 跳到 232.3。
   ⚠ **先確認漏畫的是哪一層再動手。** 閃的那幾幀放大看，三個玻璃框、
     `.nav-lamp`、品牌與所有白字**全都還在**，而且燈已經移到剛點的那一項 ——
     所以不是 JS 慢、也不是版面問題，不見的只有整個頁首裡唯一帶
     `backdrop-filter` ＋ `mask-image` 的 `.site-head::before`。
     沒有這一步會往 scrollspy 的節流或 `scroll-behavior` 的方向找很久。
   ⚠ 解法是**在它不做事的地方把它拆掉，再加一層畫不壞的底**：捲過窄帶之後
     `--glass` 的 alpha 是 1、`--head-fade` 是 100%，模糊與遮罩都已經看不出來，
     所以 `html.past-band` 時兩個都關掉，並把同一個實心色也畫在頁首自己身上
     （沒有濾鏡、沒有遮罩的那一層不可能被丟掉）。
     `past-band` 這個 class 從 2026-08-07 就一直在算，只是以前沒有人用。
   ⚠ **不要把條件從 `past-band` 放寬成 `scrolled`** —— 玻璃區的 alpha 只有
     .06~.84，那裡的模糊與遮罩是看得見的，拆掉等於把玻璃拿掉。
   ⚠ 這類「拿掉一個看不出來的東西」要**逐像素證明**：三個視窗 × 兩個位置，
     背景 0 個像素有差（只有字的邊緣差 1~2 階，那是合成層換了之後
     Chromium 的次像素反鋸齒，真機是灰階反鋸齒、根本沒有這一項）；
     再沿著 past-band 的分界前後各 60px 掃 13 個位置，兩邊的中位數亮度
     一位小數都一樣。
   ⚠ 驗「保險有沒有用」的方法：**用 `display: none` 把 `::before` 整層藏起來**，
     模擬漏畫的那一幀。改動前藏起來就重現了錄影裡那張白底鬼影，
     改動後條子仍然是深的。
