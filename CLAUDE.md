...
- `preview/hero-contact-mobile/` — **⚠ 最新一頁：手機版要不要也放地址與電話（2026-08-10，未定案）**。
  位置使用者已指定：**瀏覽數下面、1983／9／6 上面**，所以只差排法。
  第二輪剩兩案：**Ⓜ1 一行置中／Ⓜ3 兩行（電話在上、地址在下）**，
  外加兩行的行距四階（`?m=m1|m3&g=g1|g2|g3|g4`）。淘汰：Ⓜ2 一行撐開。
  > 那一輪修掉一件事：手機吃到了 ≤1159 那段「兩行版」的溝（`flex: 0 0 1.55rem`），
  > 於是圖示前面多了 7.68px 死空白，**盒子置中但看得見的那一塊偏右 3.61px**。
  > 手機兩案都是置中、沒有左緣要對，所以把溝關掉（`.ico` 回 `flex: 0 0 auto`、
  > 連結 gap 回 .45rem），偏移收到 0.47px。**不要改用「整組往左推幾 px」去補。**
