...
- `preview/spec-*/` — **七個科別色的推導過程（2026-08-09 一整天）**，一輪一頁：
  ...
  **`spec-ortho-palette/`（矯正換色系：品牌剩的米色＋稻草灰，加資料庫三個，八案，未定案）**、
...
4. **齒顎矯正的藍要不要提亮、提到哪一階**（2026-08-09）。使用者說套色不夠亮，
   ...
   **正式站與 `index.html` 一個字都還沒動。**
   ...
   > 結論：**稻草灰的色相能成立（傾向 Ⓔ2 `#707644`），米色調到能用就變摩卡褐**。
   > **兩條路擇一，不是接續。** 換掉藍會斷掉第六之八節「候診沙發」那個意象。
...
7. **⚠ 牙周與贋復現在是全站最近的一對**（填色 ΔE 13.3、深階 10.5，都低於原本的最小值 16.1）。
...
牙周治療的藍已定案 `#4478b5`／`#244369`，見第六之十四節與 `preview/spec-perio-split/`。）
