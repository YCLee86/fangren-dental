...
- `preview/logo-favicon-bridge/` — **牙洞下緣的視錯覺**（2026-08-07，進行中）。使用者覺得淺色版的洞
  比深色版更靠近嘴巴下緣。**量過：兩版路徑完全相同**（512px 逐欄掃描，洞的欄位、上下緣、
  最薄處都一樣），差別是 irradiation —— 亮的一側看起來會膨脹，而淺色版是白洞夾暗肉、
  深色版是亮肉夾暗洞，方向剛好相反。頁上是只動淺色版的五個補償方案
  （上移 +0.25／+0.50／+0.75、洞縮到 1.5×）。**選完才改 `assets/favicon.svg`**，
  屆時深淺兩版的幾何要拆成兩條路徑，用同一個 `prefers-color-scheme` 切。
