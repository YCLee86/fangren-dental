...
12. **一位醫師要不要出現在兩科底下**（2026-08-11 起，提案頁 `preview/doc-cross-spec/`）。
    使用者要「點牙周治療的時候也帶出李柄輝醫師的卡片，但標籤還是一般牙科」。
    **他專長欄的「牙周照護」那行文字已經上線；篩選這一件還沒定案。**
    要決定的是三件，全部在那一頁的切換條上：
    ① Ⓐ／Ⓑ／Ⓒ —— 不做／做／做並在專長欄把命中的字用細線標起來（Ⓒ 是我加的，
       因為 Ⓑ 之下會出現一張綠卡混在青綠裡，看的人不知道它為什麼在這裡）。
    ② Ⓝ1／Ⓝ2 —— 只做李柄輝一位（特例），還是同一條規則套到全部九位（規則）。
       ⚠ **這一題比 ① 重要**，見那一頁列的四筆。
    ③ 要不要把「顏色 ＝ 篩選」這個不變量正式放掉並寫進 PALETTE.md 第六之二節。
    > ⚠ **不要用「把 `data-spec` 改成 perio」解這題** —— 卡片會跟著變青綠，
    >   使用者已經明講要維持一般牙科的標籤。做法是新的 `data-spec-also`。
