...
4. **顯微根管要不要改紅色系、改成哪一案**（2026-08-09）。五案全部從資料庫實測值抓的，
   提案頁 `preview/spec-endo-red/`（網址可帶 `?c=1|2|3|4|5`），
   推導與數字在 [PALETTE.md](PALETTE.md) 第六之十一節。
   **正式站與 `index.html` 一個字都還沒動。**
   > 改了會放掉兩樣：顯微根管現在拿的是 **Logo 自己的苔綠**（六之八節說那是七科裡最強的
   > 對應），以及使用者自己定過的「紅在品牌裡的暗示性太強」。**兩件都已經告知過**，
   > 不要當成新問題再提一次；他要改就照做。
（兒牙白字那一件已經結案了，見上面的定案表。口腔外科的紫已定案 `#8e6299`／`#784e84`，
見 PALETTE.md 第六之九節與 `preview/spec-surg-violet/`；贋復假牙的藍已定案
`#4a5789`／`#364067`，見第六之十節與 `preview/spec-prosth-blue/`。）
