...
- `preview/doc-spec-tags/` — **⚠ 提案中：專科與專長都做成標記（2026-08-11，最新一頁）**。
  ...
  > **這一頁把上一頁整個取代掉，而且做法更乾淨**：專長從一句頓號串起來的文字變成
  > **一排標記**（九位共 32 顆），每顆自己帶 `data-spec`，所以顏色自動是那一科的。
  > **篩選改讀這些標記** —— 一位醫師被某一科篩到的條件是「專科 ＝ 那一科」**或**
  > 「專長裡有一顆是那一科」。上一頁那個看不見的 `data-spec-also` 因此**不需要了**，
  > 「篩得到什麼」和「畫面上寫什麼」再也不可能對不起來。
  > **兩態沿用這一站原本的標記語彙**（PALETTE.md 第六之二節，一個新顏色都沒有）：
  > 套色填滿 ＝「這一顆就是你正在看的那一科」，白底字套色 ＝「他是這個，但不是你篩的」。
  > 所以按牙周治療時李侑津的藥丸是實心青綠，李柄輝的「創辦醫師」退成白底綠字、
  > 但他的**牙周照護**那顆亮成實心青綠 —— 兩張卡為什麼並排、關聯有什麼不同，一眼就懂。
  > 尺寸也沿用：專長標記和 `.doc-role` 完全同一組（.72rem／1.5／.05rem .6rem／8px）。
  > 切換條：`?f=1|2|3`（**沒篩選時**的樣子 —— Ⓕ1 專科套色＋專長白底（預設，＝現在線上
  > 那個份量）／Ⓕ2 全部套色（照字面，41 顆實心）／Ⓕ3 全部白底。**按下任何一科之後三案
  > 完全一樣**）、`?x=1|2`（兒童隱適美／兒童早期矯正算不算齒顎矯正，預設不算）。
  > ⚠ **上一頁的 Ⓝ1／Ⓝ2 在這一版自動變成「全部九位」** —— 規則是從專長長出來的，
  >   沒辦法只套在一個人身上。這是使用者這次的說法帶來的結果。
  > ⚠ **維護成本只有一個**：專長 → 科別的對照是寫死在 HTML 屬性上的，日後新增或修改
  >   任何一位醫師的專長，就要順手給那個詞一個 `data-spec`，不然它會是灰的
  >   （介面灰 `all`）而且不會被任何一科篩到。完整對照表在那一頁 `<head>`。
  > 是 `index.html` 的**完整複本**，差異清單寫在它 `<head>` 最前面。
