...
| 窄帶底色方向 | **毛玻璃 ＋ 柏油漸層**（藍黑／柏油／墨色三個實色案已淘汰） | 同上 |
| 兒牙的科別色 | **1a `#c28229`**（十一輪；字與框用 `#9e6301`）。兩個提案頁都已套上、切換條已刪 | [PALETTE.md](PALETTE.md) 第六之二節 |
| chip 的兩態 | 未選＝白底＋深階字與框，選取＝套色填滿。**兒牙不例外** | 同上 |

### 還沒決定的

1. **柏油漸層要「標準」還是「深」**（差別只有四個 alpha 停點，見 PALETTE 第六之三節第 2 點）。
2. **要不要把定案套進 `preview/home-desktop/`、拿掉所有切換條**，並刪掉中間過程的幾頁。
3. COPY.md 第六節列的兩件：chips 按下的狀態要不要降一階、故事類內容之後怎麼進「主題與科別」。
4. HERO 收尾詩的三案 A／B／C（[COPY.md](COPY.md) 第四節），從 2026-08-05 就擱著。
5. **兒牙實心藥丸上的白字只有 3.22:1，不到 AA 的 4.5:1。**
   使用者要求維持白字，所以先照做，但**上線前一定要收掉**。
   兩條路（改一行就到）寫在 PALETTE 第六之二節的定案段落。
