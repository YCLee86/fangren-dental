...
- `preview/doc-spec-tags/` — **⚠ 四輪之後的定稿版，尚未上線（2026-08-11）**。
  主題與科別按下去的時候，把有關聯的醫師與文章都帶出來（醫師的關聯**同時看專科與
  專長**），並讓被點到的那幾顆自己亮起來說「我就是原因」。**切換條已刪，畫面上就是
  定案的樣子；正式站的 `index.html` 一個字都沒動。**
...
12. **醫師卡與文章卡：點到的科別才亮起來**（2026-08-11，提案頁
    **`preview/doc-spec-tags/`**）。**設計已經全部定案，只差「要不要上線」——
    正式站的 `index.html` 到現在一個字都沒動。**
    起因是「李柄輝醫師的專長加上牙周照護」（**那行文字已經上線**），接著五輪收斂成
    現在這個樣子：主題與科別按下去 → 帶出有關聯的醫師（專科**或**專長命中）與文章，
    命中的專長淡色填滿、沒命中的維持純文字、專科藥丸不是那一科就退成白底字套色、
    **文章卡的主題標籤套色填滿**。沒點任何一科時和正式站長得一模一樣。
    **完整做法、定案值、落選案與實測數字見上面第八節那一條。**
    定案：**Ⓚ3 淡色填滿・Ⓟ1 12%・Ⓩ2 .748rem・Ⓧ1 兒童矯正只算兒童牙科**。
    > **上線前還要處理的兩件：**
    > ① 要不要把「顏色 ＝ 篩選」這個不變量正式放掉，並寫進 PALETTE.md 第六之二節。
    >   這一版的代價還是它 —— 一張綠卡會出現在牙周底下 —— 只是亮起來的那顆標記
    >   把理由寫在臉上了。
    > ② 搬回 `index.html` 的時候，要帶的是：專長那句話的 `<span class="sk">` 包法、
    >   `.sk` / `.sk.tag-on` / `.doc-role.tag-off` / `.card-tag.tag-on` 四條 CSS、
    >   `.doc-role` 那行 `vertical-align`、`specHit()`、`apply()` 最後那段刷狀態的
    >   迴圈，以及 `measureLift()`。**提案頁的示範瀏覽數不要帶回去。**
    > ⚠ **不要用「把 `data-spec` 改成 perio」解這件事** —— 卡片會跟著變青綠，
    >   使用者已經明講要維持一般牙科的標籤。
    > ⚠ **`preview/doc-cross-spec/` 已經被取代**，留著看推論就好，不要拿去實作。
    > ~~Ⓕ／Ⓣ／Ⓥ 三排切換條~~ —— 都在過程中被使用者的後續說法收掉了，
    > 落選案的數字留在提案頁的 CSS 註解裡。
