...
- `preview/spec-tag-fit/` — **⚠ 提案中：專長標記要怎麼貼合它包住的那幾個字（2026-08-11）**。
  ...
  **下面比上面多 62%**；側邊留白 ÷ 字級 0.909，而同一張卡的 `.doc dt` 只有 0.553。
  > **`index.html` 已經套上預設的那一組**（Ⓥ1 ＋ Ⓧ2），這一頁是給使用者比另外兩階用的。
  > 切換條：`?fit=now|new`（現況／新版）、`?v=v1|v2|v3`（上下留白 **.385** ／.32／.28）、
  > `?x=x1|x2|x3`（側邊 .5／**.42**／.34rem）。
  > **做法**：`line-height: 1`，塊高改由上下兩個內距各自負責，由 JS 現量成
  > 「字面上下各留一樣多」。上下不對稱從 −2.18px 收到 **−0.01px**。
  > **Ⓥ1 的 0.385 是「等高」** —— 套用後塊高 21.56px，和改動前的 21.55 一樣；
  > 使用者只說位置不對、沒說塊太高，所以預設一個像素都不動高度。
