...
6. **手機版 HERO 的詩要怎麼排**（2026-08-09 起）。起因是使用者的朋友說
   ...
   >
   > **使用者 2026-08-10 選了 Ⓢ1**（`--m-top: 17.9vw`、字距 .13em、全形空格 .65em，
   > 行高與首行間隔維持現況 1.75／.55em；375 上空隙 8.0px）。**還沒套進 `index.html`** ——
   > 等第 10 項那件陰影一起做，免得同一塊改兩次。
...
10. **收尾句「巷口」壓在車頭燈上，陰影要怎麼加**（2026-08-10）。使用者選完 Ⓢ1 後回報。
    逐字量過照片底下的亮度（375 寬）：「巷」最亮 219、「口」最亮 236，
    對比度掉到 1.24／1.12（1.0 ＝ 完全看不見），其他十個字都在 3～7。
    **這是手機才有的問題** —— 電腦版的裁切是下緣貼齊，收尾句落在純暗的路面上，
    十二個字最亮處都 ≤58、對比度 ≥5.9，完全不需要處理。
    兩案在 `preview/hero-mobile-skyline/`（`?e=e1|e2`，可和 `?m=` 任意組合）：
    Ⓔ1 只加厚字自己的 text-shadow（關鍵是最內層 `0 0 2px /.9`，跟著筆畫走，
    對點光源最有效）、Ⓔ2 只把 `::before` 那塊模糊暗塊加深加大
    （`.72 → .9`、範圍放大，**blur 一個值都不動** —— 一收就會看得出是個框，
    那正是這塊東西當初刻意要避免的）。
    **兩案目前沒有限定只在手機生效，定案時要決定要不要關進 `@media (max-width: 720px)`。**
    Ⓢ1 與這一件會一起套進 `index.html`。
