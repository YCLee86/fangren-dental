...
- `preview/nav-frame-height/` — **⚠ 最新一頁：選單那三個玻璃框的幾何，手機版（2026-08-11）。
  第一輪「多高」已定案 Ⓥ1，第二輪「圓角多小」三案未定案。**
  起因是使用者回報「玻璃框的垂直段好像有點長，但字沒那麼大」。**量出來成立**：
  框是圓角 12px 的矩形，四個角吃掉 12px，中間剩下的那段直線才是眼睛讀到的「垂直段」——
  390 上是 **12.28px，和一個字（12.48）一樣長**。
  網址可帶 `?r=now|r12|r10|r8|r6`，切換條右邊有現量的讀數
  （圓角／弧佔周長／框高／直段）。
  > **第二輪：圓角要多小（未定案）。** 使用者選完高度後說「圓角的圓要小一點」。
  > 高度固定在 Ⓥ1 2.65，只動圓角，三案等距一階：**Ⓡ1 10px／Ⓡ2 8px／Ⓡ3 6px**。
  > 用「**弧佔周長**」（四個角的弧長 ÷ 整圈周長）當「讀起來有多圓」的量，390 實測：
  > Ⓡ12 46.7%／Ⓡ1 38.1%／Ⓡ2 29.9%／Ⓡ3 21.9%。
  > ⚠ **這件事是上一輪自己引出來的，不是使用者反覆**：圓角是固定 px、框高變小了，
  >   角就吃掉更大的比例 —— 同一個 12px，現況是 44.9%、收高之後變成 **46.7%，更圓**。
  >   320 上 Ⓡ12 是 **54.9%，一半以上的邊是彎的**。
  > ⚠ **12px 是全站規則**（本節上面 `nav-scrollspy` 那條：標記、搜尋框、三種卡片
  >   都是這一個，不要另外挑）。選單框改小＝頁首脫離那個家族，是使用者指定的，
  >   記在這裡是為了讓日後看到「為什麼只有頁首不一樣」的人知道不是漏改。
  >   **但 6px 不是新數字** —— `.doc dt`（醫師卡那三顆「專長／資歷／學歷」標籤）
  >   本來就是 6px，所以 Ⓡ3 是回到系統裡已經有的第二個值。
  > ⚠ **圓角維持 px、不要跟著字級走。** 這一站的圓角是「材質」不是「比例」：
  >   434px 高的文章卡和 31px 高的標記用同一個 12px。高度改 em 是因為它要跟著字，
  >   圓角不是。
  > ⚠ **框（`a::before`）和底（`.nav-lamp`）要吃同一個值**，兩個是同一個矩形的
  >   線與面，只改一邊會露出一圈錯位的邊。這一頁用同一個 `--frame-r` 餵兩者。
  > 圓角只改角、不改寬高：`--lamp-pad` 那兩邊與燈箱的 x/w/y/h 都不受影響，
  > 四個寬度實測過。
  ...
  > 左右那 3.74px 動不了（框與框要留 5px，第八節 `nav-scrollspy` 那個 320 的坑），
  > 所以只能收上下。
  > **對照組**：主題與科別那排標記（`.chips button`）是 31 ÷ 13.12 ＝ **2.36**，
  > 而選單的框是 36.28 ÷ 12.48 ＝ **2.91** —— 字更小、左右更擠，框卻比標記還高。
  > 三案（框高 ÷ 字級）：**Ⓥ1 2.65／Ⓥ2 2.36（＝標記）／Ⓥ3 2.15**。
  > **使用者選 Ⓥ1「收一階剛剛好」**，2.65 從此是這一頁的底盤，切換鈕已刪，
  > 三案的數字留在那一頁 `<head>` 的表裡。
  > ⚠ **落選的 Ⓥ3 在 320 上 H ＝ 23.64 < 24，圓角上下接在一起、變成膠囊**，直段消失。
  >   規則是「框高 < 圓角 ×2」就會發生 —— 第二輪把圓角改小，這條線跟著往下移。
  > ⚠ **`a` 的 `padding-block` 一個字都不能動。** 框是絕對定位的 ::before，改它不影響版面；
  >   改 padding 會改到 `a` 的高 → 頁首那列的列高 → 頁首總高 59.17 →
  >   **HERO 那首詩的空隙 8.0px**（Ⓢ1 定案）。三案實測 `a` 高與頁首高在四個寬度都不變。
  > 順手修掉一個原本就歪的漂移：現況框高 ＝ `1.85em ＋ 11.2px`，一半跟字級走、
  > 一半是固定 px，所以「框高 ÷ 字級」在 320／375／390／430 是 3.05／2.95／2.91／2.86 ——
  > **手機愈窄框相對愈長**，正好是最擠的那一端。三案改成純 em，四個寬度一致。
  > 這和 HERO 那輪「手機的幾何原本不是等比例的」是同一件事（第九節第 6 項第 3 點）。
  > **電腦版與平板一個值都沒動**（使用者：先做手機版就好），已和正式站逐項比對過。
  > ⚠ **這一頁踩到一個新的坑，日後複製 `index.html` 做提案頁一定要記得**：
  >   HERO 那張圖現在是 `src` ＋ **`srcset`** 兩份路徑，只把 `src=` 換成絕對路徑不夠 ——
...
0.5 **那三個玻璃框的幾何（手機版）** —— 2026-08-11 開的，頁面在
   `preview/nav-frame-height/`。
   ~~第一輪「要多高」~~ —— **已定案 Ⓥ1 2.65**（使用者：「收一階剛剛好」）。
   **第二輪「圓角要多小」還沒定**：`?r=now|r12|r10|r8|r6`，
   三案 **Ⓡ1 10px／Ⓡ2 8px／Ⓡ3 6px**（6px ＝ `.doc dt`，系統裡本來就有的第二個值）。
   數字與成因見第八節那一條。
   > **兩輪都定案之後，跟著要做的三件：**
   > ① 高度與圓角一起套進 `index.html` 的手機 @media，切換條連同 `data-r` 一起刪。
   > ② `moveLamp()` 要一起搬 —— 框是 CSS 畫的、底是 JS 餵的，
   >   兩邊各算各的就會露出半個像素的錯位。提案頁把它抽成 `frameBox()`，
   >   讀不到 `--frame-em` 就退回原本的「外擴 1px」，電腦版因此完全不受影響。
   > ③ `.nav-lamp` 的 `border-radius` 要和框一起改（同一個 `--frame-r`）。
   > **電腦版要不要跟著收，是還沒問過的一題**（兩輪使用者都指定只做手機版）。
   > 定案後電腦版會是「高 2.86～3.05 ＋ 圓角 12px」、手機版是新的值，兩邊不一樣。
