...
8. **設計註解只留在原始碼，絕對不上線。**（2026-08-13 起）
   `tools/dist.mjs` 會呼叫 `tools/strip-notes.mjs`，把 `_site/` 裡 `.html`／`.css`／`.js`
   的註解全部剝掉。**原始碼一個字都不動** —— 決策紀錄照舊寫在註解裡，這一節其他條、
   還有第九節那一整套「寫進註解」的規矩全部不變，改變的只有訪客拿到的那一份。
   起因：首頁 gzip 94.2 KB 裡有 71.3 KB（**76%**）是中文推導，每個訪客都在下載。
   剝完之後首頁 21.8 KB、`style.css` 15.9 → 3.7 KB，全站少 101 KB。
   > ⚠ **不要因此少寫註解，也不要為了瘦身去刪既有的註解。** 現在寫多少都不影響訪客。
   > ⚠ **這不是 minifier**，程式碼一個位元組都沒改。所以「畫面完全一樣」是可以逐項
   >   證明的（八種視窗 × 八頁，computed style、盒子、innerText、全頁截圖逐像素）。
   >   日後若有人想加上真正的壓縮，那是另一件事，要重新驗一次。
   > ⚠ 剝完會過語法檢查（內嵌 JS 能 parse、JSON-LD 仍合法、CSS 大括號成對），
   >   任何一項不過就讓建置失敗 —— 這類錯誤在瀏覽器裡**不會報錯**，只會讓後面
   >   整段悄悄失效。
   > ⚠ 要看剝之前的 `_site/`：`DIST_KEEP_NOTES=1 node tools/dist.mjs`。
   > ⚠ **repo 是 public，所以註解在 GitHub 上仍然讀得到。** 這一條擋的是
   >   「訪客開 fangren.net 檢視原始碼」，不是把內容變成祕密。
...
tools/
  ...
  dist.mjs              把上線檔案組進 _site/，並呼叫 strip-notes.mjs 剝掉設計註解
  strip-notes.mjs       把註解從 _site/ 的複本剝掉（原始碼不動）。見第一節第 8 條
  serve.mjs             本機預覽伺服器
...
13. **`getComputedStyle` 的自訂屬性列舉順序不能拿來比對**（2026-08-13，驗證剝註解時踩到）。
   要證明「剝掉註解之後畫面完全一樣」，最直接的做法是把每個元素的 computed style
   全部倒出來比字串。**結果 64 組全部報告不一樣，而且是假陽性** —— 屬性有 451 個、
   兩邊的**集合與每一個值都相同**，只有 `--tile` 與 `--frame-r` 這些自訂屬性的
   **排列順序**不同。那是 Blink 內部雜湊表的順序，同一份檔案穩定、換一份檔案就可能
   換一種排法，頁面完全觀察不到（盒子、innerText、全頁截圖逐像素都一樣）。
   **比對前要先 sort。**
   ⚠ 一起學到的是**驗證腳本自己要先有對照組**：拿原始版本跟**它自己**比一次，
     對照組必須是 0 差異，後面測到的差異才能歸咎於改動。第一版沒有對照組，
     花了三輪才分清楚「真的有差」和「腳本太嚴格」。
   ⚠ 第二個假陽性來源是**截圖時圖片還沒畫完**：只等 `load` ＋固定秒數，
     大張的 HERO 圖偶爾會有一次沒趕上，12/64 組因此誤報。要等到
     `document.fonts.ready` 而且每一張 `img` 都 `decode()` 完才截圖。
