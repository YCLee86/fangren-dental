...
- `preview/doc-spec-tags/` — **⚠ 提案中：專科與專長都做成標記（2026-08-11，最新一頁）**。
  使用者：「主題與科別標籤被點選時，出現有關聯的醫師，包括專科、專長。專科標籤和專長
  都做成套色標籤。如果醫師本身的標籤不是這個主題，改成白底字套色的標籤。」
  接著同一天使用者收掉了其中一半：「主題科別標籤沒有點選的，專長不需要套標籤，
  像原來一樣。是點下標籤之後，點到的專科跟專長再套標籤。」**現在這一頁是後者。**
  > **這一頁把上一頁整個取代掉，而且做法更乾淨。** 專長那句話**一個字都沒動**，
  > 只是把每個詞包一層 `<span class="sk" data-spec="…">`、每個頓號包一層
  > `<span class="sep">`。標記**只在按下某一科的那一刻長出來，而且只長在命中的詞上**。
  > **篩選改讀那些 span** —— 被某一科篩到的條件是「專科 ＝ 那一科」**或**「專長裡有
  > 一個詞屬於那一科」。上一頁那個看不見的 `data-spec-also` 因此**不需要了**，
  > 「篩得到什麼」和「畫面上寫什麼」再也不可能對不起來。
  > 三個狀態：**沒點任何一科** ＝ 和正式站完全一樣（藥丸套色、專長純文字）；
  > **點了某一科** ＝ 命中的專長變成**套色填滿**的標記、沒命中的**維持純文字**，
  > 專科藥丸 ＝ 那一科就填滿、≠ 就退成**白底字套色**。
  > 所以按牙周治療時李侑津的藥丸是實心青綠、他三個牙周的詞亮起來（植牙假牙那三個
  > 維持文字），李柄輝的「創辦醫師」退成白底綠字、但**牙周照護**亮成實心青綠 ——
  > 兩張卡為什麼並排、關聯有什麼不同，一眼就懂，而且畫面沒有變吵。
  > 材質沿用：亮起來的標記和 `.doc-role` 完全同一組（.72rem／1.5／.05rem .6rem／8px），
  > 兩態的顏色就是這一站現成的兩態（PALETTE.md 第六之二節），**沒有新顏色、新圓角**。
  > 切換條四排：`?k=1|2|3`（命中的專長要多重）、`?p=1|2|3`（Ⓚ3 的濃度）、
  > **`?z=1|2|3|4|5`（標記的字級）**、`?x=1|2`（兒童矯正算不算齒顎矯正，預設不算）。
  >
  > **Ⓩ —— 標記的字級**（2026-08-11 第三輪，使用者：「套色之後那個字好像變小了，
  > 做幾版把字放大一點；一個是現在的、一個是原來自己的大小套色，中間再做三個」）。
  > 兩端定死、中間等距內插：**Ⓩ1 .72rem／11.52px（現在，＝專科藥丸那一組，預設）**、
  > Ⓩ2 .748／11.97、Ⓩ3 .775／12.40、Ⓩ4 .803／12.85、
  > **Ⓩ5 .83rem／13.28px（＝旁邊那句話的字級，字完全沒縮）**。
  > ⚠ **使用者猜的「原大會很擠」看得出來**：Ⓩ5 的塊高 23.52px 已經超過 dd 的行高
  >   23.24px，塊幾乎把整行填滿。Ⓩ1~Ⓩ4 都塞得下。
  > ⚠⚠ **抬升量不能寫死，要 JS 現量** —— Blink 把 inline-block 的**塊內基線位置
  >   進位到整數 px**（實測「基線離塊頂」五階是 13.797／13.797／14.797／15.797／
  >   15.797，跳階不是線性），而且**同一個字級在不同 DPR 上落在不同的整數**
  >   （Ⓩ4 在 DPR1 要抬 0.274px、DPR3 要抬 1.274px）。所以改成開頁時放一顆看不見的
  >   探針量一次，字級一換就重量，寫進 `--sk-lift`。**和 `nav-scrollspy` 的
  >   `--lamp-pad` 是同一個做法、同一個理由。** 四個寬度 × 五個字級實測，
  >   上下不對稱全部 0.02px。
  > ⚠ **原本的 Ⓣ 那一排已經刪掉** —— 使用者明講「沒有點選的專長不需要套標籤，
  >   像原來一樣」，Ⓣ2（專長永遠是一排標記）落選。
  >
  > **Ⓚ —— 命中的專長要多重**（三案都只動專長，**專科藥丸永遠填滿**）：
  > Ⓚ1 套色填滿（白字，預設）／Ⓚ2 白底字套色／**Ⓚ3 淡色填滿**
  > （2026-08-11 使用者：「實心套色，但這個顏色刷淡」）。
  > Ⓚ3 的底 ＝ 該科 `--accent` 依 `--tint` 混進 `--card`，字用 `--accent-deep`，
  > **框和底同色**（框不能拿掉，那 1px 算在高度裡，拿掉三案的塊高就不一樣）。
  > ⚠ **淡底會吃掉深階字的對比**，實測（字對淡底）：
  >   12% → 兒牙 4.01 掉出 AA，其餘 4.74~10.42 全過；
  >   20% → 兒牙 3.71、牙周 4.26 掉出；30% → 再加口外 4.06，共三科掉出。
  >   **只有 12% 是「除了兒牙以外全過」**，而兒牙本來就有一條已定案的 AA 例外
  >   （填色白字 3.22）。切換條右邊會**現算**這七個數字，不必查表。
  > ⚠ Ⓚ2／Ⓚ3 的副作用：**只靠專長進來的那張卡整張沒有一顆滿色**（按牙周時的李柄輝，
  >   藥丸退成白底、專長也是淡的）。關聯還講得出來，但份量比 Ⓚ1 輕。
  >
  > **⚠ 小字往下坐，兩輪（2026-08-11）**：
  > **第一輪**使用者回報「標籤過的字有點往下跑，專長和專科都是」。成因是中文小字和
  > 大字並排時瀏覽器對的是**基線**、眼睛讀的是**字面中線**。抬升量 ＝
  > `0.345 × (父層字級 − 自己字級) ÷ 自己字級`（0.345em 是 Noto Sans TC 的中文字面
  > 中線，400px 下實測四組 0.336~0.359）→ 藥丸 `.158em`(1.82px)、標記 `.053em`(0.61px)。
  > **第二輪**使用者：「專科有對齊名字的垂直中線，但專長上標籤後看起來是對齊沒被標籤的
  > 文字下緣。」**成立，而且第一輪對錯了東西** —— **填滿的標記，眼睛讀的是那個色塊，
  > 不是塊裡面的字**，而塊裡的字本來就比塊的中線高 0.66px（塊高是 line-height ＋
  > 內距 ＋ 框算出來的，對的是字型 ascent／descent，不是中文字面）。所以「字對齊了」
  > 等於「塊往下掉 0.66px」。1440 實測不對稱量：藥丸 1.38px、標記 1.33px，
  > **標記比較明顯是因為它旁邊的字只有 13.28px，4.53px 的下墜佔了三分之一行高**。
  > 修法是把抬升量從「字對字」加到「塊對字」：藥丸 `.218em`(2.51px)、標記 `.111em`(1.28px)。
  > **定案 Ⓥ3 ＝ 只改專長**（標記 .111em，藥丸維持 .158em）—— 使用者明講專科那顆
  > 現在是對的。修完標記的不對稱從 1.33px 降到 **0.02px**。
  > Ⓥ2（兩顆都用同一條規則，藥丸再高 0.69px）留在切換條上比較。
  > ⚠ **專科藥丸那 1.82px 是正式站本來就有的** —— `.doc-role` 沒寫 vertical-align。
  >   所以這一頁的藥丸比線上高 1.82px，**那是使用者要的，不是提案頁跑掉了**。
  > ⚠ 三案都不會撐高行框（抬完標記離基線 15.70 < strut 16.22、藥丸 16.94 < 18.42），
  >   四個寬度量過整區高度與九張卡高度一個值都沒變。
  > ⚠ 第一版我在標記上寫過 `vertical-align: -.12em`，**方向加反了**（正值才是往上），
  >   實際低 1.99px。那一行早就拿掉了。
  > ⚠ **上一頁的 Ⓝ1／Ⓝ2 在這一版自動變成「全部九位」** —— 規則是從專長長出來的，
  >   沒辦法只套在一個人身上。這是使用者這次的說法帶來的結果。
  > ⚠ **維護成本只有一個**：專長 → 科別的對照是寫死在 HTML 屬性上的，日後新增或修改
  >   任何一位醫師的專長，就要順手給那個詞一個 `data-spec`，不然它不會被任何一科
  >   篩到，也永遠不會亮起來。完整對照表（32 個詞）在那一頁 `<head>`。
  > ⚠ **頓號不要用 `<i>` 包** —— `<i>` 預設是斜體，中文沒有義式斜體、瀏覽器會**合成**
  >   一個傾斜的頓號，畫面上真的看得出來。這一頁第一版踩過，改成 `<span>`。
  > 「沒篩選時和正式站一樣」是量過的：醫師介紹整區 81 個元素的 top／height／width
  > 一個值都不差；逐像素只剩字緣反鋸齒有差，成因是拆成 span 會切斷 shaping run，
  > 整行推進量累積差 **0.04px**，放大四倍逐字看不出來。
  > 是 `index.html` 的**完整複本**，差異清單寫在它 `<head>` 最前面。
...
12. **醫師卡：專科與專長都做成標記，篩選改讀標記**（2026-08-11 起，
    提案頁 **`preview/doc-spec-tags/`**）。**正式站一個字都沒動。**
    起因是「李柄輝醫師的專長加上牙周照護」（**那行文字已經上線**），接著使用者要
    「點牙周治療也帶出他的卡片，但標籤還是一般牙科」，再接著擴大成
    「主題與科別點下去就出現有關聯的醫師，包括專科與專長，兩者都做成套色標籤；
      醫師本身的標籤不是這個主題就改成白底字套色」。
    最後再收一次：「主題科別標籤沒有點選的，專長不需要套標籤，像原來一樣。
    是點下標籤之後，點到的專科跟專長再套標籤。」**提案頁已經是這一版**（Ⓣ1 預設），
    沒篩選時和正式站長得一模一樣。
    **做法與畫面見上面第八節那一條**，這裡只記還沒回答的：
    ① **Ⓧ1／Ⓧ2 —— 兒童隱適美、兒童早期矯正算不算齒顎矯正。** 兒童早期矯正和兩位
       矯正專科醫師做的不是同一件事，但字面上都是矯正。Ⓧ2 會讓矯正從 2 位變 4 位。
    ② 要不要把「顏色 ＝ 篩選」這個不變量正式放掉並寫進 PALETTE.md 第六之二節。
       這一版的代價還是它 —— 一張綠卡會出現在牙周底下 —— 只是亮起來的那顆標記
       把理由寫在臉上了。
    > ~~Ⓕ1／Ⓕ2／Ⓕ3（沒篩選時要多重）~~ —— 使用者用第三次那句話回答了：
    > **沒篩選就什麼標籤都不要**，等於三案全部落選。Ⓣ2 留著只是對照。
    > ⚠ **不要用「把 `data-spec` 改成 perio」解這件事** —— 卡片會跟著變青綠，
    >   使用者已經明講要維持一般牙科的標籤。
    > ⚠ **`preview/doc-cross-spec/` 那一頁（Ⓐ／Ⓑ／Ⓒ、Ⓝ1／Ⓝ2）已經被取代**，
    >   它的 Ⓒ 想解的問題這一版用標記本身解掉了，Ⓝ1／Ⓝ2 也自動變成「全部九位」。
    >   留著看推論就好，不要再拿去實作。
