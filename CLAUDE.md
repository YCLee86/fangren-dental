...
- `preview/logo-favicon-2way/` — **淺色五案 × 深色三案，兩邊都能換**（2026-08-07）。
  使用者覺得淺色偏深，所以往亮處做了四階：`#4f6361`（現況）`#496865` `#426d69` `#39716d` `#2d7671`。
  **提亮時彩度一定要跟著抬**：色相角（189.7°）鎖住，L\* 40.33→45.23 配 C\* 8.10→24.03。
  第一版把 C\* 鎖在 8.10 只抬 L\*，使用者一眼就說「這是褪色，不是變亮」——
  頁面底部留著那一版當對照帶。細節見 PALETTE 第六節。
