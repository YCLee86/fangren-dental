...
| **照片與窄帶的接縫** | **兩層的 alpha 一律改成 smoothstep（顏色一個都沒動）；柏油層在電腦／旋轉螢幕往上多長 64px；窄帶接在照片下面的兩種版型（手機、iPad）柏油層收到 alpha 1；手機的窄帶因此從 `#161413` 起跑，不再是 `--band-top`**（2026-08-12 上線）。三種版型三個成因：手機**完全沒有過渡**（ΔL\* +2.69~+3.65、暖轉冷）、iPad 是**殘影從 12% 突然斷成 0**、電腦／旋轉是**線性漸層起點的一階導數不連續**（馬赫帶）。二十二個視窗量過，接縫 ΔL\* 全部 ≤ 0.03。⚠ 手機那一層的 alpha 是 **S²** 不是 S（照片只有 291px 高，用 S 會把路面與門面整片壓掉）。⚠ 這**不是**六之三節否決的「深一階」—— 那一案不透明的位置在窄帶**下緣**（螢幕最底），這裡在**照片與窄帶的交界**，窄帶從同一個顏色接下去 | [PALETTE.md](PALETTE.md) 第六之十九節 |
...
16. ~~照片與窄帶之間看得到一條分界線~~ —— **2026-08-12 定案並上線**，
    見上面的定案表與 [PALETTE.md](PALETTE.md) 第六之十九節。
    > **留下來的三條規則：**
    > ・**兩層的 alpha 一律是 smoothstep，不要改回線性。** 線性漸層的起點就是一條線
    >   （一階導數跳階），這是這一輪查出來的成因之一，和顏色無關。
    > ・**手機那一層是 S²，不是 S。** 照片只有 291px 高，用 S 會把路面與亮著燈的
    >   門面整片壓掉。iPad／電腦那兩層壓住的本來就只有柏油路，用一般的 S。
    > ・**柏油層往上長只有電腦／旋轉螢幕那一份（`inset: -64px 0 0 0`）。**
    >   它是純顏色、沒有 `backdrop-filter`，所以往上長不會有框邊；
    >   模糊層維持在窄帶自己那 84px 裡，**不要順手把它一起往上拉**。
    >   也因為這一層蓋到收尾句的高度，`.hero-copy` 的 `z-index` 現在是 4（基底就抬了）。
