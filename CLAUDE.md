...
| **看診時間本身** | **維持 08:45–11:30／13:45–16:30／17:45–20:00**（使用者 2026-08-13 指定）。⚠⚠ 換版型的過程中曾一度改成板子上的 12:00／16:40／20:30，**使用者當天就指定改回來** —— 所以**不要再依板子或 LINE「訂正」這三個數字**，那是明確的決定，不是漏改。第十節那筆三邊不一致因此**仍然成立**。休診日沒變（週六、週日）。底下那句換成**「國定假日與各科特別門診請來電確認。」**（使用者定的字，逐字，不要潤飾）—— 兒牙、口外這類特別門診的醫師不是每週都到，但**不要在表上標哪幾顆是隔週、不要列日期或當月排班** | 第十節、`/history/clinic-schedule.html` |
