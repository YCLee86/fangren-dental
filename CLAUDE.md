...
4. **贋復假牙的藍要提亮到哪一階**（2026-08-09）。填色三案 × 白底的字三案，
   提案頁 `preview/spec-prosth-blue/`（網址可帶 `?f=f1|f2|f3&t=t1|t2|t3`），
   推導與數字在 [PALETTE.md](PALETTE.md) 第六之十節。
   **正式站與 `index.html` 一個字都還沒動**，等使用者從 3×3 裡挑一組。
（兒牙白字那一件已經結案了，見上面的定案表。口腔外科的紫已定案 `#8e6299`／`#784e84`，
見 PALETTE.md 第六之九節與 `preview/spec-surg-violet/`。）
