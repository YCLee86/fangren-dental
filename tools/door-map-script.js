...
(function () {
  ...
  /* 第五輪定案：大小取 Ⓑ 36。第六輪「再粗一點」→ 半寬從 36×0.2517（＝9.06，
     比例 1:2.0）加到 36×0.2889（＝10.4，**比例 1:1.73**）。
     ⚠ 這一顆是這條尺唯一的出處，改胖瘦只改 NS_R.w，不要去動 path 字串。 */
  var NS_R = { w: 0.2889, fs: 12 / 29, gap: 15 / 29 };
  ...
  function nsGeom() {
    ...
    NSW.w = h * NS_R.w; NSW.fs = h * NS_R.fs; NSW.gap = h * NS_R.gap;
    if (nswTxt) {
  ...
  ['orient', 'size', 'lotcolor', 'you', 'ns'].forEach(function (k) {
