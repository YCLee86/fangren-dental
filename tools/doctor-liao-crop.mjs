...
/* 尺：頭高佔圓的比例。70 ≈ 第一輪的現況，77／81 直接對齊站上那三位。
   ✅ **2026-09-23 使用者定案：74%**（「留74%」）。提案頁的那條尺已經收掉，
   四格仍然全部產出來 —— 提案頁的 ?h= 還吃得到，要回頭比另外三格用得上。
   ⚠ 74 比站上那三位鬆一點，**是他看過四格之後挑的，不是算出來的**。
   定案要搬進正式站時，抄的是 74 那一行的框（見底下跑完印出來的清單）。 */
export const RATIOS = [
  { k: 70, note: "第一輪那一格（最鬆）" },
  { k: 74, note: "★ 定案（2026-09-23）" },
  { k: 77, note: "和王俊偉一樣" },
