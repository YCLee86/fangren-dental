 ...
 *     不會讓首頁的計數多跳（DECISIONS.md 第九之〇節上線那天的兩件之一）。
 *
...
/* 往右：把框推出版心，貼齊螢幕邊。⚠ 上限兩段不同 ——
   ...
   兒牙那次給 32 就在 744 上多出 10px 水平捲動（DECISIONS.md 第九節）。 */
...
const nums = (v, d) => (v ? v.split(",").map(Number) : d);
/* ⚠ 兩把尺都可以在命令列換掉（2026-08-26 口外 iPad 那一輪加的）：
   很直的線稿在 ≥721 也是**高度**先卡住 —— 478×943 那一張，介紹區最矮的一格是
   443.1px，換算回寬度只有 224.6px，預設那把尺（200~400）有五格是壞的。
   **給出去的每一格都要是「選了就能上線」的**（CLAUDE.md 第八節 card-width-mobile）。 */
const PCTS = nums(flag("--sizes"), [44, 52, 60, 68, 76, 84, 92]);      /* 手機：真正在作用的是百分比 */
const PXS  = nums(flag("--wide-sizes"), [200, 240, 280, 310, 330, 360, 400]);  /* ≥721：介紹區夠寬，卡住的是 px 上限 */
