#!/usr/bin/env node
/* ==========================================================================
   preview/doctor-yang/ 的產生器
   --------------------------------------------------------------------------
   楊小瑩醫師的形象照：把 index.html 做成快照，在她那張卡上掛照片。
   起因：2026-09-23 使用者給了一張，「楊小瑩醫師圖卡合上去 預覽給我看」；
   第二輪（同日）換一張新照片：「換這張做預覽給我看」。
   照陳芷鈴那一支（git show ff8025d2:tools/doctor-chen-preview.mjs）改的。

   ⚠ 這一頁是**快照**，不要手改。要改就改這支再跑一次：
        node tools/doctor-yang-preview.mjs
   ⚠ 圖檔由 tools/doctor-yang-crop.mjs 產生，在 preview/doctor-yang/img/。
   ...
   ⚠ 定案之後要做的：把選中那一格的框搬進 tools/doctor-photo-crop.mjs 的 FACES、
     產出 assets/doctor-yang-xiaoying-400.jpg、在 index.html 那張卡補 data-face
     與 picture（照現有那五張抄），再跑 webp → topics → build；
...
const DIR  = join(ROOT, 'preview', 'doctor-yang');
const OUT  = join(DIR, 'index.html');
const KEYS = ['a', 'b', 'c', 'd'];

KEYS.forEach((k) => {
  if (!existsSync(join(DIR, 'img', 'yang-' + k + '-400.jpg')))
    throw new Error('找不到 img/yang-' + k + '-400.jpg —— 先跑 node tools/doctor-yang-crop.mjs');
});

let html = readFileSync(join(ROOT, 'index.html'), 'utf8');

const NOTE = `<!-- ==========================================================================
     提案：楊小瑩醫師的形象照・第二輪（2026-09-23）
     --------------------------------------------------------------------------
     第一張（框 845）當天已經上線，使用者看了之後：
       「髮型太圓了，合成網站上圓形感覺雙重圓」
     重新生成一張：「換這張做預覽給我看」。

     ---- 要挑的是什麼 -------------------------------------------------------
     **新照片的框大小**，以及和現況（第一張）比，是不是不再「雙重圓」。
     版面仍是 2026-09-19 定案的那一套，完全沿用。

       Ⓐ 框 870　頭頂留白 7.7%　頭高 67%
       Ⓑ 框 845　頭頂留白 6.5%　頭高 69%（預設，和第一輪挑的同一個框）
       Ⓒ 框 820　頭頂留白 5.4%　頭高 71%
       Ⓓ 框 795　頭頂留白 4.2%　頭高 74%
       現況　　　站上現在那張（第一張、框 845）

     ⚠ 上界是 870：再大框的上緣就超出原圖。

     ---- 裁切怎麼來的 -------------------------------------------------------
     原圖 1086x1448，量到 髮頂 70／眼睛 388／下巴 655／臉中線 520，
     照李柄輝的比例（眼睛在框高 44.2%、臉中線在框寬 49.3%）定位置。
       Ⓐ 91,3,870,870　Ⓑ 103,15,845,845　Ⓒ 116,26,820,820　Ⓓ 128,37,795,795

     ---- 切換條 -------------------------------------------------------------
     網址參數 ?p=now|a|b|c|d（now ＝ 站上現在那張；不帶 ＝ Ⓑ）
     ========================================================================== -->`;
...
/* ---- 楊小瑩那張卡：她的照片已經在站上（第一張），這裡把整個 picture 換掉 --
...
const H3  = '<h3>楊小瑩<span class="doc-role">贋復假牙專科醫師</span></h3>';
if (html.split(H3).length !== 2) throw new Error('找不到（或找到不只一個）楊小瑩的 h3');
const at  = html.indexOf(H3);
const art = html.lastIndexOf(TAG, at);
if (art < 0) throw new Error('楊小瑩的 h3 前面找不到 article');
const seg = html.slice(art, at);
const PIC = /<picture class="doc-face">[\s\S]*?<\/picture>/;
if (!PIC.test(seg) || !seg.includes('doctor-yang-xiaoying-400')) throw new Error('楊小瑩那張卡上找不到現況的 picture');
const ALT = '楊小瑩醫師的形象照，身著深藍診療服站在診所內';
html = html.slice(0, art) + seg.replace(PIC, '<picture class="doc-face">\n' +
  '            <img class="pv-face" src="img/yang-b-400.jpg" width="400" height="400" alt="' + ALT + '">\n' +
  '          </picture>') + html.slice(at);
/* 守門：站上七位有照片，這一輪不增不減。 */
const got = (html.match(/<article class="doc" data-face/g) || []).length;
if (got !== 7) throw new Error('data-face 有 ' + got + ' 張，預期 7 張');
...
const NOTE_MAP = JSON.stringify({ a: "新照片，裁切 91,3,870,870　頭頂留白 7.7%、頭高 67%", b: "新照片，裁切 103,15,845,845　頭頂留白 6.5%、頭高 69%", c: "新照片，裁切 116,26,820,820　頭頂留白 5.4%、頭高 71%", d: "新照片，裁切 128,37,795,795　頭頂留白 4.2%、頭高 74%" });
...
<div class="pvbar" data-open="0">
  <button class="pvbar-btn" type="button" aria-expanded="false">楊小瑩 <b></b></button>
  <div class="pvbar-panel">
    <div class="pvbar-body">
      <div class="pvbar-g">框的大小　<em>版面已定案，這裡只換裁切</em></div>
      <div class="pvbar-row" data-k="p">
        <button type="button" data-v="a">Ⓐ<small>870</small></button>
        <button type="button" data-v="b">Ⓑ<small>845</small></button>
        <button type="button" data-v="c">Ⓒ<small>820</small></button>
        <button type="button" data-v="d">Ⓓ<small>795</small></button>
      </div>
      <div class="pvbar-row">
...
(function () {
  ...
  var ALL  = ['a','b','c','d','now'];
  var LBL  = { a:'Ⓐ 870', b:'Ⓑ 845', c:'Ⓒ 820', d:'Ⓓ 795', now:'現況（第一張）' };
  var NOTE = NOTEMAP;
  var st = 'b';
  ...
  addEventListener('load', function () {
    ['a','b','c','d'].forEach(function (k) { new Image().src = 'img/yang-' + k + '-400.jpg'; });
  });

  function apply() {
    img.src = st === 'now' ? '../../assets/doctor-yang-xiaoying-400.jpg' : 'img/yang-' + st + '-400.jpg';
    ...
    history.replaceState(null, '', st === 'b' ? location.pathname : '?p=' + st);
  ...
  function measure() {
    ...
    rows += '<hr><div class="pvbar-note"><b>' + LBL[st] + '</b>：' + (NOTE[st] || '站上現在那張：第一張照片，裁切 88,72,845,845') +
...
console.log('寫好 preview/doctor-yang/index.html（' + (html.length / 1024).toFixed(0) + ' KB）');
