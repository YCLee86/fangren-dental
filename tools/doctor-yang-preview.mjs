...
const NOTE = `<!-- ==========================================================================
     提案：楊小瑩醫師的形象照（2026-09-23）
     --------------------------------------------------------------------------
     使用者給了一張，「楊小瑩醫師圖卡合上去 預覽給我看」。

     ---- 要挑的是什麼 -------------------------------------------------------
     **不是版面。** 圓頭像那一套 2026-09-19 已經定案上線，這一頁完全沿用。
     只有一張照片，所以要挑的是**框的大小**（頭在圓裡多滿）。
     起點照楊小瑩那一輪的結論：從頭頂留白 ~5% 給（Ⓒ 預設），兩側都有格子。

       Ⓐ 框 880　頭頂留白 8.2%　頭高 64%
       Ⓑ 框 845　頭頂留白 6.6%　頭高 67%（和楊小瑩定案的 6.7% 同一格）
       Ⓒ 框 810　頭頂留白 5.1%　頭高 69%（預設）
       Ⓓ 框 775　頭頂留白 3.4%　頭高 73%

     ⚠ 她是鮑伯頭、髮量厚，眼睛到髮頂比別人長，所以同樣的頭頂留白，
       頭高比例會比楊小瑩那幾格低。

     ---- 裁切怎麼來的 -------------------------------------------------------
     原圖 1024x1536，量到 髮頂 128／眼睛 445／下巴 690／臉中線 505，
     照李柄輝的比例（眼睛在框高 44.2%、臉中線在框寬 49.3%）定位置。
       Ⓐ 71,56,880,880　Ⓑ 88,72,845,845　Ⓒ 106,87,810,810　Ⓓ 123,102,775,775
...
/* ---- 楊小瑩那張卡：掛 data-face、插一張 picture -----------------------
...
html = html.replace(H3, '<picture class="doc-face">\n' +
  '            <img class="pv-face" src="img/yang-c-400.jpg" width="400" height="400" alt="' + ALT + '">\n' +
  '          </picture>\n          ' + H3);
/* 守門：站上原本五張 ＋ 楊小瑩這張 ＝ 6。數字不對就是掛錯人或掛了兩次。 */
const got = (html.match(/<article class="doc" data-face/g) || []).length;
if (got !== 6) throw new Error('data-face 有 ' + got + ' 張，預期 6 張（站上五位＋楊小瑩）');
...
const NOTE_MAP = JSON.stringify({ a: "裁切 71,56,880,880　頭頂留白 8.2%、頭高 64%", b: "裁切 88,72,845,845　頭頂留白 6.6%、頭高 67%", c: "裁切 106,87,810,810　頭頂留白 5.1%、頭高 69%", d: "裁切 123,102,775,775　頭頂留白 3.4%、頭高 73%" });
  ...
  <div class="pvbar-panel">
    <div class="pvbar-body">
      ...
      <div class="pvbar-row" data-k="p">
        <button type="button" data-v="a">Ⓐ<small>880</small></button>
        <button type="button" data-v="b">Ⓑ<small>845</small></button>
        <button type="button" data-v="c">Ⓒ<small>810</small></button>
        <button type="button" data-v="d">Ⓓ<small>775</small></button>
      </div>
      <div class="pvbar-row">
...
(function () {
  ...
  var ALL  = ['a','b','c','d','off'];
  var LBL  = { a:'Ⓐ 880', b:'Ⓑ 845', c:'Ⓒ 810', d:'Ⓓ 775', off:'現況（沒有照片）' };
  var NOTE = NOTEMAP;
  var st = 'c';
  ...
  function apply() {
    ...
    else { card.setAttribute('data-face', ''); pic.style.display = ''; img.src = 'img/yang-' + st + '-400.jpg'; }
  ...
  function measure() {
    ...
    rows += '<hr><div class="pvbar-note"><b>' + LBL[st] + '</b>：' + NOTE[st] +
      ...
      '和站上那五顆並排比比看。要對照沒有照片的樣子按最底下那一顆。</div>';
    foot.innerHTML = rows;
  }
