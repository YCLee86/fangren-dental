...
/* ⚠ 帶子上緣是**淡進來**的（前 42% 用 smoothstep 把 alpha 從 0 長到 1），
   不是一條硬邊 —— 2026-08-22 使用者：「帶子好黑，和圖片銜接的地方做漸層吧」。
   站上那條窄帶不必淡入，是因為它上面本來就是暗的路面；這張圖的帶子正上方是
   **亮著的騎樓**，同一個做法在這裡會切出一條線。
   淡入之後三格底下的白字對比：中位 19.45・最亮處 15.21（綽綽有餘）。 */
const STATSCALE = {
