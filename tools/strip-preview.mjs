...
/* 橫幅的捲軸。⚠⚠ 滑得動的那幾科會多一條水平捲軸、滑不動的沒有 ——
   在 Windows 那種**傳統捲軸**上，那一條是真的佔高度的，所以顯微根管、齒顎矯正
   這種看起來就比牙周、兒童矮一截（2026-09-26 使用者回報）。
   ⚠ 這個容器（headless Linux）用的是**覆蓋式捲軸**，浮在內容上、不佔空間，
     所以在這裡量 offsetHeight − clientHeight 三種模式都是 0，**驗不出來**。
     不要因為量不到就當作沒事。
   隱藏 ＝ 站上那排科別標記的做法（它也是橫捲，也是把捲軸藏起來、靠邊緣淡出提示）。
   留位 ＝ 捲軸照顯示，但沒有捲軸的時候也把那條高度留著，八科等高。 */
html[data-pvsb="hide"] .pv-rail.cards { scrollbar-width: none; }
...
<div class="pv-bar" id="pv-bar">
  ...
  <div class="pv-fine" id="pv-fine" hidden>
    ...
    <div class="pv-row"><span class="pv-lab">標籤移動</span><span class="pv-seg" data-k="ts">
      <button data-v="t25">0.25 秒</button><button data-v="t50">0.5 秒</button><button data-v="t60">0.6 秒</button><button data-v="t70">0.7 秒</button><button data-v="t80">0.8 秒</button>
    </span></div>
    ...
    <div class="pv-row"><span class="pv-lab">過場長度</span><span class="pv-seg" data-k="l">
      <button data-v="same">同標籤 0.6</button><button data-v="l30">0.3 秒</button><button data-v="l20">0.2 秒</button><button data-v="l15">0.15 秒</button><button data-v="l10">0.1 秒</button><button data-v="l0">直接換</button>
    </span></div>
...
(function () {
  ...
  /* 換科那一下，圖卡怎麼交棒。標籤的顏色一律在第 0 毫秒就開始變、T 毫秒後全亮
     （T ＝ 標籤換色那條尺），四種做法都讓圖卡在同一個 T 到位：
       a 同時淡入   ——「舊的直接收、新的從 0 淡上來」。中間那一瞬間最空。
       b 先出後進   —— 舊的先淡掉一半的時間，再換新的淡上來。
                      ⚠ 空的那一刻只是從第 0 毫秒搬到中點，而且是真的兩邊都 0。
       c 0.4 秒才換 —— 舊的整整撐到 400ms 都還在，然後用剩下的時間對拉換掉。
       x 疊著對拉   —— 舊的疊在新的上面淡出，新的同時淡入，**沒有任何一刻是空的**。
     ⚠ 這幾種的「顯示中的那一組」和 idx 會有一小段不同步（b 與 c 先改 idx、
  ...
  /* ⚠ 0.1 秒以下量不太出差別：一幀就是 16.7ms，0.1 秒只有六幀，
     再短就是「剩幾幀」的問題，不是曲線的問題。所以最後一格直接給「0 ＝ 直接換」，
     不要再切 0.08／0.05 這種分不出來的格子。
     ⚠ 直接換不會有空檔：舊的消失與新的出現在同一幀，不是淡入淡出。 */
  var LEN = { l30: 300, l20: 200, l15: 150, l10: 100, l0: 0 };
  ...
  var TS = { t25: 250, t50: 500, t60: 600, t70: 700, t80: 800 };   /* 標籤那一列滑多久 */
  ...
  function show(i) {
    ...
    var old = groups[prev];
    if (st.f === 'x') {
      D.style.setProperty('--pv-fxout', T + 'ms');
      applyGroups(T);
      old.classList.add('pv-out');
      fxTimer = setTimeout(function () { old.classList.remove('pv-out'); fxTimer = null; }, T + 60);
    } else if (st.f === 'b') {
      D.style.setProperty('--pv-fxout', Math.round(T / 2) + 'ms');
      old.classList.add('pv-dim');
      fxTimer = setTimeout(function () { applyGroups(T / 2); fxTimer = null; }, T / 2);
    } else if (st.f === 'c') {
      /* 撐到 400ms 都還是舊的，然後用剩下的時間**對拉**換掉。
         ⚠ 第一版是「時間到就換、新的從 0 淡上來」，量出來 470ms 那一刻
         畫面總不透明度是 0 —— 空檔只是從開頭搬到 400ms 之後，沒有解決。 */
      var hold = Math.min(400, Math.max(0, T - 120));
      fxTimer = setTimeout(function () {
        var rest = T - hold;
        D.style.setProperty('--pv-fxout', rest + 'ms');
        applyGroups(rest);
        old.classList.add('pv-out');
        fxTimer = setTimeout(function () { old.classList.remove('pv-out'); fxTimer = null; }, rest + 60);
      }, hold);
    } else {
      applyGroups(T);
    }
  ...
  function paint() {
    ...
    D.setAttribute('data-pvsb', st.sb);
  ...
  function measure(why) {
    ...
    panel.innerHTML =
      ...
      + '・過場 ' + (fxdur() ? fxdur() + 'ms' : '直接換')
      + (fxdur() === tdur() ? '' : '（早 ' + (tdur() - fxdur()) + '）')
;
  }
