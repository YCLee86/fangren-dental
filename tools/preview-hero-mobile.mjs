...
<!-- ==========================================================================
     ...
     **抵下緣 ＋ 街景多 ＋ 詩鬆兩階**（這一頁打開就是它）。
     ...
     2. data-h／data-crop／data-poem／data-cue 與整條切換條刪掉，
...
@media (max-width: 720px) {
  ...
  /* ---- 尺③　詩 -----------------------------------------------------------
     現況那一組（Ⓢ1，2026-08-10）是被 291px 的照片逼出來的：字距只剩 .131em
     （電腦版 .32em）、全形空格收到 .65em、行高 1.75（電腦版 2.45）。
     照片拉高之後天空的楔形整個放大，這些額度可以還回去。 */
  /* ---- --ph ＝ 照片實際高度 ------------------------------------------------
  ...
  html[data-poem="p2"] { --hero-fs: clamp(min(11.5px, 3.20vw), calc(var(--ph) * .0181), 13px); }
  html[data-poem="p2"] .hero-copy { padding-top: calc(var(--ph) * .108); }
  html[data-poem="p2"] .hero-poem { letter-spacing: .19em; line-height: 2.00; }
  html[data-poem="p2"] .hero-poem .g { font-size: .80em; }
  html[data-poem="p2"] .hero-poem .lead { margin-bottom: .70em; }

  html[data-poem="p3"] { --hero-fs: clamp(min(11.5px, 3.60vw), calc(var(--ph) * .0201), 14.5px); }
  html[data-poem="p3"] .hero-copy { padding-top: calc(var(--ph) * .129); }
  html[data-poem="p3"] .hero-poem { letter-spacing: .25em; line-height: 2.20; }
  html[data-poem="p3"] .hero-poem .g { font-size: .92em; }
  html[data-poem="p3"] .hero-poem .lead { margin-bottom: .82em; }
...
<div id="sw">
  ...
  <div class="r" data-k="poem"><b>詩</b>
    <button type="button" data-v="p1">現況</button><button type="button" data-v="p2">鬆一階</button><button type="button" data-v="p3">鬆兩階</button></div>
  ...
  <div id="swn" hidden>
    <p><b>結論：建議「抵下緣 ＋ 街景多 ＋ 詩鬆兩階」</b>（這一頁打開就是它）。照片從一屏的 34% 變成 85%，照片＋窄帶剛好一屏 ——
...
(function () {
  ...
  var q = location.search, def = { h: 'fit', crop: 'street', poem: 'p3', cue: '0' };
  ...
  function paint() {
    ...
    document.getElementById('swm').textContent =
      ...
      + '　裁切 ' + p.toFixed(1) + '%（看得到原圖寬的 ' + (vis / SW * 100).toFixed(0) + '%）'
      + '　詩 ' + getComputedStyle(document.querySelector('.hero-poem')).fontSize;
  }
