#!/usr/bin/env node
/* ==========================================================================
   preview/doctor-xie/ 的產生器
   --------------------------------------------------------------------------
   謝耀慶醫師的形象照：把 index.html 做成快照，在他那張卡上掛照片。
   起因：2026-09-23 使用者給了一張，「合上謝耀慶醫師的圖卡　做預覽頁給我看」。
   照 tools/doctor-liao-preview.mjs 那一支改的（同一天、同一套做法）。

   ⚠ 這一頁是**快照**，不要手改。要改就改這支再跑一次：
        node tools/doctor-xie-preview.mjs
   ⚠ 圖檔由 tools/doctor-xie-crop.mjs 產生，在 preview/doctor-xie/img/。
...
KEYS.forEach((k) => {
  if (!existsSync(join(DIR, 'img', 'xie-' + k + '-400.jpg')))
    throw new Error('找不到 img/xie-' + k + '-400.jpg —— 先跑 node tools/doctor-xie-crop.mjs');
});

let html = readFileSync(join(ROOT, 'index.html'), 'utf8');

const NOTE = `<!-- ==========================================================================
     提案：謝耀慶醫師的形象照（2026-09-23）
     --------------------------------------------------------------------------
     使用者給了一張，「合上謝耀慶醫師的圖卡　做預覽頁給我看」。
...
/* ---- 謝耀慶那張卡：掛 data-face、插一張 picture -----------------------
   ⚠⚠ 錨點用**完整的 h3**（名字＋藥丸文字，全站唯一），再往回找最近的 article。
       **不可以用 data-spec** —— 兒童牙科有兩位（林晏妤、謝耀慶），
       掛錯的話畫面完全正常，只是圖長在別人身上（2026-09-19 踩過同一族）。 */
const TAG = '<article class="doc"';
const H3  = '<h3>謝耀慶<span class="doc-role">兒童牙科</span></h3>';
if (html.split(H3).length !== 2) throw new Error('找不到（或找到不只一個）謝耀慶的 h3');
const at  = html.indexOf(H3);
const art = html.lastIndexOf(TAG, at);
if (art < 0) throw new Error('謝耀慶的 h3 前面找不到 article');
html = html.slice(0, art) + TAG + ' data-face' + html.slice(art + TAG.length);
const ALT = '謝耀慶醫師的形象照，身著白袍站在診所診療區前';
html = html.replace(H3, '<picture class="doc-face">\n' +
  '            <img class="pv-face" src="img/xie-b-400.jpg" width="400" height="400" alt="' + ALT + '">\n' +
  '          </picture>\n          ' + H3);
/* 守門：站上原本三張 ＋ 謝耀慶這張 ＝ 4。數字不對就是掛錯人或掛了兩次。 */
...
<div class="pvbar" data-open="0">
  <button class="pvbar-btn" type="button" aria-expanded="false">謝耀慶 <b></b></button>
...
(function () {
  ...
  function apply() {
    ...
    else { card.setAttribute('data-face', ''); pic.style.display = ''; img.src = 'img/xie-' + st + '-400.jpg'; }
...
console.log('寫好 preview/doctor-xie/index.html（' + (html.length / 1024).toFixed(0) + ' KB）');
