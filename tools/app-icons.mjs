...
/* =============================================================================
   ...
   3. **牙洞的放大倍率不一樣，理由也不一樣。**
      favicon 放大 **1.6 倍**，是為了 16px 下那個洞不被抗鋸齒吃掉。
      這裡放大 **1.18 倍 ＋ 上移 12**（2026-08-12 第十一輪定案），是為了扛住
      **iOS 產生捷徑圖示時那 4~5px 的柔化** —— 原始比例的洞在手機上只有 11.1px 寬，
      兩側各被吃掉一截就糊成一個淺色斑點；×1.18 之後是 13.1px，撐得住。
      ⚠ 兩邊的倍率**不必一致，也不要為了「統一」去改** —— 它們在對抗不同的東西。
...
const TARGETS = [
  ...
  /* Android 的自適應圖示。第八輪曾經為了排除嫌疑把它從 manifest 拆掉，
     第九輪的對照頁證實**和模糊無關**（拆光所有宣告的對照頁量出來一樣是 4px），
     所以已經加回去了。⚠ iOS 不會拿 purpose: maskable 當 apple-touch-icon。 */
  { file: "icon-maskable-512.png", size: 512, maskable: true },

  /* ⚠ 這裡曾經有兩筆實驗版（2026-08-12 第九、十輪，都已刪除）：
       alpha: true  透明底（iOS 直接填黑）＋ 米色標誌
       swap:  true  整片套色底 ＋ 白色標誌（比照 innovarad.tw）
     兩者都在使用者的實機上測過，結論見 /history/icon-homescreen-blur.html。
     **整片套色那一版有一個結構性的問題**：iOS 會把背景色以約 16% 均勻混進標誌，
     白標誌因此只剩 rgb(220,227,222)——而白已經是 255，**沒有補償空間**。
     反過來白底那一版的扭曲（標誌被拉亮）是可以反推補回去的，實測 ΔE 9.2 → 0.0。
     這就是最後選白底的理由，不是因為它比較銳利（它不是）。
     ⚠ 下面的 alpha / fill / swap 支援程式碼留著沒刪，日後要再做實驗
       加一筆 TARGET 就會動。 */
];
