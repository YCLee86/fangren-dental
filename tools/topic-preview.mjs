...
/* 文案在 tools/topic-copy.mjs（獨立一檔，之後只要改那一份）。
   LEGACY 是 2026-08-18 上午那一版長文，只給提案頁的「對照舊版」按鈕用。 */
import { TOPICS, LEGACY } from "./topic-copy.mjs";
...
/* ---------- 產生「科別介紹」那一塊 ----------
   形狀（2026-08-18 使用者「字太多，篇幅太長」之後改的）：
     開場兩三句 → 三到五條「這些狀況該來看一下」→ 四五組一問一答。
   目標 350~450 字、手機上一屏之內。舊版是 825 字、2.1 屏。
...
${t.qa.map(([q, a]) => `          <dt>${q}</dt>\n          <dd>${todo(a)}</dd>`).join("\n\n")}
      ...
      <!-- 對照用的舊版（825 字那一版）。**定案時連同切換條一起刪掉。** -->
      <div class="tp-legacy" hidden>
${LEGACY[spec].map(([h, ps]) =>
  `        <div class="tp-sec">` + (h ? `\n          <h2>${h}</h2>` : "") +
  `\n${ps.map((x) => `          <p>${x}</p>`).join("\n")}\n        </div>`).join("\n")}
      </div>
`;
