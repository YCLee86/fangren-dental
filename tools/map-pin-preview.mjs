...
<div class="pv-bar" id="pvBar">
  ...
  <div class="pv-fold" id="pvFold" hidden>
    ...
    <div class="pv-row">
      <span class="pv-lab">日高</span>
      <button data-k="sun" data-v="a">很高 75°</button>
...
(function () {
  ...
  /* ⚠⚠⚠ 2026-08-21 第八輪，使用者一句話點破前面七輪都在做錯的事：
       「你把整張地圖當成貼在牆壁上、光源照下去，但我想要的是**地圖平貼在
        桌上**、光源照下去的感覺。」
     前面那幾版是把整條釘身的剪影**整個平移**再疊淡 —— 那是「貼在牆上的貼紙」
     的影子（物體和牆平行，影子就是自己往旁邊挪一點）。
     這張圖是**俯視圖**，圖釘是**站在桌面上**的：
       ・尖端碰到桌面 → 那一點的影子就在原地，影子從那裡長出來
       ・頭離桌面最遠 → 它的影子被推得最遠
     所以影子是把釘身**投影**到桌面上：離尖端愈高的部分，影子往光的反方向
     推得愈遠。用一個矩陣就寫得完（以尖端為原點）：
         x' = x − L·cos(方位) · y
         y' =   − L·sin(方位) · y      （y 是往上為負，所以要取負號）
       L ＝ cot(太陽高度) —— 太陽愈高 L 愈小、影子愈短。
     ⚠ 圓形的頭被投影之後會變成**橢圓**，而且是斜的 —— 那正是站著的東西
       從正上方看下去該有的樣子，不是畫錯。
  ...
  function drawShadow(dAttr, d, R) {
    ...
    for (var i = 0; i < LAYERS; i++) {
      var t = i / (LAYERS - 1);
      var sc = 1 + sh.sp * t;               /* 以尖端為原點放大 ＝ 愈遠愈糊 */
      var p = document.createElementNS(NS, 'path');
      p.setAttribute('d', dAttr);
      p.setAttribute('opacity', sh.a.toFixed(3));
      /* ⚠ 順序是「先投影、再放大」：SVG 的 transform 列表是右邊先作用。 */
      p.setAttribute('transform', 'scale(' + sc.toFixed(4) + ') matrix(1 0 ' +
        mc.toFixed(4) + ' ' + md.toFixed(4) + ' 0 0)');
      shg.appendChild(p);
    }
  }
