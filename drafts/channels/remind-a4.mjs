 ...
 *
 *   可帶：--date=auto（印「兩天後」那個示範值）／--date="…"　--pad=12（紙的上下留白 mm）
...
/* ---- 日期那一格 --------------------------------------------------------
   ⚠⚠ 2026-09-16 定案：**印成 `0000/00/00` 的佔位符**（使用者：「日期顯示是
     0000/00/00」）—— 同姓名那一格的理由（檔頭 ⑤）：那一行是系統填的，
     紙上印一個**具體的日期**，讀的人會去對自己那一筆、而它永遠不是他那一筆。
     佔位符反而把「這一行會填上你的看診時間」講清楚了。
   ⚠ 它順帶讓「提醒2天後有約要看牙齒喔」不必再和紙上的日期對得起來 ——
     底下 `sampleDate()` 那條「取兩天後、跳過六日」因此不再是預設，
     **但留著**：`--date=auto` 走得回去（`--date=…` 也吃任何字串）。          */
...
let DATE = arg("date", PLACEHOLDER_DATE);
if (DATE === "auto") DATE = sampleDate();
const NAME = arg("name", "");
