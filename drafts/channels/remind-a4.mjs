 ...
 *
 * ⚠⚠ 換媒介要逐項問的（DECISIONS.md 第九節第 28 條 ⑤）：
 *   ① **紙上的按鈕按不下去** —— 三顆都還在，因為這張紙的用途正是「你手機上會看到這個」；
 *      按鈕是那則訊息的一部分，拿掉反而看不懂。
 *   ② **姓名與日期是系統填的** —— 日期紙上印示範值，**寫法照使用者指定的
 *      `0000/00/00 (五) 09:00`**（同廠商那一版印著 2025/01/11 那種帶年份的斜線寫法），
...
/* ---- 日期那一格 --------------------------------------------------------
   ⚠⚠ 2026-09-16 定案：**日期那一段照 `0000/00/00` 的寫法印**（使用者：「日期顯示是
     0000/00/00」，隔一句更正「我是給你日期格式　不是真的 00　只有日期要變而已
     其他星期　時間都不用改」）—— 所以那幾個 0 是**位數不是字面**：印的仍然是一個
     真的示範日期，只有**日期那一段**從「9月18日」換成「2026/09/18」，
     **後面的星期與時間一個字都沒動**。
   ⚠⚠⚠ 中間那一版（整串印成字面的 `0000/00/00`、連星期與時間一起吃掉）**已經作廢**，
     不要拿它「訂正」現行的寫法。
