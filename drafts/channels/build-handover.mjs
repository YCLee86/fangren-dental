...
const ITEMS = [
  ...
  { n: "③", title: "綁定完成", askLabel: "廠商回覆：", ask: "綁定完成訊息為公版，故無法調整",
    ...
    note: "<b>那句公版我們不用改，也不必取代它。</b>「✅ 09XXXXXX 綁定成功！」是病人端送出的訊息；"
      ...
      + "或是<b>在公版那句後面再多送一則</b>。細節與三個要確認的問題見下面「還需要答案的事」第 3 項。",
  ...
  { n: "⑤", title: "預約成功 ＋ 約診紀錄查詢", askLabel: "廠商回覆：",
...
const sections = ITEMS.map((it) => {
  ...
  <h2 class="pv-h2" id="m${it.n}">${it.n} ${esc(it.title)}
    <span class="t">${it.askLabel ?? "廠商要的："}${esc(it.ask)}</span></h2>
