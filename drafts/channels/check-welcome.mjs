...
/* ⚠⚠ 掃之前要先把**否定形**拿掉（2026-09-03，做綁定完成那一則時發現）：
   「這裡**沒有專人**即時回覆訊息」這句話裡面就含著「有專人」與「即時回」——
   照字面掃會把**守得最好的那一句**報成說謊。招呼卡沒寫到這句話，所以這個洞一直沒炸。 */
const strip = (s) => s.replace(/沒有專人/g, "＿").replace(/無專人/g, "＿")
                      .replace(/沒有(人|專人)?即時回覆/g, "＿");
for (const t of texts) for (const w of LIE)
  if (strip(say(t)).includes(w)) bad.push(`「${say(t)}」出現「${w}」—— 這個帳號沒有專人即時回覆，那是說謊`);
