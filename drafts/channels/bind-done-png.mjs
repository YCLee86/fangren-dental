 ...
 *     → preview/line-bind-done/shot-bind-done.png    ✅ 2026-09-04 定稿的那一張
 *
...
const out = path.join(DIR, "shot-bind-done.png");
   ...
   要檢查「形狀對不對」，不是只檢查檔案有沒有產生）。 */
{
  const buf = fs.readFileSync(out);
  const w = buf.readUInt32BE(16), h = buf.readUInt32BE(20);
  if (Math.abs(w - 268 * SCALE) > SCALE * 3)
    throw new Error(`出圖 ${w}px 寬，該是 ${268 * SCALE} 左右`);
  if (h < w) throw new Error(`出圖 ${w}×${h} —— 高比寬還小，多半只拍到一截`);
  console.log(`preview/line-bind-done/shot-bind-done.png　${w}×${h}` +
    `（${SCALE}×，卡片實寬 ${w / SCALE}px）　${(buf.length / 1024).toFixed(0)}KB`);
}

await browser.close();
console.log("\n線上網址（等 Cloudflare 建置跑完才打得開，先看 /version.txt）：");
console.log("  https://fangren.net/preview/line-bind-done/shot-bind-done.png");
