...
for (const im of imgs) {
  ...
  /* ⭐ 定稿那一張一律 1080 見方；路線圖那幾格也是；模擬圖（三格、410px）不在此限 */
  if (im.src === "fangren-map-1080.png" || /^far-(?!profile|slot)/.test(im.src))
...
for (const [i, want, nm] of [[0, doorDetail, "door-detail.txt"], [1, farDetail, "far-detail.txt"]])
...
for (const [d, nm] of [[doorDetail, "door-detail.txt"], [farDetail, "far-detail.txt"]]) {
...
const askLine0 = (/還缺一個：雲林縣政府/.test(PAGE) ? "雲林縣政府" : undefined);
const askLine = askLine0;
ok(askLine !== undefined, "第三部分找不到「還缺一個：雲林縣政府」那一段");
if (askLine !== undefined) ok(askLine === askNames.join("、"),
  `頁上列的「還沒有位置」和 far-map.json 對不上：\n    頁面 ${askLine}\n    檔案 ${askNames.join("、")}`);
ok(askNames.length === 1, `far-map.json 裡沒有座標的有 ${askNames.length} 個，`
  + "而頁上那一段寫的是「雲林縣政府」一個 —— 補了座標就要一起改那一段");
...
console.log(`✓ 靜態檢查通過（圖 ${imgs.length} 張、兩塊詳情逐字相同、`
  + `留下的兩句與拿掉的兩句都對得上門口那張、三條網址對得上 index.html、`
  + `待補的 ${askNames.length} 個地標頁面與資料對得上）`);
