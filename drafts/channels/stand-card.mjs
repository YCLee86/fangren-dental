...
export const ID上 = 0.005, ID下 = 0.0049, ID字級 = 0.038;
/* ⚠⚠⚠ 2026-09-17 使用者：「@fafa070 和 QRcode 的間距比上面多一點　可以再縮看看」——
 *   量出來他看到的是**一塊看不見的東西**：那顆碼四周有四格靜區（規範，收了會掃不出來），
 *   而靜區是透明的 —— 所以 `<img>` 的上緣到**看得到的碼**之間，永遠隔著
 *   `靜區佔框 × 框寬` ＝ 目前 4.75 mm 的空白。margin 寫 0.2 mm，眼睛看到的是 5.9 mm。
 * ⚠⚠ 所以 `ID下` 改成**看得到的那個間距**（ink → 碼），CSS 的 margin 自己把靜區扣掉
 *   （會是負的，那是對的）；`疊高()` 吃同一個算式，兩邊不可以分家。
 * ⚠⚠⚠ **扣掉靜區省下來的 4.75 mm 正好讓 QR 走到貼齊**（框 53.4 mm ＝ 碼 43.9
 *   ＝「輕鬆接收診所新訊」那一行），而插圖那一條不但沒有被壓縮、還長了 0.9 mm。
 * ⚠ 碼的**下緣**也有同樣一塊 4.75 mm 的空白（碼與插圖之間）——**刻意沒有動**：
 *   那裡本來就需要一點喘息，而且他沒有提。 */
...
const QRVB = +(readFileSync(QRSRC, "utf8").match(/viewBox="0 0 (\d+(?:\.\d+)?) /) ?? [])[1];
if (!QRVB) throw new Error("讀不到那顆 QR 的 viewBox");
/* 四格靜區佔整張圖多少 —— 兩邊各一塊，所以「看不見的上緣空白」＝ 它 × 框寬 */
export const 靜區佔框 = 4 / QRVB;
export const 碼上空 = () => 靜區佔框 * CARD.QR佔卡寬;
...
export const 疊高 = (c, M = BDGAP, g = PGAP) => {
  ...
  return 上內距 + 1.5 * hd.fs + 分上 + 帶高() + 分下 + bd
    + (c.QR上 ? ID上 + 1.5 * ID字級 + ID下 - 碼上空() : 0.04) + CARD.QR佔卡寬
...
.card .cue.id + .qr{margin-top:calc(var(--cw) * ${(ID下 - 碼上空()).toFixed(5)})}
