...
/* ⑰ 4-6：兩條實作路徑、四條查證各自帶出處、還沒查到的兩件都要在 */
{
  ...
  /* ⚠⚠ 2026-09-18 使用者指定副標改成「以下為AI 提取的LINE官方文件」——
     那是**出處的宣告**，逐字照他寫的；副標、導言與最後那一句是同一件事的三個
     說法，不可以只改其中一個（只改副標的話，頁面會一邊說 AI 提取、一邊說
     診所查證過，而每一道版面守門都會過）。 */
  if (!S6.副標 || !html.includes(esc(S6.副標))) bad.push("⑰ 4-6 的副標沒有逐字印出來");
  const 舊說法 = ["診所這邊對照", "查證過"].filter((w) => s46.includes(w));
  if (舊說法.length) bad.push(`⑰ 4-6 上還留著舊的出處說法：${舊說法.join("、")}`);
