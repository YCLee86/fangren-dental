   ...
   ⚠ 每一件改回去畫面都還是很正常，所以每一件都要有一道。 */
{
  ...
  for (const 案 of B.順序案) {
    ...
    /* ⚠⚠⚠ 2026-09-18：**兩種用途的「重複」規則不一樣**，所以要先問這一份是誰的。
       ・立牌那一條是分隔線、只「多放一兩顆」→ 重複哪幾顆是設計項（白名單 `重複`）。
       ・約診卡那幾條是「九顆一循環接不完的長度」→ 重複是結構性的，白名單套不上去
         （13 要多 4 顆、16 要多 7 顆，硬守白名單那兩顆會各出現三到四次 ＝ 一個花紋），
         改成兩條算得出來的：每一顆最多 ⌈n ÷ 9⌉ 次、次數差 ≤1，而且多的是**最窄的那幾顆**。 */
    ok(["立牌", "約診卡"].includes(案.用途), `${案.標籤} 沒有寫 用途（立牌／約診卡）—— 重複那一條要看它`);
    const 形狀數 = Object.keys(WM).length;
    const 次 = new Map();
    for (const k of 案.序) 次.set(k, (次.get(k) || 0) + 1);
    ok(案.顆 < 形狀數 || 次.size === 形狀數,
      `${案.標籤} 只用到 ${次.size} 個形狀 —— 九顆要每一顆都出現過`);
    if (案.用途 === "立牌") {
      const 多 = 案.序.filter((k, i) => 案.序.indexOf(k) !== i);
      for (const k of 多) ok(B.重複.includes(k), `${案.標籤} 重複了 ${k}，而它不在「重複」那張清單裡`);
      /* ⚠⚠⚠ 改動前這一行寫的是 `new Set(序).size === 序.length - 多.length` ——
         那**恆等於成立**（每一顆重複 c 次就往 `多` 丟 c−1 個，兩邊必然相等），
         所以「有一顆重複了三次」從來沒有人在守。真正守得住的是**算得出來的上限**：
         九個形狀、白名單裡 w 顆可以重複 → 排 n 顆時最少也要有一顆出現
         `1 + ⌈(n − 9) ÷ w⌉` 次（9→1、10／11→2）。超過它 ＝ 有一顆被多用了。 */
      const 上限 = 案.顆 <= 形狀數 ? 1 : 1 + Math.ceil((案.顆 - 形狀數) / B.重複.length);
      for (const [k, c] of 次)
        ok(c <= 上限, `${案.標籤} 的 ${k} 出現 ${c} 次，而 ${案.顆} 顆最多只需要 ${上限} 次`);
    } else {
      const 上限 = Math.ceil(案.顆 / 形狀數);
      for (const [k, c] of 次)
        ok(c <= 上限, `${案.標籤} 的 ${k} 出現 ${c} 次，${案.顆} 顆平均下來最多 ${上限} 次`);
      ok(Math.max(...次.values()) - Math.min(...次.values()) <= 1,
        `${案.標籤} 每一顆出現的次數差超過一次 —— 多出來的那幾顆要平均`);
      /* 多出來的那幾顆 ＝ 最窄的那幾顆（寬度出處 wm-sizes.json，同寬再照名字排） */
      const 窄 = Object.keys(WM).sort((a, b) => WM[a].w - WM[b].w || (a < b ? -1 : 1)).slice(0, 案.顆 - 形狀數);
      const 多 = [...次].filter(([, c]) => c > 1).map(([k]) => k).sort();
      ok(多.join("／") === [...窄].sort().join("／"),
        `${案.標籤} 多出來的是 ${多.join("／") || "—"}，該是最窄的那 ${案.顆 - 形狀數} 顆（${窄.join("／")}）`);
    }
  ...
  /* ⚠⚠⚠ 2026-09-16 再一輪（使用者：「保留 M1 M2　再做兩版介於兩個之間」）——
     那把尺從「倍數」變成「顆數」，**每一格自己帶著基數**（11×3／10×3／9×3／11×2）。
     ⚠⚠ 基數換了「哪幾顆重複」就跟著換（那是他 2026-09-16 挑定的一格），所以
       每一格都要把重複那一串印在頁上；這裡只驗「有沒有印」，選哪一格是他的決定。 */
  ok(B.顆案.length >= 2, "顆數那把尺只剩一格 —— 那就不是尺了");
  ok(B.顆案.some((k) => k.基 === B.顆數 && k.倍 === B.倍), `顆案裡沒有定案那一格（${B.顆數} × ${B.倍}）`);
  /* ⚠⚠ 兩格算出同一個顆數 ＝ 那把尺上有兩格長得一模一樣（基數 10×3 打成 11×3 就會這樣）——
     畫面完全正常，只是尺少了一格，而且會有兩格同時標「現在這樣」。 */
  {
    const 顆 = B.顆案.map((k) => k.基 * k.倍);
    ok(new Set(顆).size === 顆.length,
      `顆案裡有兩格算出同一個顆數（${顆.join("／")}）—— 那把尺少了一格`);
  }
  for (const k of B.顆案) {
    ok(Number.isInteger(k.倍) && k.倍 >= 1, `顆案裡的 ${k.標籤} 不是正整數倍 —— 接縫沒有人驗過`);
    ok(B.順序案.some((x) => x.顆 === k.基), `顆案裡的 ${k.標籤} 用的基數 ${k.基} 在順序案裡沒有`);
    ok(!k.標籤.includes("定案"), `${k.標籤} 的標籤裡自己寫著「定案」`);
    ok(!/\d+\s*顆/.test(k.標籤), `${k.標籤} 的標籤裡寫死了顆數 —— 那個數字要現算（基數一改它就會說謊）`);
    const b = 帶(k.基 * k.倍, undefined, k.基);
    ok(b.顆數 === k.基 * k.倍, `${k.標籤} 排出來不是 ${k.基 * k.倍} 顆`);
    ok(b.基 === k.基, `${k.標籤} 排出來用的是基數 ${b.基}，不是宣告的 ${k.基}`);
  ...
  /* ⚠⚠⚠ 2026-09-16 定案 Ⓜb（基數 9）之後，**「重複」那一欄的意思換了**：它現在是
     「允許重複的那幾顆」＝ 一張白名單，不是「定案那一份裡真的重複的是哪幾顆」——
     因為 9 顆那一份**一顆都不重複**，而 10／11 顆那兩份還在資料裡、上面那一圈仍然
     拿這張白名單在驗它們有沒有偷放別顆。所以這一道從「逐字相等」改成兩件：
     ・定案那一份裡重複的那幾顆，每一顆都要在白名單裡（漏掉 ＝ 上面那一圈會放行一顆沒人挑過的）
     ・白名單裡不可以有「三份順序都沒有重複過」的顆（多放一顆 ＝ 那一圈就少守一件） */
  for (const k of 重複of(B.顆數))
    ok(B.重複.includes(k), `定案那一份（${B.顆數} 顆）重複了 ${k}，而它不在「重複」那張白名單裡`);
  {
    const 用過 = new Set(B.順序案.flatMap((x) => 重複of(x.顆)));
    for (const k of B.重複)
      ok(用過.has(k), `「重複」那張白名單裡的 ${k} 三份順序都沒有重複過 —— 白名單多放一顆，上面那一圈就少守一件`);
  }
  ok(B.重複_說明 && B.重複_說明.includes("白名單"),
    "「重複」那一欄的意思換了（從「真的重複的是哪幾顆」變成白名單），資料裡卻沒有寫下來");
  ...
  /* ⚠⚠⚠ 2026-09-16（使用者：「顏色再淡一點」）——**變的是混多少白不是換一支色**：
     色相仍然釘在柔墨上，`淡` 只是往白色混幾成。換一支色畫面照樣正常，所以要有一道。 */
  ok(B.墨.toLowerCase() === "#5c5f57", `色相那一支是 ${B.墨}，不是柔墨（站上 --ink-soft，也是 色案_落選 ⓒ 那一支）`);
  ok(B.淡案.length >= 2, "顏色那把尺只剩一格 —— 那就不是尺了");
  ok(B.淡案.some((k) => k.值 === B.淡), `淡案裡沒有預設那一格（${B.淡}）`);
  ok(墨色.toLowerCase() === 淡色(B.淡).toLowerCase(), `標誌算出來是 ${墨色}，不是柔墨混白 ${B.淡}`);
  for (const k of B.淡案) {
    ok(k.值 >= 0 && k.值 < 1, `淡案裡的 ${k.標籤} 混白比例不在 0~1 之間`);
    ok(!k.標籤.includes("定案"), `${k.標籤} 的標籤裡自己寫著「定案」`);
  }
  /* ⚠⚠⚠ 這一道**翻面了**（同 71-13 的留白那一條）：預設**可以**低於裝飾性圖形那條 3:1
     —— 這一條的工作是分隔不是傳達，而站上每一條分隔線（`--rule`）自己也只有 1.55。
     但只准在三件同時成立時：① 那一格在尺上 ② 頁面上講得出那條線在哪
     ③ **尺上仍然有一格過得了 3:1**（不然一把「全部都在底下」的尺會把這一道靜靜地關掉）。 */
  if (1.05 / (lum(墨色) + 0.05) < 3) {
    ok(B.淡案.some((k) => 1.05 / (lum(淡色(k.值)) + 0.05) >= 3),
      "顏色那把尺上一格都過不了 3:1 —— 那條線要有人守得住，不然這一道等於沒開");
    /* ⚠⚠⚠ 這一道要找**那一整句**，不要找那幾個數字 —— `3:1` 與 `1.55` 在這一頁上
       不只出現一次（每一格的 .mn 自己就印著 3:1），掃數字等於沒掃：把那句話刪掉
       照樣全綠（踩過）。所以逐段找那句話裡獨一無二的三段。 */
    for (const t of ["#cdd0d2", "1.55"])
      ok(GEN.includes(t), `預設那一格低於 3:1，面板上卻沒有講出那條線在哪（少了「${t}」）`);
    ok(/對白 \$\{k\.對白/.test(GEN) || /k\.對白\.toFixed/.test(GEN),
      "面板／頁面沒有逐格印對白 —— 跨線的時候要講一句（第 28 條 ④）");
  }
  ...
  const 色條 = bands.filter((x) => x.startsWith('<svg class="bnd nrb"'));
  ok(卡條.length + 尺條.length + 色條.length === bands.length,
    "頁上有一條分隔線的 class 不是 bnd／bnd mrb／bnd nrb");
  ...
  {
    ok(色條.length === 0, `Ⓟ 那把尺的帶子條又畫回來了（${色條.length} 格）—— Ⓟ2 已定案，那把尺收掉了`);
    ok(!/bnd nrb/.test(GEN), "產生器裡還留著 bnd nrb");
    ok(淡格().length >= 2, "淡案只剩一格 —— 落選那幾格不可以刪（尺收掉了，數字還要印）");
    ok(淡格().some((k) => k.現在), "淡案裡沒有一格是定案那一格 —— 那把尺量不出「改了多少」");
    ok(/for \(const k of 淡格\(\)\)/.test(GEN), "面板沒有逐格印 Ⓟ 那把尺");
  }
  for (const m of 卡條)
    ok((m.match(/<svg x="/g) ?? []).length === BAND.顆數, `卡上那一條不是 ${BAND.顆數} 顆`);
  /* ── Ⓜ 那把尺：2026-09-16 定案 Ⓜb（27 顆 ＝ 基數 9 × 3），帶子條收掉了 ──
     ⚠⚠⚠ 這一道**翻面不放水**（同 71-13、71-18）：頁面上一格都不准畫，而四格量出來的
       每一個數字要改由**面板**逐格印，資料裡一格都不可以刪（同 50-22：尺可以收，
       它量出來的數字不可以跟著消失）。
     ⚠⚠ 面板要印的不只是尺寸，還有那兩欄**代價** —— 哪幾顆重複（換基數就換掉 Ⓚ3）、
       以及裁不裁得動（一格「選了也印不出來」而看不出來，就是送了一個假選項）。 */
  {
    ok(尺條.length === 0, `Ⓜ 那把尺的帶子條又畫回來了（${尺條.length} 格）—— Ⓜb 已定案，那把尺收掉了`);
    ok(!/bnd mrb/.test(GEN), "產生器裡還留著 bnd mrb");
    ok(顆格().length >= 2, "顆案只剩一格 —— 落選那幾格不可以刪（尺收掉了，數字還要印）");
    ok(顆格().some((k) => k.現在), "顆案裡沒有一格是定案那一格 —— 那把尺量不出「改了多少」");
    ok(/for \(const k of 顆格\(\)\)/.test(GEN),
      "面板沒有逐格印 Ⓜ 那把尺 —— 尺收掉之後，那是它量出來的數字唯一的去處");
    for (const t of ["k.一顆mm", "k.洞mm", "k.間隔mm", "k.帶高mm", "k.到主文mm", "k.那一條", "k.重複", "k.裁.裁比"])
      ok(GEN.includes(t), `面板少印了 Ⓜ 那把尺的 ${t}`);
    ok(/裁圖那一支會擋下來/.test(GEN), "面板沒有標出「選了也印不出來」那一格 —— 給出去的每一格都要是選了就能上線的");
    ok(/和挑定的那一格不一樣/.test(GEN), "面板沒有標出哪幾格換掉了「哪幾顆重複」—— 換基數不是免費的");
    /* ⚠ 「← 定案」那個記號是**算出來的**（`k.現在`），不可以寫死。
       ⚠⚠⚠ 這一道第一版寫成「GEN 裡有沒有 `k.現在 ?`」—— 而顏色那把尺的標籤裡也有一個
         （`${k.現在 ? "・現在這樣" : ""}`），所以把面板那一個換成寫死的字照樣全綠
         ＝ 這一站第十六次「掃整頁等於沒掃」。改成**切出面板那一圈**再找。 */
    {
      const i = GEN.indexOf("for (const k of 顆格())");
      const 那一圈 = i < 0 ? "" : GEN.slice(i, GEN.indexOf("\n    /*", i));
      ok(/k\.現在 \?/.test(那一圈),
        "面板那一圈裡「← 定案」不是算出來的 —— 寫死的話換一格，它會靜靜地指著舊的那一格");
    }
    /* ⚠⚠ 裁法只有一份：這一頁與裁圖那一支都 import 它，各寫一份的話面板會印一組
       和真的裁出來對不起來的數字，而且不報錯。 */
    ...
    ok(S.插圖.墨框 && ["L", "R", "T", "B"].every((x) => Number.isInteger(S.插圖.墨框[x])),
      "資料裡的 插圖.墨框 缺了一半 —— 面板算不出每一格裁不裁得動");
  }
...
ok(/Ⓜ 顆數/.test(GEN) && /Ⓚ 基數/.test(GEN) && /Ⓛ 間距/.test(GEN), "面板沒有印那三把尺");
