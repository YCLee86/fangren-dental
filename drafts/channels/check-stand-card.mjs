   ...
   ⚠ 每一件改回去畫面都還是很正常，所以每一件都要有一道。 */
{
  ...
  /* 重複of 要和 JSON 那張清單對得起來（定案那一格）——各算各的就是兩個真相 */
  ok(重複of(B.顆數).slice().sort().join() === B.重複.slice().sort().join(),
    `${B.顆數} 顆那一份順序裡重複的是 ${重複of(B.顆數).join("、")}，而「重複」那張清單寫的是 ${B.重複.join("、")}`);
     ...
     ⚠ 「不畫」那幾格不畫在頁上，但它們的數字仍然要印在面板（尺可以收，數字不可以）。 */
  {
    ...
    畫.forEach((k, i) => {
      ...
      /* ⚠⚠⚠ 基數換了「哪幾顆重複」就跟著換 —— 那是他挑定過的一格，**每一格都要印出來**，
         不然中間那兩格看起來就像免費的。 */
      ok(H.includes(`基數 ${k.基} × ${k.倍}`), `Ⓜ 那把尺 ${k.標籤} 沒有印出它用的是哪一個基數`);
      ok(H.includes(k.重複.length ? k.重複.join("、") : "一顆都沒有"),
        `Ⓜ 那把尺 ${k.標籤} 沒有印出哪幾顆重複（換基數就換掉那一格，不是免費的）`);
    ...
    /* ⚠ 中間那兩格用的基數和挑定的不一樣 —— 頁面上要有一句標出來，不可以靜靜地放進去 */
    ok(畫.every((k) => k.基 === S.帶子.顆數) || /和挑定的那一格不一樣/.test(H),
      "尺上有一格用的基數不是挑定的那一個，頁面上卻沒有標出來");
