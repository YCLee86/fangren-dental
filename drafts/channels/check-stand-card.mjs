     ...
     畫面上只會少掉最後幾個字，而每一道尺寸守門都會過（所以產生器直接 throw）。 */
{
  ...
  /* ⚠⚠⚠ 2026-09-16 使用者：「上面兩段文字的驚嘆號拿掉」—— 加回去畫面完全正常
     （少兩個字而已，字級釘在 Ⓕ、折行也不會變），每一道尺寸守門都會過，只有讀字才看得出來。
     ⚠ 現況那一張要留著它的驚嘆號（它是照片的逐字對照），所以這一道只掃「案」。 */
  for (const c of S.案) ok(!lines(c).join("").includes("！"), c.標籤 + " 又出現驚嘆號 —— 2026-09-16 使用者指定拿掉");
  const 現嘆 = (lines(S.現況).join("").match(/！/g) ?? []).length;
  ok(現嘆 === 5, `現況那一張數到 ${現嘆} 個驚嘆號（照片上是 5 個）—— 它是逐字對照，也是上面那一道的正向對照`);
