   ...
   ⚠ 每一件改回去畫面都還是很正常，所以每一件都要有一道。 */
{
  ...
  if (1.05 / (lum(墨色) + 0.05) < 3) {
    ...
    for (const t of ["<code>#cdd0d2</code>", "對白是 " + (1.05 / (lum("#cdd0d2") + 0.05)).toFixed(2), "裝飾性圖形那條 3:1"])
      ok(H.includes(t), `預設那一格低於 3:1，頁面上卻沒有講出那條線在哪（少了「${t}」）`);
  ...
  {
    const 色 = 色條.map((m) => (m.match(/color:(#[0-9a-f]{6})/i) ?? [])[1]?.toLowerCase());
    ok(色條.length === 淡格().length, `Ⓟ 那把尺畫出 ${色條.length} 格（應該是 ${淡格().length} 格）`);
    淡格().forEach((k, i) => {
      ok(色[i] === k.色.toLowerCase(),
        `Ⓟ 那把尺第 ${i + 1} 格（${k.標籤}）${色[i] == null ? "根本沒有畫出來" : `畫出來是 ${色[i]}，不是 ${k.色}`}`);
      ok(H.includes(k.色) && H.includes(k.對白.toFixed(2)),
        `Ⓟ 那把尺 ${k.標籤} 沒有把色碼或對白印出來`);
      ok((色條[i].match(/<svg x="/g) ?? []).length === BAND.顆數,
        `Ⓟ 那把尺第 ${i + 1} 格不是 ${BAND.顆數} 顆 —— 那把尺只准動顏色`);
    });
  }
  for (const m of 卡條)
