...
- ⚠ **植體的螺絲特寫、剖面圖、器械盤** —— 器械特寫是反方向（DECISIONS.md 第九節第 19 項）。
...
九輪定稿，DECISIONS.md 第九節寫著「不要再重寫這一頁」，補一行是補、不是重寫）。
...
## 七、2026-09-02：科別色換過之後，帶子跟著換到新的套色

使用者：「感覺和標籤現在的選色還是不太一樣」。站上的色 2026-08-26 前後換過
（`0a674dc` 植牙・假牙重建定案上線 Ⓒ、`fb4e4be` 重點字 Ⓓ3）：

    [data-spec="prosth"] { --accent: #465885; --accent-deep: #2a4677; }

⚠⚠ **我第一次比對是錯的** —— 那條工作分支落後 `main` 一百多個 commit，
拿到的是**舊的** `#335b8b`，還跟使用者說「站上的色沒有換過」。
**CLAUDE.md 第二節第一條「開工前一定要先同步」，這一輪整個踩上去。**
下次動任何跟站上色票有關的事，第一件事是 `node tools/sync.mjs`，
第二件事是 `grep 'data-spec="<spec>"' index.html` 現場讀一次。

⚠⚠ 另一條會再用到的：**「和標籤不一樣」要分成「底色」和「字」兩件事量。**
標籤是**純白**字，分享卡的帶子預設是**紙色** `#e2e5e6`。同一塊藍上面，
紙色字帶灰綠、整條會鈍一階 —— 底色明明已經對上了，看起來仍然不一樣。
（這一輪使用者最後選的是紙色字那一版，白字版落選。）

**新的參數**（牆仍然是 `og-shift` 填出來的 rgb(233,229,217)，所以補償色重算）：

    node drafts/og-shift.mjs drafts/og-topic-prosth-src.jpg prosth --shift 45
    node tools/og-plate.mjs prosth --blend multiply --tintcolor '#536db5' \
      --ink 0.18 --blur 6 --loc full --locpos stack

| | 值 |
| --- | --- |
| 目標（站上的套色） | `#465885` |
| 補償色 `M` | `#536db5`（＝ ((套色 − .18×墨) ÷ .82) × 255 ÷ 牆） |
| 帶子實測（十段橫掃） | `#465984`～`#495b89` —— **ΔE 0.9，肉眼分不出** |
| 紙色字對帶子 | **5.54**（門檻 4.5 ✅） |
| 對矯正套色 `#4478b5` 的 ΔE | 16.0（換色之前那一版深階是 21.0） |
