...
**狀態：✅ 定案（2026-08-24）。第四版生出來使用者說「這個很好～ 就用這張」。**
成品 `assets/og-topic-endo.jpg`（原檔 `drafts/og-topic-endo-src-raw.jpg`）。
**要重做就用第五節那一份提示詞**，逐輪的推導留在第五之〇 ~ 五之三節。
...
## 五之四、定稿那一張的實際數字（2026-08-24）

| | 定稿 | 門檻 | |
| --- | --- | --- | --- |
| 無彩空白 | **23.2%** | < 5% | ❌ **已知取捨**：房間被畫成純白的牆（頂 17% 中位 `#ffffff`）。使用者看過就是這一張，**不要拿這個數字回頭改圖** |
| 邊緣密度 | 22.7% | ≥ 30% | ❌ 同上（四版都在 16~23% 之間） |
| 頂 17% 牆色・紅色通道 | 255 | ≥ 203 | ✅ 白牆對帶子最寬鬆 |
| 帶子（兩組字之間） | 合成色 **`#a94d49`**、紙色字 中位 **4.18**・最亮處 **4.06** | ≒ 站上套色 `#ae4f4d`、對比同一般牙科／牙周那一級 | ✅ |
| 帶子安全區 | 左右各 160px、兩組字間距 410px | ≥ 160 | ✅ iMessage 中央 78.7% 的裁法下字沒有被切（`drafts/endo-card-212-imsg.jpg`） |

⚠⚠ **那兩個沒過的數字要照實記著，但不要「訂正」它** —— 同兒牙填色白字 3.22、
分享卡帶子 4.40：使用者看過畫面、指定用這一張。門檻是插畫師交件前的自我把關，
不是使用者決定之後的否決權。**下一科仍然要照門檻跑。**

**實際跑的三步**（比原本多一步，因為上緣有烘進去的白框）：

    # ① 裁掉上緣 44 列純白（og-resize 的守門會攔，不裁跑不過）
    #    → drafts/og-topic-endo-src.jpg（1424×708，比例 2.011）
    node tools/og-resize.mjs drafts/og-topic-endo-src.jpg endo        # 再往內裁 5% 到 1.91
    node tools/og-plate.mjs endo --blend multiply --tintcolor '#cb5755' \
      --ink 0.18 --blur 6 --loc full --locpos stack

⚠ `#cb5755` 是照純白的牆算的（`M = ((套色 − 0.18×墨) ÷ 0.82) × 255 ÷ 255`）。
⚠ **`drafts/og-topclean.mjs` 這一輪沒有用到** —— 放大圈的上緣確實壓在帶子底下，
但量過帶子最亮處仍有 4.06、肉眼讀不出那條弧，所以不動它。
