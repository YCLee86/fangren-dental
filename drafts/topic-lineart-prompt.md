...
**⚠⚠ 2026-08-24 的現況**：`general`／`perio`／`endo` 三科**兩張都做好了**；
**兒牙／植牙／口外／矯正四科兩張都沒有**（`og:image` 退回 `assets/og-home.jpg`）。
...
| | 一般牙科 | 牙周 | 顯微根管 |
| --- | --- | --- | --- |
| 分段 | `@media (min-width: 834px)` | **721px**（他是在 iPad mini 直放 744 上定的） | 721px |
| 大小 | `min(76%, 360px)` | 手機 `min(81.25%, 390px)`／≥721 `min(100%, 480px)` | 手機 `min(76%, 360px)`／≥721 **`min(70%, 330px)`** |
| 濃度 | `.10`／`.48` | `.115`／`.15` | `.107`／`.30` |
| 圖的形狀 | 近正方 832×788 | 橫的 1024×755 | 直的 887×949 |

⚠⚠ **顯微根管那一輪多學到一條：近正方或直的圖，卡住的是高度不是對比度** ——
480px 寬會算出 514px 高，比介紹區（1440 上 480.7）還高，而偽元素是 `bottom: 0`，
會往上凸到 h1 那一區。**提案頁的面板要現場比一次「圖有沒有比介紹區高」。**

⚠ **臨界濃度跟著該科的套色走**（愈淺愈寬鬆）：一般牙科 `#3f654a` 是 **.101**、
牙周 `#317d78` 是 **.116**、顯微根管 `#ae4f4d` 是 **.107** —— 卡住的是柔墨
`--ink-soft` 的次要文字，不是主文。
⚠ **「iPad 會壓到柔墨」不是通則**：牙周那張是橫的、橫向伸進流程那五行才會壓到；
顯微根管那張是直的，≥721 只壓到深墨（上限 .665），濃度因此純粹是美感。
