# 分享圖提示詞：齒顎矯正（`og-topic-ortho`）

**狀態：⏳ 梗已選定（2026-08-24）——`Ⓐ 一起看片` 的畫面 ＋ `Ⓒ 又跑回去的那一位` 的情緒。
提示詞在第四節，還沒生過圖。**
規格依 [ILLUSTRATION.md](../ILLUSTRATION.md) 第十一節（250px 判準）與
[TEAM.md](../TEAM.md) 第一節第 9 號（插畫師的三個交件門檻）。
文案脈絡見 COPY.md 第九之十六節與 `tools/topic-copy.mjs` 的 `ortho`。
順序：**分享圖在前、線稿底圖在後**（線稿要從這張裁一段當姿勢參考，第十二節）。

## 一、動筆之前已經被鎖死的四件

### 1. ⚠⚠ 牆的顏色先算再構圖：這一科只能在**冷光**裡畫

帶子用相乘上色，補償色 = ((套色 − 0.18×墨) ÷ 0.82) × 255 ÷ 牆的該通道。
矯正套色 `#4478b5` 算出來的分子是 **R 74 / G 137 / B 212** ——
也就是**頂 17% 那面牆的藍通道要 ≥ 212**（R ≥ 74、G ≥ 137），帶子才追得上
站上那顆藍標籤。

| 牆 | B | 過不過 |
| --- | --- | --- |
| 冷白牆、白色天花、螢幕／燈箱的溢光 | 230~250 | ✅ |
| 淺藍天 | ~232 | ✅ |
| 米黃牆、木頭色、陶土色 | 170~195 | ❌ 帶子只能往深階漂 |
| 夜景 | — | ❌ |

⚠ 這一格和兒牙**方向相反**（兒牙卡在紅通道 R ≥ 228，只能室內暖光；
矯正卡在藍通道，暖色牆一開始就出局）。**這是七科裡唯一天生要冷光的一科。**
深階 `#244369` 的下限只有 B ≥ 119，寬鬆很多 —— 落套色還是落深階，
等圖出來照兒牙那次做成 LINE 模擬讓使用者挑（紙色字：套色 3.61／深階 7.97；
純白字配套色 4.57。冷色底照判準用紙色）。

### 2. 不能和站上這五張撞

| 已上線 | 它的語彙 |
| --- | --- |
| 一般牙科（分享卡） | 白天巷弄街景、三組人各做各的事 |
| 牙周治療（分享卡） | 診間、醫師持水槍、擬人牙齒＋細菌四散（動） |
| 顯微根管（分享卡） | 診間、醫師坐椅子＋顯微鏡＋大放大圈（靜的一瞬） |
| 兒童牙科（分享卡） | 診間、醫師單膝蹲下、孩子坐診療椅、媽媽站右邊 |
| 〈牙齒矯正〉文章 HERO（會出現在同一頁的文章卡上） | 三格：街上的朋友／餐廳／臥室的牙套盒 |

→ **不畫擬人牙齒**（第三張就是同一個模子）、**不畫診療椅上的治療**
（兒牙那張已經是）、**不畫餐廳／臥室／街上戴牙套的生活片段**（同一頁下面就有那張卡）、
**醫師不坐醫師椅**（顯微根管那張是坐姿）。

### 3. 這一頁的讀者不需要被安撫

七科裡唯一一頁（COPY.md 第九之十六節）：他做過功課、在社群裡看過裝置與拔不拔牙，
缺的是**針對他自己那一口牙的判斷**。所以圖上要有的是「**兩個人一起看同一件東西**」，
不是安慰、不是歡迎、不是求救。

### 4. 圖上不准出現的（除了第十一節那九條）

- ⚠⚠ **任何時間暗示**：日曆、月份、進度條、before/after 兩格。
  療程長度與回診間隔**還沒問到診所**，整頁刻意一個數字都沒有（`ask`）。
- ⚠⚠ **任何比較**：畫壞的牙、別家的成果、對照組。頁面上「排整齊只是一半」
  之所以能寫，正是因為它沒有提到任何人。
- 病人不要畫成青少年 —— 三個現場裡使用者指名看重的是「**年紀不小了，
  現在弄還來得及嗎**」，而學生矯正是最一般的想像（他說那一類比例要降）。
  建議 **35~55 歲**。

---

## 二、三個梗（提案中，2026-08-24）

那一頁的軸：`stance` = **「排整齊只是一半 —— 咬得不對，拆掉之後很容易又跑回去。」**
`close` = 「查到的不妨帶來問問，好處壞處應該都要弄清楚。」

### Ⓐ 一起看片（插畫師推薦）

診間旁的看片角落。醫師站著，手指點在一片**亮著的側顱 X 光／口內掃描的螢幕**上；
病人（三十幾~五十歲）坐在旁邊的圓凳前傾看，**手上還捏著自己查來的那幾張紙／手機**。
右後方遠一點，一位助理正把一副上下顎模型放上架子（各做各的事 ＝「活潑」）。

- **對到**：［拔不拔　拍照、照 X 光看過，才知道要不要］、［骨頭］、close 的「查到的不妨帶來問問」。
- **視覺中心**：發亮的片子（畫面高的 ~45%），是整張圖唯一亮的東西。
- **250px**：兩個人的身體都朝向同一塊亮面 —— 那個關係是大塊面，縮到 212px 也讀得出。
- **牆**：燈箱／螢幕的冷白溢光讓上緣輕鬆過 B ≥ 212。✅
- **風險**：片子上的細節要收（只留顱側輪廓與一排牙），畫細會變雜點；
  絕不能畫成左右對照兩片（＝分格＋時間暗示）。

### Ⓑ 上下對得上（模型在手上）

醫師站著，雙手各拿上顎與下顎模型，正把它們**合起來**，一根手指停在後牙咬合的地方；
病人在旁邊湊近看。背後架子上還有幾副模型（元素少而大）。

- **對到**：直接畫那一頁的**重心那句話** —— 排整齊只是一半，上下要對得上。
- **視覺中心**：那副模型，可以放到畫面高的 ~35%（比頭大很多）——**三案裡 250px 最好讀的**。
- **風險**：模型容易被讀成「假牙」（會和日後植牙那張撞）。
  要畫得出牙弓＋弓絲，而且動作是**在手上對咬**，不是擺在桌上。

### Ⓒ 又跑回去的那一位

病人（曾經矯正過、三十幾歲）指尖輕碰自己的前牙、眉頭微皺；醫師在旁邊已經把片子調出來，
伸手示意「來，一起看一下」。

- **對到**：`cases` 第二句「矯正過一次，後來又跑掉了」＋ stance 的因果。
  這是全站唯一「失望過又回來」的讀者群，被看見的力道最強。
- **風險（三件，所以列為備案）**：① 動作太小，250px 下讀不出他在碰自己的牙；
  ② 容易變成懊悔的畫面（品牌那關要拉力不要推力）；
  ③ 一不小心就被讀成「別家做壞的」＝ 同業比較，紅線。
- **建議**：不單獨做，把它的**情緒**疊到 Ⓐ／Ⓑ 的病人身上（表情是「這次想弄清楚」）。

---

## 三、選定之後要跑的

1. 提示詞（AI 專家重排：正面敘述在前、AVOID 收成最後一小段）＋ 參考圖分組標用途。
2. 交件門檻：無彩空白 < 5%、邊緣密度 ≥ 30%、每個人的線相差 < 20 階
   （`drafts/og-measure.mjs`、`og-measure-ink.mjs`）；頂 17% 的 B 通道實測 ≥ 212。
3. `node tools/og-resize.mjs drafts/og-topic-ortho-src.jpg ortho`
   → `node tools/og-plate.mjs ortho --blend multiply --tintcolor <算出來的補償色> --ink 0.18 --blur 6 --loc full --locpos stack`
4. 帶子落套色還是深階，做成 LINE 模擬（`node drafts/line-mock.mjs ortho`）讓使用者挑。
5. 定案之後才做線稿底圖（姿勢參考從這張裁）。

---

## 二之一、✅ 使用者選的：Ⓐ 的畫面 ＋ Ⓒ 的情緒（2026-08-24）

**畫面走 Ⓐ**（兩個人一起看一片亮著的側顱 X 光），
**病人的設定走 Ⓒ**（矯正過一次又跑掉、這次想弄清楚的那一位）。

這個組合解掉 Ⓒ 單獨做的三個風險，而且一個都沒有損失：

| Ⓒ 單獨做的風險 | 合併之後 |
| --- | --- |
| 動作太小，250px 讀不出「他在碰自己的牙」 | 畫面的可讀性由 Ⓐ 那塊發亮的片子扛，情緒只需要落在**表情與前傾的身體**上 |
| 容易變成懊悔的畫面 | 他不是在回想，是在**看下一步** —— 視線朝外、朝向片子，不是低頭 |
| 會被讀成「別家做壞的」 | 畫面上沒有「上一次」的任何痕跡（沒有對照片、沒有舊模型），只有這一次的片子 |

⚠⚠ 所以病人的情緒**只用三件事表達**：眉頭有一點點收、身體明顯前傾、
手上捏著自己查來的紙。**不畫懊悔、不畫嘆氣、不畫手碰牙。**

---

## 三、250px 下會讀到什麼（先想這件事，再想畫面好不好看）

訊息卡實測只有 **212 CSS px** 寬。能活下來的只有五塊：

    ① 畫面中央一塊**發亮的長方形**（看片燈箱）——整張圖唯一亮的東西
    ② 左邊一個穿白袍的人，手臂抬起、手指點在那塊亮面上
    ③ 右邊一個坐著、身體前傾的人，手上一疊白紙
    ④ 兩個人的臉都被那塊亮面打亮 ＝ 他們在看同一件東西
    ⑤ 右後方一個小小的人，在把模型放上架子

所以：**燈箱要大**（畫面高的 45%，212px 的卡上約 60px 高）、
**兩個人要大**（站著的醫師全身佔畫面高的 ~75%）、
片子上只留**顱側的輪廓線與一排牙**，不要畫細節 —— 細節在那個尺寸只會變成雜點。

⚠ 燈箱**只有一片**。兩片並排 ＝ 分格 ＋ 時間暗示，兩條紅線一起踩。

---

## 四、參考圖清單（用途要分開標）

⚠ 不標用途的參考圖會被整張抄走（風格、顏色、構圖一起）——TEAM.md 第一節第 10 號。

| # | 檔案 | **只參考這個** | **不要參考** |
| --- | --- | --- | --- |
| ① | `assets/og-topic-endo.jpg` | **人的畫法**：線的實度、平塗的臉（點眼睛、沒有眼白）、白袍與刷手服的形狀、紙紋與整體密度 | **顏色**（那張是磚紅系，這一張是冷白＋藍）、構圖、放大圈、擬人牙齒 |
| ② | `assets/og-topic-general.jpg` | **「幾件小事同時在發生」的節奏**：三組人各做各的事、彼此不互看 | 構圖（那是室外街景）、人數、房子 |
| ③ | **⚠ 要跟使用者要**：一張**牙科側顱 X 光**（cephalometric）的實拍 | **片子上的形狀**：顱側輪廓、鼻與下巴的線、上下排牙齒的位置關係 | 顏色、畫法、上面的任何文字與刻度 |
| ④ | 選配：診所自己那台**看片燈箱／螢幕**的實拍 | 機器的形狀與比例 | 房間、顏色 |

⚠ ③ 正是 ILLUSTRATION.md 第十之一節那條的適用對象：**形狀不要用文字描述，用參考圖。**
沒有 ③ 就會得到一張「看起來像 X 光但哪裡都不對」的片子 —— 而這一頁的讀者做過功課，
他看得出來。

---

## 五、提示詞（第一版，逐字，可直接複製）

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, few large objects, one single continuous scene, no panels and no dividing
lines.

THE WHOLE PICTURE IS BRIGHT, CLEAN AND COOL - a corner of a dental clinic by day where two
people are looking at a lit X-ray together. The walls are a very light COOL off-white with a
faint blue in them, about #e6ebf0 - never cream, never beige, never tan and never yellow, and
never a neutral dead grey either. DO NOT PUT AN OVERALL WARM, AMBER OR SEPIA CAST OVER THE
IMAGE. Read this paragraph as the mood of the whole image before drawing anything else.

THE SIX THINGS THAT MATTER MOST, IN ORDER:

1. DRAW THE PEOPLE EXACTLY IN THE STYLE OF THE REFERENCE ILLUSTRATION OF THE DENTIST.
   Thin hand-drawn linework whose weight varies and sometimes breaks - NOT a thick even
   outline. Each face is ONE FLAT SKIN TONE with no shading, no modelling, no cheekbones and
   no jaw shadow. On the face there is only: the outline, two eyes drawn as small simple
   dots with NO whites, NO pupils and NO highlights, two short eyebrows, a tiny nose mark, a
   small mouth line and an ear. Hair is a flat shape in two tones with no individual strands.
   EVERY PERSON IN THIS PICTURE IS DRAWN WITH EXACTLY THE SAME LINE WEIGHT AND THE SAME
   SOLIDITY, including the one furthest away. This is the single most important instruction.

2. THE LIT X-RAY VIEWER IS THE HERO AND THE ONLY BRIGHT THING IN THE PICTURE. A single
   upright rectangular light box standing on a low counter in the MIDDLE of the picture,
   TALLER THAN IT IS WIDE, its glowing face about two fifths of the picture height. DRAW IT
   BIG - after the two people it is the largest thing here. Its face glows an even cool
   white (#f4f8fb), spilling soft light onto the counter, onto the wall behind it and onto
   both faces, so it is obvious that both people are lit by it. ONE SINGLE FILM is clipped to
   it - never two, never a row, never a before-and-after pair. On the film, drawn in soft
   pale grey lines on the glowing ground: the SIDE PROFILE OF A HUMAN SKULL exactly as in the
   reference X-ray photograph - forehead, nose, chin, and the upper and lower rows of teeth
   meeting each other. Big simple shapes only: no measuring lines, no scales, no arrows, no
   numbers and no writing of any kind.

3. THE DENTIST IS STANDING TO THE LEFT OF THE LIGHT BOX, AND HE IS POINTING AT IT. A
   Taiwanese man in his late thirties, short black hair, clean-shaven, standing upright and
   turned three-quarters towards the light box. HIS ARM GEOMETRY, EXACTLY: the arm nearest
   the light box is RAISED TO ABOUT CHEST HEIGHT, the elbow stays bent and close to his body,
   the forearm reaching in at a slight upward angle, the INDEX FINGER EXTENDED and its tip
   TOUCHING the lower middle of the film where the two rows of teeth meet - the other fingers
   curled in loosely. His other arm hangs relaxed at his side. HIS EYES ARE ON THE FILM, not
   on us and not on the patient. He is explaining something quietly, mouth slightly open.
   HE IS NOT WAVING, NOT PRESENTING WITH AN OPEN PALM, NOT GESTURING TOWARDS THE VIEWER AND
   NOT GREETING ANYONE. USE THIS ANCHOR: standing, the top of his head sits just below the
   top sixth of the picture and his legs are cropped by the bottom edge, so he is drawn large
   and close to us.

4. THE PATIENT IS SITTING ON A ROUND STOOL TO THE RIGHT OF THE LIGHT BOX, LEANING IN. A
   Taiwanese man or woman of about forty - an ordinary working adult, NOT a teenager and NOT
   a student. He sits on a simple round stool, KNEES TOGETHER AND BODY CLEARLY TIPPED FORWARD
   FROM THE HIPS towards the film, forearms resting on his thighs. IN BOTH HANDS HE HOLDS A
   SMALL SHEAF OF PRINTED PAPER - three or four loose A4 sheets with a few grey lines of
   text-like marks on them (marks only, NO readable writing) - held up a little, as if he has
   brought his own homework with him. HIS EYES ARE ON THE FILM.
   HIS EXPRESSION IS "THIS TIME I WANT TO UNDERSTAND IT PROPERLY": mouth closed and calm,
   the inner ends of his eyebrows drawn very slightly together in concentration, chin
   slightly lifted towards the film. He is ATTENTIVE AND ENGAGED. He is NOT sad, NOT
   regretful, NOT sighing, NOT anxious, NOT frowning hard, NOT smiling broadly, and he is
   NOT touching his own mouth, teeth or jaw with his hand.

5. A THIRD PERSON IS BUSY WITH SOMETHING ELSE IN THE BACKGROUND, AND SHE IS NOT LOOKING AT
   THEM. Further back on the RIGHT, smaller because she is further away but drawn with
   exactly the same line weight, a dental nurse stands side-on at an open shelving unit,
   BOTH HANDS PLACING A PAIR OF PLASTER DENTAL MODELS - an upper and a lower jaw - onto a
   shelf. Two or three more pairs of models already sit on the shelves beside them. She is
   absorbed in her own small task. This is what makes the picture feel alive: THREE PEOPLE,
   TWO SEPARATE THINGS HAPPENING AT ONCE, nobody posing for us.

6. NO WRITING ANYWHERE in the image, in any language - not on the film, not on the papers,
   not on the wall, not on the shelves.

EVERYONE LOOKS DIFFERENT. The three people differ clearly in age, build, hair and clothing
colour: the dentist is tall and slim in a white coat; the patient is stockier, in everyday
clothes; the nurse is shorter with her hair tied up. No two faces are alike.

COLOUR. The clinic blue of this speciality is #4478b5 and it appears in only three places:
the dentist's scrubs under his open white coat (a soft dusty blue, #6a94c4, with a slightly
deeper blue trim), the cool glow of the light box, and one thin blue line along the edge of
the counter. Everything else is quiet: the patient's clothes are a warm oatmeal shirt
(#d8cdbb) over dark slate trousers, the nurse's scrubs are pale slate blue-grey, the plaster
models are chalky off-white, the shelving is pale wood-grey. The picture must NOT become an
all-blue image: it is a bright white room with a few blue things in it.

THE ROOM - IMPORTANT AND EXACT. FIVE LARGE, SIMPLE OBJECTS AND NOTHING ELSE: the light box
on its low counter; the open shelving unit with the plaster models on the right; a tall
window with a plain cool sky beyond it on the left, cropped by the picture edge; the round
stool; and a long clean counter running behind them. The floor is a pale seamless clinic
vinyl, slightly cooler than the wall, softly reflective with a few long horizontal
reflection strokes, meeting the wall in a CONTINUOUS COVED SKIRTING that curves up the wall.
The room is SIMPLE BECAUSE THE THINGS IN IT ARE FEW AND LARGE, NOT because it is empty:
there must be no bare unbroken expanse of wall in the lower two thirds of the picture. NO
posters, NO framed pictures, NO plants, NO instrument trays, NO cabinets full of small
bottles, NO dental chair.

THE TOP SIXTH OF THE PICTURE IS PLAIN WALL AND NOTHING ELSE - no heads, no hands, no
objects, no light box, no shelves, no window frame, and nothing darker or brighter anywhere
inside that strip.

TEXTURE AND FINISH - fine even paper grain over the whole image, flat colour areas with a
few loose hand-drawn shading strokes, gentle and warm-hearted, the look of a printed
editorial illustration.

AVOID: any second film, any before-and-after comparison, any panel or dividing line; a
calendar, a clock, a progress bar or any hint of how long treatment takes; a dental chair
with someone being treated in it; a cartoon tooth with a face; braces or aligners shown as
props; anyone waving or greeting the viewer; anyone looking at the camera; a faded,
translucent or ghostly background figure; readable text or numbers anywhere; a crowded room;
a warm yellow or sepia overall cast; cream, beige or tan walls.
```

---

## 六、交件前要過的門檻（插畫師自己跑，不過就不拿出來）

| | 門檻 | 腳本 |
| --- | --- | --- |
| 無彩空白（S<12 且 L>80） | **< 5%** | `node drafts/og-measure.mjs <圖>` |
| 邊緣密度 | **≥ 30%** | `node drafts/og-measure-ink.mjs <圖>` |
| 三個人的線相差 | **< 20 階** | 同上 |
| 站著的醫師佔畫面高 | **≥ 70%** | 目視 ＋ 量 |
| 頂 17% 是不是純牆 | 沒有東西 | `og-measure.mjs` 的區塊表 |
| **頂 17% 的藍通道** | **B ≥ 212**（這一科的硬約束，第一節） | 同上 |

⚠ 這一張最可能出事的是**無彩空白**：冷白牆很容易掉進 S<12 那一格。
提示詞因此把牆寫成**帶一點藍的冷白 `#e6ebf0`**（S 約 22）而不是純白或中性灰。
第一版量出來若超過 5%，先看是不是牆被畫成了純白。

## 七、管線（圖回來之後）

    node tools/og-resize.mjs drafts/og-topic-ortho-src.jpg ortho
    node tools/og-plate.mjs ortho --blend multiply --tintcolor #5294e1 \
      --ink 0.18 --blur 6 --loc full --locpos stack

⚠ `#5294e1` 是**牆量到 `#e6ebf0`（230/235/240）時**算出來的補償色，
帶子會落在套色 `#4478b5`；牆的實測值一變就要重算（公式在第一節）。
落深階 `#244369` 的話補償色是 `#264e7f`。
兩種都做一張，用 `node drafts/line-mock.mjs ortho` 放進 LINE 訊息的模擬裡讓使用者挑
（紙色字：套色 3.61／深階 7.97 —— 冷色底照判準用紙色，不換白字）。

## 八、還要問使用者的

1. **側顱 X 光的參考圖**（第四節 ③）—— 沒有它片子一定會畫歪。
2. **醫師畫成男的還是女的？** 這一版預設**男醫師** —— 已上線四張是
   一般牙科（男）／牙周（男）／顯微根管（女）／兒童牙科（女），
   矯正排在導覽列最後一顆，鄰居是還沒畫的口外，兩邊都不撞。要換一句話就好。
3. **病人設定成「矯正過一次又跑掉」的那一位**（使用者已選），
   但頁面上沒有任何一句寫「上一次在哪裡做的」—— 圖上也**刻意不留痕跡**。
   這一點如果他想再明確一點，只能靠表情，不能靠道具。

---

## 五之一、Ⓑ 的提示詞（2026-08-24，使用者另外要的一版）

⚠ **這是另一個梗，不是 Ⓐ 的修改版** —— 兩份提示詞各自完整，可以各生一張比。
病人的設定沿用使用者已選的 Ⓒ 情緒（矯正過一次又跑掉、這次想弄清楚），
所以兩張的差別**只有梗本身**：Ⓐ 看的是片子（骨頭、拔不拔），Ⓑ 看的是**上下咬合**
（＝那一頁的重心那句話「排整齊只是一半」）。

**250px 下會讀到的五塊**：① 中央一雙手捧著的兩塊白色牙弓，正要合起來
② 左邊穿白袍的人 ③ 右邊坐著前傾的人 ④ 背景一扇亮的窗 ⑤ 遠處推著小車的人。

⚠⚠ **Ⓑ 特有的三個風險**（提示詞裡逐條擋掉了）：
・**模型被讀成活動假牙** → 寫死 plaster study models on trapezoid bases，
  沒有粉紅色牙齦壓克力、沒有金屬鉤。
・**無彩空白爆表** → 石膏是白的、牆也是白的，兩塊加起來很容易超過 5%。
  石膏因此寫成**帶暖的米白 `#efe9df`**（S 約 33）、牆是**帶藍的冷白 `#e6ebf0`**（S 約 22），
  兩塊都不准是純白或中性灰。
・**變成裝置展示** → 模型上**不畫矯正器與弓絲**（那一頁刻意沒寫戴什麼）。

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, few large objects, one single continuous scene, no panels and no dividing
lines.

THE WHOLE PICTURE IS BRIGHT, CLEAN AND COOL - a corner of a dental clinic by day, with
daylight coming in from a tall window. The walls are a very light COOL off-white with a
faint blue in them, about #e6ebf0 - never cream, never beige, never tan and never yellow, and
never a neutral dead grey either. DO NOT PUT AN OVERALL WARM, AMBER OR SEPIA CAST OVER THE
IMAGE. Read this paragraph as the mood of the whole image before drawing anything else.

THE SIX THINGS THAT MATTER MOST, IN ORDER:

1. DRAW THE PEOPLE EXACTLY IN THE STYLE OF THE REFERENCE ILLUSTRATION OF THE DENTIST.
   Thin hand-drawn linework whose weight varies and sometimes breaks - NOT a thick even
   outline. Each face is ONE FLAT SKIN TONE with no shading, no modelling, no cheekbones and
   no jaw shadow. On the face there is only: the outline, two eyes drawn as small simple
   dots with NO whites, NO pupils and NO highlights, two short eyebrows, a tiny nose mark, a
   small mouth line and an ear. Hair is a flat shape in two tones with no individual strands.
   EVERY PERSON IN THIS PICTURE IS DRAWN WITH EXACTLY THE SAME LINE WEIGHT AND THE SAME
   SOLIDITY, including the one furthest away. This is the single most important instruction.

2. THE PAIR OF PLASTER JAW MODELS IS THE HERO, AND THEY ARE BEING CLOSED TOGETHER. Held in
   the dentist's two hands in the MIDDLE of the picture, at his chest height, NEAREST TO US
   AND LARGER THAN ANYTHING ELSE IN THE SCENE - together they stand about a third of the
   picture height. THE UPPER MODEL IS IN HIS UPPER HAND WITH ITS ARCH OF TEETH FACING DOWN;
   THE LOWER MODEL IS IN HIS LOWER HAND WITH ITS ARCH FACING UP; the two arches are aligned
   and almost meeting, with a NARROW GAP still open between them, so it is obvious he is in
   the act of closing them onto each other. Each model is a simple chalky block: a plain
   TRAPEZOID BASE with a horseshoe row of teeth on it, the individual teeth drawn as a row
   of simple rounded shapes, big and readable. THE PLASTER IS A WARM OFF-WHITE (#efe9df,
   shaded with #ddd5c6) WITH LOOSE HAND-DRAWN SHADING STROKES ON IT - never pure white and
   never flat. THE MODELS ARE PLAIN PLASTER: no braces, no brackets, no wires, no pink gum
   acrylic, no metal clasps, no springs - they are study models, NOT dentures.

3. THE DENTIST IS STANDING BEHIND THE MODELS, SHOWING HOW THEY MEET. A Taiwanese man in his
   late thirties, short black hair, clean-shaven, standing upright and slightly turned
   towards the seated patient, his weight even and easy. HIS HANDS, EXACTLY: both forearms
   come up in front of his chest, elbows bent and staying close to his body; one hand holds
   the upper model from above, the other holds the lower model from below; THE INDEX FINGER
   OF THE LOWER HAND IS EXTENDED AND POINTS AT THE BACK TEETH, where the two arches meet at
   the side. HIS EYES ARE ON THE MODELS, not on us and not on the patient. He is explaining
   something quietly, mouth slightly open. HE IS NOT PRESENTING THE MODELS TO THE VIEWER,
   NOT HOLDING THEM UP LIKE A TROPHY, NOT WAVING AND NOT GREETING ANYONE. USE THIS ANCHOR:
   standing, the top of his head sits just below the top sixth of the picture and his legs
   are cropped by the bottom edge, so he is drawn large and close to us.

4. THE PATIENT IS SITTING ON A ROUND STOOL TO THE RIGHT, LEANING IN TO LOOK. A Taiwanese man
   or woman of about forty - an ordinary working adult, NOT a teenager and NOT a student. He
   sits on a simple round stool, KNEES TOGETHER AND BODY CLEARLY TIPPED FORWARD FROM THE HIPS
   towards the models, forearms resting on his thighs. IN BOTH HANDS HE HOLDS A SMALL SHEAF
   OF PRINTED PAPER - three or four loose A4 sheets with a few grey lines of text-like marks
   on them (marks only, NO readable writing) - resting on his knees, as if he has brought his
   own homework with him. HIS EYES ARE ON THE GAP BETWEEN THE TWO MODELS.
   HIS EXPRESSION IS "THIS TIME I WANT TO UNDERSTAND IT PROPERLY": mouth closed and calm,
   the inner ends of his eyebrows drawn very slightly together in concentration, chin
   slightly lifted towards the models. He is ATTENTIVE AND ENGAGED. He is NOT sad, NOT
   regretful, NOT sighing, NOT anxious, NOT frowning hard, NOT smiling broadly, and he is
   NOT touching his own mouth, teeth or jaw with his hand.

5. A THIRD PERSON IS BUSY WITH SOMETHING ELSE IN THE BACKGROUND, AND SHE IS NOT LOOKING AT
   THEM. Further back on the LEFT, smaller because she is further away but drawn with exactly
   the same line weight, a dental nurse walks past pushing a small clinic trolley - a simple
   box on castors with one folded cloth on its top shelf. She is absorbed in her own small
   task and does not look at the other two. This is what makes the picture feel alive: THREE
   PEOPLE, TWO SEPARATE THINGS HAPPENING AT ONCE, nobody posing for us.

6. NO WRITING ANYWHERE in the image, in any language - not on the papers, not on the wall,
   not on the trolley, not on the shelves.

EVERYONE LOOKS DIFFERENT. The three people differ clearly in age, build, hair and clothing
colour: the dentist is tall and slim in a white coat; the patient is stockier, in everyday
clothes; the nurse is shorter with her hair tied up. No two faces are alike.

COLOUR. The clinic blue of this speciality is #4478b5 and it appears in only three places:
the dentist's scrubs under his open white coat (a soft dusty blue, #6a94c4, with a slightly
deeper blue trim), the cool sky in the window, and one thin blue line along the edge of the
counter. Everything else is quiet: the patient's clothes are a warm oatmeal shirt (#d8cdbb)
over dark slate trousers, the nurse's scrubs are pale slate blue-grey, the shelving is pale
wood-grey. The picture must NOT become an all-blue image: it is a bright white room with a
few blue things in it.

THE ROOM - IMPORTANT AND EXACT. FOUR LARGE, SIMPLE OBJECTS AND NOTHING ELSE: a TALL WINDOW
on the left with a plain cool sky beyond it, its lower half behind the nurse; an OPEN
SHELVING UNIT on the right holding two or three more pairs of plaster models, big and simple;
a long clean counter running behind them; and the round stool. The floor is a pale seamless
clinic vinyl, slightly cooler than the wall, softly reflective with a few long horizontal
reflection strokes, meeting the wall in a CONTINUOUS COVED SKIRTING that curves up the wall.
The room is SIMPLE BECAUSE THE THINGS IN IT ARE FEW AND LARGE, NOT because it is empty:
there must be no bare unbroken expanse of wall in the lower two thirds of the picture. NO
posters, NO framed pictures, NO plants, NO instrument trays, NO cabinets full of small
bottles, NO dental chair.

THE TOP SIXTH OF THE PICTURE IS PLAIN WALL AND NOTHING ELSE - no heads, no hands, no models,
no shelves, no window frame, and nothing darker or brighter anywhere inside that strip.

TEXTURE AND FINISH - fine even paper grain over the whole image, flat colour areas with a
few loose hand-drawn shading strokes, gentle and warm-hearted, the look of a printed
editorial illustration.

AVOID: dentures, partial dentures, pink gum-coloured acrylic, metal clasps; braces, brackets,
wires or aligners anywhere; a giant single tooth; a cartoon tooth with a face; a calendar, a
clock, a progress bar or any hint of how long treatment takes; any before-and-after
comparison, any panel or dividing line; a dental chair with someone being treated in it;
anyone waving, greeting or presenting to the viewer; anyone looking at the camera; a faded,
translucent or ghostly background figure; readable text or numbers anywhere; a crowded room;
a warm yellow or sepia overall cast; cream, beige or tan walls; pure flat white plaster.
```

⚠ Ⓑ 的房間比 Ⓐ 少一件（沒有燈箱），所以**邊緣密度更容易不足** ——
窗、架上的模型、推車、檯面那條線是撐密度的四樣，第一版若低於 30%
先看是不是把架子畫空了。

---

## 五之二、Ⓐ 與 Ⓑ 都被退回（2026-08-24）

> 使用者：「這兩版我覺得都**沒有表達出病人牙齒凌亂或是需要矯正的感覺**，
> 　或是**醫師專精矯正、專科醫師的水準**。」

存檔：`drafts/og-topic-ortho-v1-xray.jpg`（Ⓐ）、`drafts/og-topic-ortho-v2-models.jpg`（Ⓑ），
兩張都是 1424×752。

### ⚠⚠ 病灶：我把「兩個人一起看」當成主題，但主題應該是「**他們看到的那個東西**」

分享卡只有 **212px**，人在那個尺寸只是兩塊剪影 —— **資訊全部要落在被看的那樣東西上**，
而那樣東西在這兩張裡都太小、而且內容是錯的：

| | 牙齒在卡上剩多大（212px 寬，框是目測） | 內容對不對 |
| --- | --- | --- |
| Ⓐ X 光 | 片子上那排牙約 **9×5px** ＝ 一個小點 | ❌ 而且片子上的牙是**整齊**的 |
| Ⓑ 模型 | 牙列約 **39×10px** ＝ 一條白帶 | ❌ 模型的牙**排得整整齊齊**，等於在說「已經很好了」 |

**「亂」與「專科」都不在畫面上，是靠讀者腦補的。** 使用者的兩句話逐字成立。

### 另外兩件（他沒提，但下一版一定要擋）

- ⚠⚠ **Ⓐ 那片 X 光是一顆有眼窩的骷髏頭**。分享卡是聊天訊息裡掃過去的一眼，
  縮圖上先讀到骷髏 —— 那是推力不是拉力（品牌那一關）。
  **下一版只留側臉輪廓線與上下顎，不要顱骨。**
- ⚠ **Ⓑ 左下角多出一隻沒有身體的手**（托著下顎模型那隻），比例也比醫師的手大一號。
  成因是我把「雙手各拿一副」寫得太獨立，模型就分給了兩個人。
  **要嘛寫死「同一個人的兩隻手，兩條手臂都看得見」，要嘛只讓他拿一副。**

### ⚠⚠ 「畫在病人嘴裡」這條路走不通，先算過了

最直覺的做法是讓病人張嘴露出擁擠的牙 —— 但大人全身要佔畫面高 70%（硬規格 3），
那張嘴在 212px 的卡上只有 **3~4px**。**要讓「亂」看得見，那排牙就必須離開嘴巴、
單獨放大成畫面的主體。** 下面三案都是照這個結論設計的。

### 三個新方向（提案中）

| | 梗 | 「亂」看得見嗎 | 「專科」看得見嗎 | 250px 好讀嗎 |
| --- | --- | --- | --- | --- |
| **Ⓓ 咬不上的那一副** | Ⓑ 的修正版：模型的牙**擁擠歪斜**，而且**上下咬不對**（後牙之間留著一道楔形的縫），醫師的手指停在那道縫上。模型放大到畫面高 45%，兩個人退到兩側 | ✅ | ○ 他看的是**咬合**不是排列 ＝ 那一頁的重心 | ✅ |
| **Ⓔ 描圖線** | Ⓐ 的修正版：骷髏拿掉，片子放大到畫面 60%，上面是側臉輪廓＋上下顎＋**擠在一起的牙**，並疊著矯正專科在畫的幾條分析線與一個角；醫師手上是描圖筆，正在畫 | △ 片子上的牙仍然小 | ✅✅ **那幾條線別科不做** | ❌ 線在 212px 會糊 |
| **Ⓕ 比人還高的一排牙**（插畫師推薦） | 超現實比例（＝牙周那張已經用過、站上接受的語彙）：醫師站在一排**比他還高的牙齒**旁邊，其中幾顆明顯歪斜擠在一起；他正伸手把最歪的那一顆**扶正**，旁邊一處讓出了空位、底下有一顆**還沒長出來的牙**正要上來。病人站在另一側看 | ✅✅ 一眼就是亂的 | ✅✅ 「不對稱拔牙、**牙沒長出來**，都會評估」正是那一行，而且是使用者自己口述的差異化 | ✅✅ 畫面上最大的形狀就是那排牙 |

⚠ Ⓕ 和牙周那張的差別要寫死在提示詞裡：**牙齒沒有臉、沒有手腳**（牙周與顯微根管
已經各用過一次擬人牙齒，第三次就是同一個模子），而且**動作不同**
（牙周是噴水趕細菌，這裡是**把一顆牙扶到位**）。

---

## 五之三、Ⓓ 的提示詞（咬不上的那一副，2026-08-24）

**共同的三道守門**（Ⓓ Ⓔ 都寫進去了，是上一輪退回的三件）：
① **牙齒一定要是亂的** —— 這是整張圖最重要的內容，不是背景細節。
② **不准出現沒有身體的手** —— 每一隻手都要接得到看得見的手臂與人。
③ **不准畫顱骨** —— 骷髏在 212px 的縮圖上先被讀到，那是推力。

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, few large objects, one single continuous scene, no panels and no dividing
lines.

THE WHOLE PICTURE IS BRIGHT, CLEAN AND COOL - a corner of a dental clinic by day, daylight
from a tall window. The walls are a very light COOL off-white with a faint blue in them,
about #e6ebf0 - never cream, never beige, never tan, never yellow, and never a neutral dead
grey either. DO NOT PUT AN OVERALL WARM, AMBER OR SEPIA CAST OVER THE IMAGE.

THE MOST IMPORTANT CONTENT OF THIS PICTURE IS THAT THE TEETH ARE CROOKED AND THE TWO JAWS DO
NOT MEET PROPERLY. If a viewer sees this image at thumbnail size and cannot tell that the
teeth are untidy, the illustration has failed. Draw that first, then everything else.

THE SIX THINGS THAT MATTER MOST, IN ORDER:

1. THE PAIR OF PLASTER JAW MODELS IS THE HERO AND IT IS BIG. The two models stand together
   on a low counter in the MIDDLE OF THE PICTURE, TOGETHER ABOUT FORTY-FIVE PER CENT OF THE
   PICTURE HEIGHT and nearer to us than anything else. The lower model rests on the counter
   with its horseshoe arch facing up; the upper model sits above it, arch facing down, held
   steady by the dentist. Each model is a simple chalky block: a plain TRAPEZOID BASE with a
   horseshoe row of large, clearly separated teeth on it. THE PLASTER IS A WARM OFF-WHITE
   (#efe9df, shaded with #ddd5c6) WITH LOOSE HAND-DRAWN SHADING STROKES - never pure white,
   never flat. They are study models, NOT dentures: no pink gum-coloured acrylic, no metal
   clasps, no braces, no brackets, no wires.

2. HOW THE TEETH ARE WRONG - DRAW ALL THREE OF THESE, LARGE AND OBVIOUS:
   • CROWDING: in the upper front row, TWO NEIGHBOURING TEETH ARE ROTATED AND OVERLAP EACH
     OTHER - one pushed forward out of the line, the one beside it tucked in behind it. One
     more tooth further along leans at a clear angle. THE FRONT EDGE OF THE ROW IS UNEVEN AND
     WAVY, never a straight tidy line.
   • THE UPPER ARCH SITS FORWARD OF THE LOWER ONE: the upper front teeth overhang the lower
     front teeth by about the width of one tooth, so the two arches plainly do not line up.
   • AN OPEN WEDGE: at the BACK of the arches on the side nearest us, the upper and lower
     teeth DO NOT TOUCH. There is a clear wedge-shaped gap between them, narrow at the front
     and widening towards the back, big enough to see instantly at thumbnail size.
   The lower row is untidy too, but less so. NOTHING about these two rows should look neat,
   finished or corrected.

3. THE DENTIST IS STANDING BEHIND THE MODELS AND POINTING INTO THAT GAP. A Taiwanese man in
   his late thirties, short black hair, clean-shaven, standing upright behind the counter,
   turned three-quarters towards the seated patient. BOTH OF HIS ARMS ARE FULLY VISIBLE AND
   BOTH CONNECT TO HIS BODY: one hand rests on top of the upper model, steadying it; the
   other arm comes forward with the INDEX FINGER EXTENDED, its tip stopping just at the open
   wedge between the back teeth. HIS EYES ARE ON THAT GAP, not on us and not on the patient.
   He is explaining something quietly, mouth slightly open. THERE IS NO HAND ANYWHERE IN THIS
   PICTURE THAT DOES NOT BELONG TO A VISIBLE PERSON - no floating hand, no cropped forearm
   entering from outside the frame, exactly two hands on the dentist. USE THIS ANCHOR:
   standing, the top of his head sits just below the top sixth of the picture and the counter
   crosses the picture at about two thirds of its height.

4. THE PATIENT IS SITTING ON A ROUND STOOL TO THE RIGHT, LEANING IN TO LOOK. A Taiwanese man
   or woman of about forty - an ordinary working adult, NOT a teenager and NOT a student. He
   sits on a simple round stool, KNEES TOGETHER AND BODY CLEARLY TIPPED FORWARD FROM THE HIPS
   towards the models, forearms on his thighs, a small sheaf of three or four printed A4
   sheets in one hand (grey text-like marks only, NO readable writing). HIS EYES ARE ON THE
   GAP BETWEEN THE TWO ARCHES. HIS EXPRESSION IS "THIS TIME I WANT TO UNDERSTAND IT
   PROPERLY": mouth closed and calm, the inner ends of his eyebrows very slightly drawn
   together in concentration. He is NOT sad, NOT regretful, NOT anxious, NOT smiling broadly,
   and he is NOT touching his own mouth or jaw.

5. A THIRD PERSON IS BUSY WITH SOMETHING ELSE IN THE BACKGROUND, AND SHE IS NOT LOOKING AT
   THEM. Further back on the LEFT, smaller because she is further away but drawn with exactly
   the same line weight, a dental nurse walks past pushing a small clinic trolley - a simple
   box on castors with one folded cloth on its top shelf. She is absorbed in her own task.
   THREE PEOPLE, TWO SEPARATE THINGS HAPPENING AT ONCE, nobody posing for us.

6. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

DRAW THE PEOPLE EXACTLY IN THE STYLE OF THE REFERENCE ILLUSTRATION OF THE DENTIST. Thin
hand-drawn linework whose weight varies and sometimes breaks - NOT a thick even outline. Each
face is ONE FLAT SKIN TONE with no shading, no modelling, no cheekbones and no jaw shadow. On
the face there is only: the outline, two eyes drawn as small simple dots with NO whites, NO
pupils and NO highlights, two short eyebrows, a tiny nose mark, a small mouth line and an ear.
Hair is a flat shape in two tones with no individual strands. EVERY PERSON IS DRAWN WITH
EXACTLY THE SAME LINE WEIGHT AND SOLIDITY, including the one furthest away. The three people
differ clearly in age, build, hair and clothing colour, and no two faces are alike.

COLOUR. The clinic blue of this speciality is #4478b5 and it appears in only three places:
the dentist's scrubs under his open white coat (a soft dusty blue, #6a94c4, with a slightly
deeper trim), the cool sky in the window, and one thin blue line along the counter edge.
Everything else is quiet: the patient wears a warm oatmeal shirt (#d8cdbb) over dark slate
trousers, the nurse pale slate blue-grey, the shelving pale wood-grey. The picture must NOT
become an all-blue image: it is a bright white room with a few blue things in it.

THE ROOM - FOUR LARGE SIMPLE OBJECTS AND NOTHING ELSE: a TALL WINDOW on the left with a plain
cool sky beyond it; an OPEN SHELVING UNIT on the right holding two or three more pairs of
plaster models; the long counter the models stand on; and the round stool. The floor is a
pale seamless clinic vinyl, slightly cooler than the wall, softly reflective with a few long
horizontal strokes, meeting the wall in a CONTINUOUS COVED SKIRTING. The room is SIMPLE
BECAUSE THE THINGS IN IT ARE FEW AND LARGE, NOT because it is empty: no bare unbroken expanse
of wall in the lower two thirds. NO posters, NO framed pictures, NO plants, NO instrument
trays, NO cabinets of small bottles, NO dental chair.

THE TOP SIXTH OF THE PICTURE IS PLAIN WALL AND NOTHING ELSE - no heads, no hands, no models,
no shelves, no window frame, nothing darker or brighter inside that strip.

TEXTURE AND FINISH - fine even paper grain over the whole image, flat colour areas with a few
loose hand-drawn shading strokes, gentle and warm-hearted, the look of a printed editorial
illustration.

AVOID: neat, straight, evenly spaced teeth; two arches that meet perfectly; dentures, pink
gum acrylic, metal clasps; braces, brackets, wires or aligners; a skull, a skeleton or an
X-ray; a floating hand or a forearm with no body; a giant single tooth; a cartoon tooth with
a face; a calendar, a clock or a progress bar; any before-and-after comparison, panel or
dividing line; a dental chair with someone being treated in it; anyone waving, greeting or
presenting to the viewer; anyone looking at the camera; a faded or ghostly background figure;
readable text or numbers; a crowded room; a warm yellow or sepia cast; cream, beige or tan
walls; pure flat white plaster.
```

---

## 五之四、Ⓔ 的提示詞（描圖線的側顱片，2026-08-24）

⚠⚠ 片子上畫的**不是 X 光照片，是描圖**（tracing）—— 這一版把顱骨整個換成
「透光片上的幾條線」，一來骷髏消失，二來**描圖本身就是矯正專科在做的事**。
⚠ 線要**少而長**（三條 ＋ 一個角），212px 下才不會糊成一團；角度旁邊**不准有數字**。

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, few large objects, one single continuous scene, no panels and no dividing
lines.

THE WHOLE PICTURE IS BRIGHT, CLEAN AND COOL - a corner of a dental clinic by day. The walls
are a very light COOL off-white with a faint blue in them, about #e6ebf0 - never cream, never
beige, never tan, never yellow, and never a neutral dead grey either. DO NOT PUT AN OVERALL
WARM, AMBER OR SEPIA CAST OVER THE IMAGE.

THE MOST IMPORTANT CONTENT OF THIS PICTURE IS THE LIT TRACING ON THE VIEWER: a side view of a
jaw whose front teeth are visibly crowded and tilted, with a few long analysis lines drawn
over it. If a viewer sees this image at thumbnail size and cannot tell that the teeth are
untidy, the illustration has failed. Draw that first, then everything else.

THE SIX THINGS THAT MATTER MOST, IN ORDER:

1. THE LIT VIEWER IS THE HERO AND IT IS BIG. An upright rectangular light box standing on a
   low counter in the MIDDLE of the picture, TALLER THAN IT IS WIDE, its glowing face ABOUT
   SIXTY PER CENT OF THE PICTURE HEIGHT. Its face glows an even cool white (#f4f8fb),
   spilling soft light onto the counter, the wall behind it and both faces, so it is obvious
   that both people are lit by it. ONE SINGLE SHEET is clipped to it - never two, never a
   row, never a before-and-after pair.

2. WHAT IS ON THAT SHEET - THIS IS A LINE TRACING, NOT A PHOTOGRAPH AND NOT AN X-RAY IMAGE.
   Drawn in soft medium-grey hand-drawn lines on the glowing ground, large and simple:
   • THE OUTLINE OF A FACE IN PROFILE, facing left: forehead, nose, lips and chin as ONE
     CONTINUOUS SOFT LINE. NO skull, NO bone, NO eye socket, NO cheekbone, NO teeth roots
     and NO texture - just the profile line.
   • BELOW AND BEHIND IT, THE SIMPLE OUTLINE OF THE UPPER AND LOWER JAW as two smooth
     shapes, one above the other.
   • THE TEETH ARE THE POINT AND THEY ARE DRAWN LARGER THAN LIFE: a row of clearly separated
     teeth along each jaw. AT THE FRONT, THREE TEETH LEAN FORWARD AT AN OBVIOUS ANGLE AND
     TWO OF THEM OVERLAP EACH OTHER, so the front of the row is uneven and wavy; the upper
     front teeth also stick out well in front of the lower ones. NOTHING about this row
     should look neat, straight or corrected.
   • OVER ALL OF THAT, EXACTLY THREE LONG STRAIGHT ANALYSIS LINES IN THIN CLINIC BLUE
     (#4478b5): one running along the upper jaw, one along the lower jaw, and one from the
     bridge of the nose down past the chin - two of them crossing near the nose so that a
     clear ANGLE is visible between them. The angle is marked with ONE small blue arc and
     NOTHING ELSE: no numbers, no letters, no labels, no arrows, no measuring ticks, no grid.
     THESE THREE LINES AND THE ARC ARE THE ONLY BLUE MARKS ON THE SHEET.

3. THE DENTIST IS STANDING TO THE LEFT OF THE VIEWER, DRAWING ONE OF THOSE LINES. A Taiwanese
   man in his late thirties, short black hair, clean-shaven, standing upright, turned
   three-quarters towards the sheet. BOTH OF HIS ARMS ARE FULLY VISIBLE AND BOTH CONNECT TO
   HIS BODY: the near arm is raised to about chest height, elbow bent and close to his body,
   holding a SLIM PENCIL whose tip TOUCHES the sheet at the end of one of the blue lines, as
   if he has just drawn it; the other arm hangs relaxed at his side. HIS EYES ARE ON THE
   SHEET, not on us and not on the patient. He is explaining something quietly, mouth
   slightly open. HE IS NOT WAVING, NOT PRESENTING WITH AN OPEN PALM AND NOT GREETING ANYONE.
   THERE IS NO HAND ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A VISIBLE PERSON. USE
   THIS ANCHOR: standing, the top of his head sits just below the top sixth of the picture
   and his legs are cropped by the bottom edge.

4. THE PATIENT IS SITTING ON A ROUND STOOL TO THE RIGHT, LEANING IN TO LOOK. A Taiwanese man
   or woman of about forty - an ordinary working adult, NOT a teenager and NOT a student. He
   sits on a simple round stool, KNEES TOGETHER AND BODY CLEARLY TIPPED FORWARD FROM THE HIPS
   towards the sheet, forearms on his thighs, a small sheaf of three or four printed A4
   sheets in one hand (grey text-like marks only, NO readable writing). HIS EYES ARE ON THE
   CROWDED FRONT TEETH. HIS EXPRESSION IS "THIS TIME I WANT TO UNDERSTAND IT PROPERLY": mouth
   closed and calm, the inner ends of his eyebrows very slightly drawn together in
   concentration. He is NOT sad, NOT regretful, NOT anxious, NOT smiling broadly, and he is
   NOT touching his own mouth or jaw.

5. A THIRD PERSON IS BUSY WITH SOMETHING ELSE IN THE BACKGROUND, AND SHE IS NOT LOOKING AT
   THEM. Further back on the RIGHT, smaller because she is further away but drawn with
   exactly the same line weight, a dental nurse stands side-on at an open shelving unit,
   BOTH HANDS PLACING A PAIR OF PLASTER JAW MODELS onto a shelf where two or three more pairs
   already sit. She is absorbed in her own task. THREE PEOPLE, TWO SEPARATE THINGS HAPPENING
   AT ONCE, nobody posing for us.

6. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS - not on the sheet, not
   on the papers, not on the wall, not on the shelves.

DRAW THE PEOPLE EXACTLY IN THE STYLE OF THE REFERENCE ILLUSTRATION OF THE DENTIST. Thin
hand-drawn linework whose weight varies and sometimes breaks - NOT a thick even outline. Each
face is ONE FLAT SKIN TONE with no shading, no modelling, no cheekbones and no jaw shadow. On
the face there is only: the outline, two eyes drawn as small simple dots with NO whites, NO
pupils and NO highlights, two short eyebrows, a tiny nose mark, a small mouth line and an ear.
Hair is a flat shape in two tones with no individual strands. EVERY PERSON IS DRAWN WITH
EXACTLY THE SAME LINE WEIGHT AND SOLIDITY, including the one furthest away. The three people
differ clearly in age, build, hair and clothing colour, and no two faces are alike.

COLOUR. The clinic blue of this speciality is #4478b5 and it appears in only three places:
the three analysis lines on the sheet, the dentist's scrubs under his open white coat (a soft
dusty blue, #6a94c4), and one thin blue line along the counter edge. Everything else is
quiet: the patient wears a warm oatmeal shirt (#d8cdbb) over dark slate trousers, the nurse
pale slate blue-grey, the shelving pale wood-grey, the plaster models chalky warm off-white.
The picture must NOT become an all-blue image: it is a bright white room with a few blue
things in it.

THE ROOM - FOUR LARGE SIMPLE OBJECTS AND NOTHING ELSE: the light box on its low counter; the
open shelving unit with plaster models on the right; a tall window with a plain cool sky on
the left, cropped by the picture edge; and the round stool. The floor is a pale seamless
clinic vinyl, slightly cooler than the wall, softly reflective with a few long horizontal
strokes, meeting the wall in a CONTINUOUS COVED SKIRTING. The room is SIMPLE BECAUSE THE
THINGS IN IT ARE FEW AND LARGE, NOT because it is empty: no bare unbroken expanse of wall in
the lower two thirds. NO posters, NO framed pictures, NO plants, NO instrument trays, NO
cabinets of small bottles, NO dental chair.

THE TOP SIXTH OF THE PICTURE IS PLAIN WALL AND NOTHING ELSE - no heads, no hands, no light
box, no shelves, no window frame, nothing darker or brighter inside that strip.

TEXTURE AND FINISH - fine even paper grain over the whole image, flat colour areas with a few
loose hand-drawn shading strokes, gentle and warm-hearted, the look of a printed editorial
illustration.

AVOID: a skull, a skeleton, an eye socket, bone texture or anything that looks like a real
X-ray photograph; neat, straight, evenly spaced teeth; numbers, letters, labels, arrows,
measuring ticks or a grid anywhere on the sheet; more than three blue lines; a second sheet
or any before-and-after comparison; panels or dividing lines; a floating hand or a forearm
with no body; a giant single tooth; a cartoon tooth with a face; a calendar, a clock or a
progress bar; a dental chair with someone being treated in it; anyone waving, greeting or
presenting to the viewer; anyone looking at the camera; a faded or ghostly background figure;
readable text; a crowded room; a warm yellow or sepia cast; cream, beige or tan walls.
```

---

## 五之五、Ⓓ 與 Ⓔ 也被退回，使用者給了新概念（2026-08-24）

> 「感覺都不太行欸　我想到**關鍵報告裡男主角用手在檢查各種檔案**　用那個概念
> 　牙醫師**用手在比對牙齒　測量　角度　位置　排牙齒**　旁邊病患覺得**期待　信任**的樣子」

存檔：`drafts/og-topic-ortho-v3-tracing.jpg`（Ⓔ）、`drafts/og-topic-ortho-v4-models.jpg`（Ⓓ）。

### 這兩版各自錯在哪（下一版要擋的）

- **Ⓔ 描圖**：專科感確實出來了（描圖線、鉛筆），但**牙齒還是整整齊齊一排**，
  而且側臉輪廓被畫成一條抖動的怪線、三條藍線沒有交會成可讀的角。
  ⚠ 「牙齒要亂」我寫在最前面**還是沒有生效** —— 成因是那排牙**太小**
  （在片子上只佔片子的五分之一），模型會優先把小東西畫成「一般的樣子」。
  **要讓它畫亂，那個東西就得是畫面上最大的東西。**
- **Ⓓ 模型**：⚠⚠ 兩副模型並排 ＝ **被讀成 before/after**（左邊那副咬得整整齊齊、
  右邊那副散開），這正好是紅線；石膏塊巨大笨重像兩塊磚，醫師被擋成半身。
  ⚠ 「上下咬不對」寫了三條幾何，模型只做到「上下分開」，沒有做到「排列亂」。

> **這一輪學到的一句**：**「亂」是畫面裡最大的那個東西才畫得出來的。**
> 前四版把它交給片子上的一小排牙、模型上的一小排牙，四次都失敗。

### 新的梗（使用者給的）：他用手在空中把牙齒排好

⚠⚠ **取的是「動作與掌控感」，不是電影的科幻介面。** 站上的語彙是溫暖的手繪平塗，
所以：**沒有發光格線、沒有 HUD、沒有粒子、沒有手套、沒有藍色科幻光**，
牙齒就是站上那種白牙，**單純浮在他面前的空氣裡**。
⚠ 那張電影截圖**不餵進生成** —— 我們只取動作，餵進去會連藍色介面與版權畫面一起抄。

三件事因此一次解決：
・**亂看得見** —— 浮空的牙弓是畫面上最大的東西（畫面高的一半），幾顆明顯離線。
・**專科水準** —— 他在**量**（一條弧形參考線與一個角）、在**排**（指間捏著一顆正要放回去）。
・**期待與信任** —— 病人抬頭看著那排牙，這是七科裡唯一「往前看」的表情。

⚠ **不是 before/after**：畫面上只有**一排**牙齒與**一條參考線**，
有幾顆已經貼上線、有幾顆還離得遠 —— 那是**正在進行中**，不是兩個時間點的對照。

## 五之六、Ⓖ 的提示詞（隔空把牙齒排好，2026-08-24）

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, few large objects, one single continuous scene, no panels and no dividing
lines.

THE WHOLE PICTURE IS BRIGHT, CLEAN AND COOL - a corner of a dental clinic by day, daylight
from a tall window. The walls are a very light COOL off-white with a faint blue in them,
about #e6ebf0 - never cream, never beige, never tan, never yellow, and never a neutral dead
grey either. DO NOT PUT AN OVERALL WARM, AMBER OR SEPIA CAST OVER THE IMAGE. THIS IS A WARM,
HAND-DRAWN EDITORIAL ILLUSTRATION, NOT A SCIENCE-FICTION IMAGE: there is no glowing
interface, no grid, no hologram panel, no particles and no futuristic technology anywhere.

THE MOST IMPORTANT CONTENT OF THIS PICTURE IS A ROW OF LARGE TEETH FLOATING IN THE AIR, SOME
OF THEM CLEARLY OUT OF LINE, WITH THE DENTIST ARRANGING THEM WITH HIS BARE HANDS. If a viewer
sees this image at thumbnail size and cannot tell that some teeth are crooked and that the
man is putting them into place, the illustration has failed. Draw that first, then everything
else.

THE SIX THINGS THAT MATTER MOST, IN ORDER:

1. THE FLOATING ROW OF TEETH IS THE HERO AND IT IS HUGE. Filling the MIDDLE of the picture,
   ABOUT HALF THE PICTURE HEIGHT and about two fifths of its width: TWELVE INDIVIDUAL TEETH
   hanging in the air, each one a simple rounded tooth shape with a short root, drawn in warm
   off-white (#efe9df, shaded with #ddd5c6) with loose hand-drawn shading strokes - never
   pure white, never flat. THEY HAVE NO FACES, NO EYES AND NO ARMS - they are teeth, not
   characters. They are all roughly the same size, each one big enough to read on its own,
   and they simply float in the air in front of the dentist with a little space between them,
   casting a soft shadow on the counter below. NO glass, NO screen, NO frame, NO glow, NO
   grid, NO sparkles around them.

2. HOW THE ROW IS ARRANGED - THIS IS WHERE THE WHOLE MEANING IS. A single THIN CURVED BLUE
   GUIDE LINE (#4478b5) sweeps across the air from left to right, the shape of a dental arch:
   that line is where the teeth are supposed to sit.
   • ABOUT SEVEN OF THE TEETH ALREADY SIT NEATLY ON THAT CURVE, evenly spaced and upright.
   • THE OTHER FIVE ARE PLAINLY OUT OF PLACE: one is tipped over at a steep angle, one is
     rotated so we see its side, two are drifting well ABOVE and BELOW the curve, and one has
     drifted forward towards us, overlapping its neighbour. The difference between the tidy
     ones and the stray ones must be obvious at thumbnail size.
   • Near the left end, TWO SHORT STRAIGHT BLUE LINES cross to make ONE CLEAR ANGLE, marked
     with a single small blue arc - a measurement being taken. THAT ARC, THE TWO SHORT LINES
     AND THE CURVE ARE THE ONLY BLUE MARKS IN THE AIR. No numbers, no letters, no labels, no
     arrows, no ticks, no dotted rulers, no floating panels.

3. THE DENTIST IS WORKING WITH HIS TWO BARE HANDS IN MID-AIR. A Taiwanese man in his late
   thirties, short black hair, clean-shaven, standing on the LEFT, turned three-quarters
   towards the floating row, feet planted and shoulders level. BOTH ARMS ARE RAISED IN FRONT
   OF HIS CHEST, ELBOWS BENT, BOTH FULLY VISIBLE AND BOTH CONNECTED TO HIS BODY:
   • HIS NEAR HAND PINCHES ONE STRAY TOOTH between thumb and index finger and is lifting it
     back towards the curve - the tooth is caught in mid-move, a hand's width from its place.
   • HIS OTHER HAND IS OPEN, palm towards the row, fingers relaxed and slightly spread,
     hovering beside the teeth as if holding the rest of the row steady.
   HIS EYES ARE ON THE TOOTH BETWEEN HIS FINGERS, not on us and not on the patient. His mouth
   is slightly open, explaining quietly as he works. He is calm, absorbed and completely in
   control - a craftsman at work, NOT a magician, NOT a presenter, NOT a performer. HIS HANDS
   ARE BARE: no gloves, no wires, no gadgets, nothing on his fingertips. THERE IS NO HAND
   ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A VISIBLE PERSON. USE THIS ANCHOR:
   standing, the top of his head sits just below the top sixth of the picture and his legs
   are cropped by the bottom edge.

4. THE PATIENT IS SITTING ON A ROUND STOOL ON THE RIGHT, LOOKING UP AT THE TEETH, AND HE IS
   LOOKING FORWARD TO THIS. A Taiwanese man or woman of about forty - an ordinary working
   adult, NOT a teenager and NOT a student. He sits on a simple round stool, body tipped
   slightly forward, hands resting easily on his knees with a small sheaf of three or four
   printed A4 sheets under one hand (grey text-like marks only, NO readable writing). HIS
   CHIN IS LIFTED AND HIS EYES ARE ON THE FLOATING TEETH.
   HIS EXPRESSION IS HOPEFUL AND TRUSTING - EYEBROWS LIFTED A LITTLE AT THE INNER ENDS, EYES
   OPEN AND SOFT, THE BEGINNING OF A SMALL CLOSED SMILE, SHOULDERS DOWN AND RELAXED. He looks
   like someone who can finally see how this is going to work. He is NOT anxious, NOT
   frowning, NOT sad, NOT astonished, NOT laughing, and he is NOT touching his own mouth.

5. A THIRD PERSON IS BUSY WITH SOMETHING ELSE IN THE BACKGROUND, AND SHE IS NOT LOOKING AT
   THEM. Far back on the RIGHT, small because she is distant but drawn with exactly the same
   line weight, a dental nurse stands side-on at an open shelving unit, BOTH HANDS PLACING A
   PAIR OF PLASTER JAW MODELS onto a shelf where two or three more pairs already sit. She is
   absorbed in her own task. THREE PEOPLE, TWO SEPARATE THINGS HAPPENING AT ONCE, nobody
   posing for us.

6. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

DRAW THE PEOPLE EXACTLY IN THE STYLE OF THE REFERENCE ILLUSTRATION OF THE DENTIST. Thin
hand-drawn linework whose weight varies and sometimes breaks - NOT a thick even outline. Each
face is ONE FLAT SKIN TONE with no shading, no modelling, no cheekbones and no jaw shadow. On
the face there is only: the outline, two eyes drawn as small simple dots with NO whites, NO
pupils and NO highlights, two short eyebrows, a tiny nose mark, a small mouth line and an ear.
Hair is a flat shape in two tones with no individual strands. EVERY PERSON IS DRAWN WITH
EXACTLY THE SAME LINE WEIGHT AND SOLIDITY, including the one furthest away. The three people
differ clearly in age, build, hair and clothing colour, and no two faces are alike.

COLOUR. The clinic blue of this speciality is #4478b5 and it appears in only four places: the
curved guide line and the small angle in the air, the dentist's scrubs under his open white
coat (a soft dusty blue, #6a94c4), the cool sky in the window, and one thin blue line along
the counter edge. Everything else is quiet: the patient wears a warm oatmeal shirt (#d8cdbb)
over dark slate trousers, the nurse pale slate blue-grey, the shelving pale wood-grey. The
picture must NOT become an all-blue image and it must NOT look lit by blue light: it is a
bright white room with a few blue things in it.

THE ROOM - FOUR LARGE SIMPLE OBJECTS AND NOTHING ELSE: a TALL WINDOW on the left with a plain
cool sky beyond it; a long clean counter running behind the floating teeth at about two
thirds of the picture height; an OPEN SHELVING UNIT with plaster models far back on the
right; and the round stool. The floor is a pale seamless clinic vinyl, slightly cooler than
the wall, softly reflective with a few long horizontal strokes, meeting the wall in a
CONTINUOUS COVED SKIRTING. The room is SIMPLE BECAUSE THE THINGS IN IT ARE FEW AND LARGE, NOT
because it is empty: no bare unbroken expanse of wall in the lower two thirds. NO posters, NO
framed pictures, NO plants, NO instrument trays, NO cabinets of small bottles, NO dental
chair, NO monitors and NO screens.

THE TOP SIXTH OF THE PICTURE IS PLAIN WALL AND NOTHING ELSE - no heads, no hands, NO FLOATING
TEETH, no guide line, no shelves, no window frame, nothing darker or brighter inside that
strip. The whole floating row stays below that strip.

TEXTURE AND FINISH - fine even paper grain over the whole image, flat colour areas with a few
loose hand-drawn shading strokes, gentle and warm-hearted, the look of a printed editorial
illustration.

AVOID: holograms, glowing screens, glass panels, HUDs, grids, wireframes, scan lines, data
readouts, particles, sparkles, light beams, gloves, wires or any futuristic technology; a
blue glow over the room; teeth with faces, eyes or limbs; a whole row of perfectly tidy
teeth; a second row of teeth or any before-and-after comparison; panels or dividing lines;
numbers, letters, labels, arrows or measuring ticks; a floating hand or a forearm with no
body; a skull, a skeleton or an X-ray; a dental chair with someone being treated in it;
anyone waving, greeting or presenting to the viewer; anyone looking at the camera; a faded or
ghostly background figure; readable text; a crowded room; a warm yellow or sepia cast; cream,
beige or tan walls; pure flat white teeth.
```

⚠ **這一版最可能出事的三件**（回圖先看這三個）：
① **牙齒飄進頂 17%** —— 浮空的東西最容易往上跑，提示詞寫了兩次，仍要量。
② **變成科幻** —— 只要出現發光介面或藍光染整張，就是品牌那一關過不了，要重生成。
③ **十二顆牙被畫成一整排整齊的** —— 前四版都栽在這裡；驗收時先數「明顯離線的有幾顆」，
   少於三顆就重生成。

---

## 五之七、Ⓖ v1 的量測與使用者的四件（2026-08-24）

存檔 `drafts/og-topic-ortho-v5-air.jpg`。**梗過了**（牙齒夠大、有歪的、有弧線與角度），
使用者要改四件：

> 「首先　**病患不要再拿著資料了**　圖片上看不懂什麼意思　再來　**醫師要帶主題色手套**
> 　**手指的地方可以像電影那樣發光**　**排列牙齒可以用箭頭標示方向　或是移動**
> 　讓牙齒移動看起來比較動態感」

⚠⚠ **後三件推翻了我 v1 寫的三條 AVOID**（no gloves／no glow／no arrows），
其中「不放箭頭」是 ILLUSTRATION.md 第十一節硬規格 6 的一半 ——
**這是使用者第二次推翻那一條**（第一次是顯微根管的放大圈，2026-08-24 同日）。
**推翻同樣是有條件的**，條件寫在 v2 提示詞裡：
・箭頭**只有三支、而且大**（每支至少兩顆牙寬），不准有小箭頭群、虛線、刻度；
・發光**只在指尖**，不是整隻手、沒有光束、沒有粒子、臉上不打光；
・手套是**實心的主題色**，而**刷手服讓開改成淺灰** —— 兩塊藍在 212px 下會糊在一起，
  手套是這張圖眼睛要跟著走的地方，必須是畫面上最飽和的那一塊藍。

### 量測（`node drafts/og-measure-band.mjs <圖> "#4478b5"`，這一輪新寫的通用腳本）

| | v5 | 已上線四張的**原圖** |
| --- | --- | --- |
| 邊緣密度 | **16.6%** ❌ | general 32.4／perio 21.9／endo 22.1／kids 25.8 |
| 頂 17% 的 B 通道中位 | 244 ✅（這一科要 ≥ 212） | 172／202／246／214 |
| 安靜區被佔 | 4.59%（醫師頭髮與左上窗框） | 30.9／1.5／8.4／2.6 |
| 無彩空白 | 0.3% ✅ | — |

⚠⚠ **密度是硬傷，比已上線最空的那一張還空 24%** —— 成因是大片白牆＋白地板，
和兒牙第一版（21.4%）同一個病：把「背景簡單」做成了「畫面空」。
v2 因此補了四樣**大**東西：窗（含窗格與窗外屋頂輪廓）、檯面的抽屜線、
三層的模型架、地板的長反光線。**不是加小雜物。**

⚠ `assets/og-topic-*.jpg` 是**疊過帶子的成品**，量密度與頂 17% 要用
`drafts/og-topic-*-src.jpg`（帶子本身有字，會把密度墊高、把頂端的藍壓掉）。

## 五之八、Ⓖ v2 的提示詞（手套＋指尖發光＋箭頭＋補密度，2026-08-24）

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, few large objects, one single continuous scene, no panels and no dividing
lines.

THE WHOLE PICTURE IS BRIGHT, CLEAN AND COOL - a corner of a dental clinic by day, daylight
from a tall window. The walls are a very light COOL off-white with a faint blue in them,
about #e6ebf0 - never cream, never beige, never tan, never yellow, and never a neutral dead
grey either. THIS IS A WARM, HAND-DRAWN EDITORIAL ILLUSTRATION, NOT A SCIENCE-FICTION IMAGE:
apart from one small glow at the dentist's fingertips there is no glowing interface, no
screen, no grid, no hologram panel and no particles anywhere.

THE MOST IMPORTANT CONTENT OF THIS PICTURE IS A ROW OF LARGE TEETH FLOATING IN THE AIR, SOME
OF THEM CLEARLY OUT OF LINE AND VISIBLY BEING MOVED INTO PLACE BY THE DENTIST'S GLOVED HANDS.
If a viewer sees this image at thumbnail size and cannot tell that some teeth are crooked and
that they are on the move, the illustration has failed. Draw that first, then everything else.

THE SEVEN THINGS THAT MATTER MOST, IN ORDER:

1. THE FLOATING ROW OF TEETH IS THE HERO AND IT IS HUGE. Filling the MIDDLE of the picture,
   ABOUT HALF THE PICTURE HEIGHT and about two fifths of its width: TWELVE INDIVIDUAL TEETH
   hanging in the air, each one a simple rounded tooth shape with a short root, drawn in warm
   off-white (#efe9df, shaded with #ddd5c6) with loose hand-drawn shading strokes - never
   pure white, never flat. THEY HAVE NO FACES, NO EYES AND NO ARMS - they are teeth, not
   characters. Each one is big enough to read on its own, and they float in the air in front
   of the dentist with a little space between them, casting soft shadows on the counter below.

2. HOW THE ROW IS ARRANGED - THIS IS WHERE THE WHOLE MEANING IS. A single THIN CURVED BLUE
   GUIDE LINE (#4478b5) sweeps across the air from left to right, the shape of a dental arch:
   that line is where the teeth are supposed to sit.
   • ABOUT SEVEN OF THE TEETH ALREADY SIT NEATLY ON THAT CURVE, evenly spaced and upright.
   • THE OTHER FIVE ARE PLAINLY OUT OF PLACE: one is tipped over at a steep angle, one is
     rotated so we see its side, two are drifting well ABOVE and BELOW the curve, and one has
     drifted forward towards us, overlapping its neighbour.
   • Near the left end, TWO SHORT STRAIGHT BLUE LINES cross to make ONE CLEAR ANGLE, marked
     with a single small blue arc - a measurement being taken.

3. THE TEETH ARE ON THE MOVE, AND THE MOVEMENT IS SHOWN WITH EXACTLY THREE BIG ARROWS.
   THREE - not four, not a dozen. Each arrow is a THICK, SIMPLE, SLIGHTLY CURVED BLUE ARROW
   (#4478b5) with ONE clean triangular head, AT LEAST TWICE AS LONG AS A TOOTH IS WIDE, so
   that each one is unmistakable at thumbnail size. Each arrow STARTS BESIDE ONE STRAY TOOTH
   AND POINTS TO THE EMPTY GAP ON THE CURVE WHERE THAT TOOTH BELONGS: one arrow curves
   downward, one sweeps sideways along the curve, one rotates around a tooth to show it
   turning. Behind the two teeth that are travelling, add THREE OR FOUR SHORT HAND-DRAWN
   MOTION STROKES trailing after them, and tilt those teeth slightly along their direction of
   travel so they read as moving, not parked. NO other arrows anywhere: no small arrows, no
   dotted paths, no arrowheads on the guide line, no ruler ticks, no repeated ghost copies of
   a tooth.

4. THE DENTIST IS WORKING WITH HIS TWO GLOVED HANDS IN MID-AIR. A Taiwanese man in his late
   thirties, short black hair, clean-shaven, standing on the LEFT, turned three-quarters
   towards the floating row, feet planted and shoulders level. HE WEARS CLOSE-FITTING CLINIC
   GLOVES IN SOLID BLUE (#4478b5) with a soft rolled cuff at each wrist - the gloves are the
   most saturated blue in the whole picture and they are plain, with no pattern, no wires, no
   sensors and no lights on them. BOTH ARMS ARE RAISED IN FRONT OF HIS CHEST, ELBOWS BENT,
   BOTH FULLY VISIBLE AND BOTH CONNECTED TO HIS BODY:
   • HIS NEAR HAND PINCHES ONE STRAY TOOTH between thumb and index finger and is lifting it
     back towards the curve - THE FINGERTIPS ACTUALLY TOUCH THE TOOTH, and that tooth is
     caught in mid-move, a hand's width from its place.
   • HIS OTHER HAND IS OPEN, palm towards the row, fingers relaxed and slightly spread,
     hovering beside the teeth as if holding the rest of the row steady.
   AT HIS FINGERTIPS THERE IS A SMALL SOFT GLOW: a warm-white halo (#fdf8ec, fading out) no
   bigger than a fingertip around the thumb and index finger that hold the tooth, and a
   fainter one at the tips of the open hand. THE GLOW IS ONLY AT THE FINGERTIPS - no beams,
   no rays, no rings, no sparkles, no particles, the gloves themselves do not glow, and there
   is no light on his face or on the wall.
   HIS EYES ARE ON THE TOOTH BETWEEN HIS FINGERS, not on us and not on the patient. His mouth
   is slightly open, explaining quietly as he works. He is calm, absorbed and completely in
   control - a craftsman at work, NOT a magician, NOT a presenter, NOT a performer. THERE IS
   NO HAND ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A VISIBLE PERSON.

5. THE PATIENT IS SITTING ON A ROUND STOOL ON THE RIGHT, LOOKING UP AT THE TEETH, AND HE IS
   LOOKING FORWARD TO THIS. A Taiwanese man or woman of about forty - an ordinary working
   adult, NOT a teenager and NOT a student. He sits on a simple round stool, body tipped
   slightly forward, ONE HAND RESTING ON HIS KNEE AND THE OTHER LOOSELY HOLDING THE EDGE OF
   THE STOOL. HIS HANDS ARE EMPTY: no papers, no folder, no phone, no clipboard, nothing at
   all in his hands or on his lap. HIS CHIN IS LIFTED AND HIS EYES ARE ON THE FLOATING TEETH.
   HIS EXPRESSION IS HOPEFUL AND TRUSTING - eyebrows lifted a little at the inner ends, eyes
   open and soft, the beginning of a small closed smile, shoulders down and relaxed. He looks
   like someone who can finally see how this is going to work. He is NOT anxious, NOT
   frowning, NOT sad, NOT astonished, NOT laughing, and he is NOT touching his own mouth.

6. A THIRD PERSON IS BUSY WITH SOMETHING ELSE IN THE BACKGROUND, AND SHE IS NOT LOOKING AT
   THEM. Far back on the RIGHT, small because she is distant but drawn with exactly the same
   line weight, a dental nurse stands side-on at an open shelving unit, BOTH HANDS PLACING A
   PAIR OF PLASTER JAW MODELS onto a shelf. She is absorbed in her own task.

7. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

DRAW THE PEOPLE EXACTLY IN THE STYLE OF THE REFERENCE ILLUSTRATION OF THE DENTIST. Thin
hand-drawn linework whose weight varies and sometimes breaks - NOT a thick even outline. Each
face is ONE FLAT SKIN TONE with no shading, no modelling, no cheekbones and no jaw shadow. On
the face there is only: the outline, two eyes drawn as small simple dots with NO whites, NO
pupils and NO highlights, two short eyebrows, a tiny nose mark, a small mouth line and an ear.
Hair is a flat shape in two tones with no individual strands. EVERY PERSON IS DRAWN WITH
EXACTLY THE SAME LINE WEIGHT AND SOLIDITY, including the one furthest away. The three people
differ clearly in age, build, hair and clothing colour, and no two faces are alike.

COLOUR. The clinic blue of this speciality is #4478b5. IT IS STRONGEST ON THE GLOVES, and it
also appears in the curved guide line, the three arrows and the small angle in the air, and
in one thin line along the counter edge. THE DENTIST'S SCRUBS UNDER HIS OPEN WHITE COAT ARE
PALE WARM GREY (#cfd0cc), NOT BLUE - the gloves must stand out against them. Everything else
is quiet: the patient wears a warm oatmeal shirt (#d8cdbb) over dark slate trousers, the
nurse pale slate blue-grey, the shelving pale wood-grey. The picture must NOT become an
all-blue image and must NOT look lit by blue light: it is a bright white room with a few blue
things in it.

THE ROOM MUST NOT LOOK EMPTY. Use FEW BUT LARGE objects, each one drawn with clear edges:
• A TALL WINDOW filling the left edge, with THREE simple frame bars across it, and beyond it
  the pale flat outlines of two or three low neighbouring rooftops and one tree - drawn as
  simple silhouettes in cool pale grey-blue, no detail.
• A LONG COUNTER crossing the picture behind the floating teeth at about two thirds of the
  picture height, with TWO large drawer lines along its front and a thin blue edge line.
• AN OPEN SHELVING UNIT far back on the right with THREE shelves, each holding a pair of
  chalky plaster jaw models.
• THE FLOOR is pale seamless clinic vinyl, slightly cooler than the wall, with SIX OR SEVEN
  LONG HORIZONTAL REFLECTION STROKES across it, meeting the wall in a CONTINUOUS COVED
  SKIRTING that curves up the wall.
NO posters, NO framed pictures, NO plants, NO instrument trays, NO cabinets of small bottles,
NO dental chair, NO monitors and NO screens - the room is furnished by those four large
things only.

THE TOP SIXTH OF THE PICTURE IS PLAIN WALL AND NOTHING ELSE - the whole floating row, the
guide line, every arrow, BOTH PEOPLE'S HEADS, the window frame and the shelving unit ALL STAY
BELOW THAT STRIP. Nothing darker or brighter anywhere inside it.

TEXTURE AND FINISH - fine even paper grain over the whole image, flat colour areas with a few
loose hand-drawn shading strokes, gentle and warm-hearted, the look of a printed editorial
illustration.

AVOID: holograms, glowing screens, glass panels, HUDs, grids, wireframes, scan lines, data
readouts, particles, sparkles, light beams, glowing gloves, wires or sensors on the hands; a
blue glow over the room or on faces; teeth with faces, eyes or limbs; a whole row of
perfectly tidy teeth; more than three arrows; small arrows, dotted paths or ruler ticks;
ghost copies of a tooth; anything in the patient's hands; a second row of teeth or any
before-and-after comparison; panels or dividing lines; numbers, letters or labels; a floating
hand or a forearm with no body; a skull, a skeleton or an X-ray; a dental chair with someone
being treated in it; anyone waving, greeting or presenting to the viewer; anyone looking at
the camera; a faded or ghostly background figure; readable text; a bare empty room; a warm
yellow or sepia cast; cream, beige or tan walls; pure flat white teeth.
```

⚠ 回圖要量的四件（`node drafts/og-measure-band.mjs <圖> "#4478b5"`）：
① **邊緣密度 ≥ 22%**（＝已上線四張的下限；v1 只有 16.6）
② 頂 17% 的 B 通道 ≥ 212、安靜區被佔 < 5%（v1 的頭髮與窗框要退出去）
③ 箭頭是不是恰好三支、而且夠大
④ 有沒有變成科幻（發光只准在指尖）

---

## 五之九、Ⓖ v2 的量測 ＋ 使用者放掉「不要科技感」那條限制（2026-08-24）

存檔 `drafts/og-topic-ortho-v6-arrows.jpg`。四件都做到了（手套、指尖發光、三支大箭頭、
病人手上淨空），但使用者：

> 「這樣稍微有感覺了　但**還是差有點遠**　我覺得我們**不要這個嚴格限制不要高科技感**
> 　我想像這個醫師戴著手套　指尖發光　**操控浮在空中的多張X光片或觸控螢幕**
> 　螢幕上有許多**牙弓或牙齒**　讓醫師隨需要**操控方向或提取牙齒**」

⚠⚠⚠ **這是使用者第三次推翻規格**（第一次放大圈、第二次箭頭、這次是整個
「不要科幻」的方向）。**照做，不要再擋。**

### 量測：補「大東西」失敗了，而且失敗得很有道理

| | v5 | v6 | 已上線四張的原圖 |
| --- | --- | --- | --- |
| 邊緣密度 | 16.6% | **17.7%** ❌ | 21.9 ~ 32.4% |
| 無彩空白 | 0.3% | **4.5%** ⚠（門檻 5） | — |
| 頂 17% B 通道 | 244 ✅ | 248 ✅ | 172 ~ 246 |
| 安靜區被佔 | 4.59% | 3.2% | 1.5 ~ 8.4% |

⚠⚠ **窗、抽屜線、三層架子、地板反光全部補上去，密度只動了 1.1 個百分點。**
成因是**它們都是「亮底上的淡線」** —— 這一站的邊緣密度是靠**明暗落差**算的，
淡藍線畫在白牆上落差不到 8 階，等於沒畫。
> **記起來：畫面太空的時候，加細節沒有用，要加「深色的塊」。**
> 這也是為什麼 general（32.4%）最高 —— 那張有深色的屋簷、招牌與騎樓。

**所以使用者要的深色發光面板，剛好是這張圖現在最缺的東西**：
科技感、密度、212px 的可讀性（深底亮線的對比遠強過米白牙齒配白牆），一次解決三件。

### ⚠ 這一版要守住的界線（品牌那一關）

**畫風不變**：手繪線、平塗、紙紋、同一套人物畫法（點眼睛、無眼白、平塗臉）。
**變的只有內容。** 七張分享卡放在一起仍然是同一個世界，只有這一科在做未來感的事。
⚠ **房間仍然是明亮的白** —— 不要為了科技感把整間壓暗：頂 17% 的藍通道要 ≥ 212，
壓暗的話帶子就追不上站上那顆藍標籤（會被迫落深階）。
**深色只給面板，光只給面板的線與指尖。**

## 五之十、Ⓗ 的提示詞（浮空的發光面板，2026-08-24）

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, few large objects, one single continuous scene, no dividing lines across
the picture.

STYLE FIRST, AND IT DOES NOT CHANGE: this is a warm, HAND-DRAWN EDITORIAL ILLUSTRATION with
thin uneven ink lines, flat colour and a fine paper grain - the same hand that drew the
reference illustration of the dentist. What is NEW here is only the SUBJECT: the dentist is
working on glowing floating screens. Draw a futuristic idea in a hand-drawn book style. It is
NOT a 3D render, NOT a photo, NOT a neon cyberpunk poster and NOT a video-game interface.

THE ROOM IS A BRIGHT WHITE DENTAL CLINIC BY DAY. Walls a very light COOL off-white with a
faint blue in them, about #e6ebf0; a pale clinic floor. THE ROOM STAYS BRIGHT - do not darken
it, do not turn the lights off, do not flood it with blue light. ALL THE DARKNESS AND ALL THE
GLOW IN THIS PICTURE BELONG TO THE FLOATING SCREENS AND TO HIS FINGERTIPS, nowhere else.

THE MOST IMPORTANT CONTENT: A BIG DARK-BLUE GLOWING SCREEN FLOATING IN MID-AIR WITH A DENTAL
ARCH ON IT, SOME OF ITS TEETH CLEARLY OUT OF LINE, AND THE DENTIST PULLING ONE REAL TOOTH OUT
OF IT WITH HIS GLOVED FINGERS. If a viewer sees this at thumbnail size and cannot tell that
he is handling teeth on floating screens, the illustration has failed.

THE SEVEN THINGS THAT MATTER MOST, IN ORDER:

1. THE MAIN FLOATING SCREEN IS THE HERO AND IT IS BIG AND DARK. A single rectangular panel of
   dark translucent blue glass (#2b4a72, the wall faintly visible through it), floating
   upright in the MIDDLE of the picture, ABOUT FIFTY-FIVE PER CENT OF THE PICTURE HEIGHT,
   turned very slightly towards us. Its edge is ONE thin bright cyan-white line (#bcdcff) and
   its corners are softly rounded; a gentle halo of light spreads a little way into the air
   around it. THE DARK PANEL AGAINST THE WHITE ROOM IS THE STRONGEST CONTRAST IN THE PICTURE.

2. WHAT IS ON THE MAIN SCREEN. Drawn as GLOWING PALE CYAN-WHITE LINE ART (#bcdcff) on the
   dark glass, large and simple: A DENTAL ARCH SEEN FROM ABOVE - a horseshoe of TWELVE teeth,
   each tooth a clear simple shape, big enough to count. FOUR OF THOSE TEETH ARE PLAINLY
   WRONG: one rotated sideways, one tipped over, one pushed out of the horseshoe towards the
   middle, and ONE MISSING FROM THE ROW, leaving an obvious gap. TWO THICK SHORT CURVED
   ARROWS in the same glowing cyan show which way two of the crooked teeth must travel.
   Nothing else on the panel: no numbers, no letters, no charts, no menus, no buttons, no
   sliders, no grid, no scan lines, no tiny icons.

3. HE IS PULLING A REAL TOOTH OUT OF THE SCREEN - THIS IS THE MOMENT OF THE PICTURE. Between
   the thumb and index finger of his near hand, held OUTSIDE the panel and slightly in front
   of it, there is ONE TOOTH THAT HAS BECOME SOLID AND REAL: warm off-white (#efe9df, shaded
   with #ddd5c6) with hand-drawn shading strokes, TWICE THE SIZE of the teeth drawn on the
   screen, and it is the ONLY solid tooth in the air. A SINGLE SOFT TRAIL OF LIGHT connects
   it back to the empty gap in the arch on the panel - one trail, not many. It reads
   instantly as "he has lifted that tooth out of the picture to look at it".

4. THE DENTIST IS OPERATING THE SCREENS WITH BOTH GLOVED HANDS. A Taiwanese man in his late
   thirties, short black hair, clean-shaven, standing on the LEFT, turned three-quarters
   towards the panels, feet planted, shoulders level, calm and absorbed - a specialist at
   work, NOT a magician and NOT a performer. HE WEARS CLOSE-FITTING CLINIC GLOVES IN SOLID
   BLUE (#4478b5) with a soft rolled cuff - plain gloves, no pattern, no wires, no sensors,
   no lights built into them. BOTH ARMS ARE RAISED IN FRONT OF HIS CHEST, ELBOWS BENT, BOTH
   FULLY VISIBLE AND BOTH CONNECTED TO HIS BODY: the near hand pinches the solid tooth (point
   3); THE OTHER HAND IS OPEN WITH FINGERS SPREAD, palm towards the main panel and a little
   away from it, in the middle of TURNING the panel - the panel is tilted to follow his hand.
   AT HIS FINGERTIPS THERE IS A SMALL BRIGHT GLOW: a soft cyan-white halo (#dff0ff) no bigger
   than a fingertip at the tips of both hands, brightest where his fingers meet the tooth.
   THE GLOW IS ONLY AT THE FINGERTIPS AND ON THE PANELS - no beams across the room, no rays
   from his eyes, no sparkles, no floating dust, no light on the walls or the floor, and his
   face is lit normally by daylight.
   HIS EYES ARE ON THE SOLID TOOTH IN HIS FINGERS. His mouth is slightly open, explaining
   quietly as he works. THERE IS NO HAND ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A
   VISIBLE PERSON.

5. TWO SMALLER SCREENS FLOAT BEHIND AND BESIDE THE MAIN ONE, CLEARLY SMALLER SO THE PICTURE
   NEVER SPLITS INTO EQUAL PANELS. Each is about FORTY PER CENT of the main panel's size,
   the same dark blue glass with a thin glowing edge, tilted at a slight angle and partly
   overlapped by the main panel:
   • ONE, up and to the left, carries the GLOWING LINE PROFILE OF A FACE seen from the side -
     forehead, nose, lips, chin, and the two jaws with a row of teeth. NO skull, no bone, no
     eye socket.
   • ONE, lower and to the right, carries A SECOND DENTAL ARCH seen from a different angle,
     smaller and simpler.
   They are quiet background information: fewer lines, no arrows, nothing written on them.

6. THE PATIENT IS SITTING ON A ROUND STOOL ON THE RIGHT, LOOKING UP AT THE SCREENS, AND HE IS
   LOOKING FORWARD TO THIS. A Taiwanese man or woman of about forty - an ordinary working
   adult, NOT a teenager and NOT a student. He sits on a simple round stool, body tipped
   slightly forward, ONE HAND ON HIS KNEE AND THE OTHER LOOSELY ON THE EDGE OF THE STOOL; HIS
   HANDS ARE COMPLETELY EMPTY - no papers, no folder, no phone. HIS CHIN IS LIFTED AND HIS
   EYES ARE ON THE MAIN SCREEN. HIS EXPRESSION IS HOPEFUL AND TRUSTING - eyebrows lifted a
   little at the inner ends, eyes open and soft, the beginning of a small closed smile,
   shoulders down and relaxed. He looks like someone who can finally see how this is going to
   work. He is NOT anxious, NOT frowning, NOT astonished, NOT laughing, NOT touching his own
   mouth, and the glow does not turn his face blue.

7. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

DRAW THE PEOPLE EXACTLY IN THE STYLE OF THE REFERENCE ILLUSTRATION OF THE DENTIST. Thin
hand-drawn linework whose weight varies and sometimes breaks - NOT a thick even outline. Each
face is ONE FLAT SKIN TONE with no shading, no modelling, no cheekbones and no jaw shadow. On
the face there is only: the outline, two eyes drawn as small simple dots with NO whites, NO
pupils and NO highlights, two short eyebrows, a tiny nose mark, a small mouth line and an ear.
Hair is a flat shape in two tones with no individual strands. EVERY PERSON IS DRAWN WITH
EXACTLY THE SAME LINE WEIGHT AND SOLIDITY, including the one furthest away. The three people
differ clearly in age, build, hair and clothing colour, and no two faces are alike.

A THIRD PERSON IS BUSY WITH SOMETHING ELSE: far back on the RIGHT, small because she is
distant but drawn with the same line weight, a dental nurse stands side-on at an open
shelving unit, both hands placing a pair of chalky plaster jaw models onto a shelf. She is
absorbed in her own task and does not look at the screens.

COLOUR. The speciality blue is #4478b5: it is strongest on THE GLOVES, and it also gives the
floating panels their dark blue and the counter edge its thin line. The glowing line art on
the panels is a pale cyan-white (#bcdcff). THE DENTIST'S SCRUBS UNDER HIS OPEN WHITE COAT ARE
PALE WARM GREY (#cfd0cc), NOT BLUE, so the gloves stand out. The patient wears a warm oatmeal
shirt (#d8cdbb) over dark slate trousers; the nurse pale slate blue-grey; the shelving pale
wood-grey. The room itself keeps its warm daylight: the picture is a BRIGHT WHITE ROOM WITH
DARK GLOWING PANELS IN IT, not a dark blue picture.

THE ROOM - FOUR LARGE SIMPLE OBJECTS AND NOTHING ELSE: a TALL WINDOW filling the left edge
with three simple frame bars and the pale flat silhouettes of two low rooftops beyond it; a
LONG COUNTER crossing behind the panels at about two thirds of the picture height, with two
large drawer lines; an OPEN SHELVING UNIT far back on the right with three shelves of plaster
models; and the round stool. The floor is pale clinic vinyl with six or seven long horizontal
reflection strokes, meeting the wall in a continuous coved skirting. NO posters, NO framed
pictures, NO plants, NO instrument trays, NO cabinets of small bottles, NO dental chair, NO
desktop monitors and NO keyboards.

THE TOP SIXTH OF THE PICTURE IS PLAIN WALL AND NOTHING ELSE - every floating panel, every
glow, both people's heads, the window frame and the shelving unit ALL STAY BELOW THAT STRIP.
Nothing darker or brighter anywhere inside it.

TEXTURE AND FINISH - fine even paper grain over the whole image, flat colour areas with a few
loose hand-drawn shading strokes, gentle and warm-hearted, the look of a printed editorial
illustration even though the subject is futuristic.

AVOID: a 3D render, a photorealistic image, a neon cyberpunk look, a dark or night-time room,
blue light washing over the room or the faces; holographic rainbow sheen, lens flare,
particles, floating dust, light beams crossing the room, glowing gloves, wires or sensors on
the hands; menus, buttons, sliders, charts, grids, scan lines, tiny icons, numbers, letters
or any writing on the panels; more than three floating panels; panels of equal size that
split the picture into a comic strip; teeth with faces, eyes or limbs; a whole arch of
perfectly tidy teeth; a skull, a skeleton or an X-ray photograph; anything in the patient's
hands; a floating hand or a forearm with no body; a dental chair with someone being treated
in it; anyone waving or looking at the camera; a faded or ghostly background figure; a
crowded room; cream, beige or tan walls.
```

⚠ 回圖要量（`node drafts/og-measure-band.mjs <圖> "#4478b5"`）：
① **邊緣密度 ≥ 22%** —— 深色面板進來之後這一項應該會大幅改善，若還是不到就是面板畫太淡
② 頂 17% 的 B ≥ 212、安靜區 < 5% —— **面板不可以飄進那一條**
③ 無彩空白 < 5%（v6 已經 4.5%，深面板會把它拉下來）
④ 面板是不是**三張、而且大小明顯不同**（等大就變成分格）

---

## 五之十一、Ⓗ v1 的量測 ＋ 使用者的四件（2026-08-24）

存檔 `drafts/og-topic-ortho-v7-panels.jpg`。使用者：「**有更接近了**」，要改四件：

> 「**螢幕感覺可以再多很多　不一定要大張的　小張的圍繞在醫師周圍**
> 　**助理在遠處幫忙整理各種資料**
> 　螢幕上目前有人臉　牙齒　牙弓　特別的資訊只有最大的螢幕有幾個箭頭而已
> 　**讓螢幕上有更多科技感的標記　或符號**
> 　**醫師不用真的提取出一顆牙齒**　而是所有跟牙齒相關的資訊　任由醫師專業　自信的操作
> 　就像牙齒矯正　被醫師透過各種矯正裝置　技術　往正確的地方排列」

### ⚠⚠ 量測推翻了我上一輪的判斷：密度是「邊」給的，不是「深」給的

| | v5 | v6 | **v7** | 已上線四張的原圖 |
| --- | --- | --- | --- | --- |
| 邊緣密度 | 16.6% | 17.7% | **14.2%** ❌ 更低 | 21.9 ~ 32.4% |
| 無彩空白 | 0.3% | 4.5% ⚠ | **0.4%** ✅ | — |
| 平均明度 | 85.0 | 85.5 | 73.9 | — |
| 頂 17% B 通道 | 244 | 248 | 240 ✅ | 172 ~ 246 |

**我上一輪預測「深色面板會把密度拉上來」，量出來相反。** 成因：那張大面板
**內部是一整片平坦的深藍**（沒有明暗落差 ＝ 沒有邊），而且它還把背後有線條的房間遮掉了。
> **修正後的通則：邊緣密度是「邊」給的，不是「深」給的。**
> 一大片平坦的色塊，不管亮的暗的，都會把密度往下拉。
> 有效的是**很多有邊框、內部有記號的中小型物件** —— 正好是使用者這一輪要的。

### 這一版要改的（寫進 Ⓗ v2）

1. **螢幕變多變小**：一張主螢幕 ＋ **七張小螢幕弧形圍繞醫師**，大小各異（主的 20~40%）。
2. **每張螢幕都要有內容與符號** —— 這是密度的來源，也是使用者要的科技感。
3. ⚠⚠ **符號只准幾何形狀**（圓環、刻度、角度弧、十字準星、節點小圓、虛線路徑、對齊角括號），
   **不准任何像字母或數字的形狀** —— v7 的牙弓上已經長出幾個像字的小記號，
   那是規格 7（圖上不放字）的破口，而且模型畫中文一定壞掉。
4. **拿掉「提取一顆實體的牙」**（使用者指定）。改成**他的手指在螢幕平面上捏著一顆牙、
   沿著牙弓的弧線把它移到位** —— 牙齒始終是螢幕上的發光線稿。
5. **助理改成在遠處整理資料**：站在一排小螢幕前，把其中一張往旁邊推、其他疊成一落。

## 五之十二、Ⓗ v2 的提示詞（一大七小的螢幕群，2026-08-24）

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, one single continuous scene, no comic-strip panels.

STYLE FIRST, AND IT DOES NOT CHANGE: this is a warm, HAND-DRAWN EDITORIAL ILLUSTRATION with
thin uneven ink lines, flat colour and a fine paper grain - the same hand that drew the
reference illustration of the dentist. What is NEW is only the SUBJECT: an orthodontist
working inside a cloud of glowing floating screens. Draw a futuristic idea in a hand-drawn
book style. NOT a 3D render, NOT a photo, NOT neon cyberpunk, NOT a video-game interface.

THE ROOM IS A BRIGHT WHITE DENTAL CLINIC BY DAY. Walls a very light COOL off-white with a
faint blue in them, about #e6ebf0; a pale clinic floor. THE ROOM STAYS BRIGHT - do not darken
it and do not flood it with blue light. All the darkness and all the glow belong to the
floating screens and to his fingertips.

THE PICTURE IN ONE LINE: A CONFIDENT ORTHODONTIST STANDS INSIDE A CURVED CLOUD OF GLOWING
SCREENS FULL OF TEETH AND MEASUREMENTS, MOVING A TOOTH INTO ITS PLACE WITH HIS GLOVED FINGERS,
WHILE HIS PATIENT WATCHES AND LOOKS FORWARD TO IT.

THE SEVEN THINGS THAT MATTER MOST, IN ORDER:

1. THE SCREENS ARE MANY AND THEY CURVE AROUND HIM - ONE BIG, SEVEN SMALL. All of them are
   panels of dark translucent blue glass (#2b4a72, the room faintly visible through them),
   each with ONE thin bright cyan-white edge line (#bcdcff), softly rounded corners and a
   gentle halo of light around it. They hang in the air at different depths and angles,
   SWEEPING IN A WIDE ARC FROM HIS UPPER LEFT, ACROSS IN FRONT OF HIM, DOWN TO HIS LOWER
   RIGHT, so he is plainly standing INSIDE his own workspace.
   • THE MAIN SCREEN: in front of him, slightly left of the picture's centre, about FIFTY PER
     CENT of the picture height.
   • THE SEVEN SMALLER SCREENS: between a fifth and two fifths of the main screen's size,
     ALL DIFFERENT SIZES AND ALL TILTED AT DIFFERENT ANGLES, some overlapping each other,
     the further ones smaller and fainter. NO two screens are the same size, and no screens
     are laid out in a neat grid or a straight row.

2. EVERY SCREEN HAS SOMETHING ON IT, DRAWN AS GLOWING PALE CYAN-WHITE LINE ART (#bcdcff):
   • MAIN SCREEN: A DENTAL ARCH SEEN FROM ABOVE - a horseshoe of TWELVE teeth, each a clear
     simple shape. THREE OF THEM ARE PLAINLY OUT OF LINE: one rotated sideways, one tipped
     over, one pushed inwards out of the horseshoe. TWO THICK SHORT CURVED ARROWS show which
     way two of them must travel, and a few small measurement marks sit around the arch.
   • The seven small screens carry, one each: a SIDE PROFILE of a face in glowing line (no
     skull, no eye socket); UPPER AND LOWER TEETH SEEN FROM THE SIDE, biting together; ONE
     SINGLE TOOTH seen in three-quarter view with a curved rotation arrow around it; A SECOND
     DENTAL ARCH, simpler and smaller; and THREE PURE-DIAGRAM SCREENS carrying only marks -
     one a set of concentric circles with tick marks around the rim, one a dotted path with
     small round nodes along it, one an angle drawn between two straight lines with an arc
     across it.
   ⚠ THE MARKS ARE PURELY GEOMETRIC: circles, rings, arcs, crosshairs, short tick marks,
   small round nodes, dotted paths, corner brackets. THERE ARE NO LETTERS, NO NUMBERS, NO
   WORDS, NO LABELS AND NOTHING THAT RESEMBLES WRITING ANYWHERE ON ANY SCREEN, in any
   language. Each small screen carries only ONE OR TWO large marks - never a busy dashboard.

3. HE IS MOVING A TOOTH INTO PLACE, AND IT STAYS ON THE SCREEN. On the main screen, the thumb
   and index finger of his near hand are PINCHING ONE OF THE CROOKED TEETH and sliding it
   along the horseshoe towards the empty place where it belongs; his fingers are ON the
   glass, and a SHORT SOFT TRAIL OF LIGHT follows behind that tooth to show it moving. THE
   TOOTH REMAINS PART OF THE GLOWING LINE ART ON THE SCREEN - he does NOT pull anything out
   of the screen, and NO solid three-dimensional tooth floats anywhere in this picture.

4. THE DENTIST IS CONFIDENT AND COMPLETELY IN COMMAND. A Taiwanese man in his late thirties,
   short black hair, clean-shaven, standing on the LEFT, turned three-quarters towards the
   screens, feet planted, shoulders level and open, chin level - calm, absorbed, unhurried, a
   specialist doing what he does every day. NOT a magician, NOT a performer, NOT straining.
   HE WEARS CLOSE-FITTING CLINIC GLOVES IN SOLID BLUE (#4478b5) with a soft rolled cuff -
   plain, no pattern, no wires, no sensors, no lights built into them. BOTH ARMS ARE RAISED
   IN FRONT OF HIM, ELBOWS BENT, BOTH FULLY VISIBLE AND BOTH CONNECTED TO HIS BODY: the near
   hand pinches and slides the tooth (point 3); THE OTHER HAND IS OPEN, fingers spread, palm
   towards a smaller screen beside him, PUSHING IT GENTLY ASIDE so it tilts away - he is
   working several screens at once.
   AT HIS FINGERTIPS THERE IS A SMALL BRIGHT GLOW: a soft cyan-white halo (#dff0ff) no bigger
   than a fingertip at the tips of both hands, brightest where his fingers touch the glass.
   THE GLOW IS ONLY AT THE FINGERTIPS AND ON THE SCREENS - no beams across the room, no
   sparkles, no floating dust, no light on the walls or floor, and his face is lit normally
   by daylight, never blue.
   HIS EYES ARE ON THE TOOTH HE IS MOVING. His mouth is slightly open, explaining quietly as
   he works. THERE IS NO HAND ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A VISIBLE
   PERSON.

5. THE PATIENT IS SITTING ON A ROUND STOOL ON THE RIGHT, LOOKING UP AT THE SCREENS, AND HE IS
   LOOKING FORWARD TO THIS. A Taiwanese man or woman of about forty - an ordinary working
   adult, NOT a teenager and NOT a student. He sits on a simple round stool, body tipped
   slightly forward, ONE HAND ON HIS KNEE AND THE OTHER LOOSELY ON THE EDGE OF THE STOOL; HIS
   HANDS ARE COMPLETELY EMPTY. HIS CHIN IS LIFTED AND HIS EYES ARE ON THE MAIN SCREEN. HIS
   EXPRESSION IS HOPEFUL AND TRUSTING - eyebrows lifted a little at the inner ends, eyes open
   and soft, the beginning of a small closed smile, shoulders down and relaxed. He is NOT
   anxious, NOT frowning, NOT astonished, NOT laughing, NOT touching his own mouth, and the
   glow does not turn his face blue.

6. THE NURSE IS FAR BACK ON THE RIGHT, SORTING THE RECORDS, AND SHE IS NOT LOOKING AT THEM.
   Small because she is distant, but drawn with exactly the same line weight as everyone
   else: she stands side-on in front of HER OWN LITTLE CLUSTER OF FOUR SMALL FLOATING SCREENS
   - one hand sliding one screen away to the side, the other hand resting on a second, and
   two more already stacked neatly behind them like filed cards. She is absorbed in her own
   task. THIS IS WHAT MAKES THE PICTURE FEEL ALIVE: two separate pieces of work happening at
   the same time, nobody posing for us.

7. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

DRAW THE PEOPLE EXACTLY IN THE STYLE OF THE REFERENCE ILLUSTRATION OF THE DENTIST. Thin
hand-drawn linework whose weight varies and sometimes breaks - NOT a thick even outline. Each
face is ONE FLAT SKIN TONE with no shading, no modelling, no cheekbones and no jaw shadow. On
the face there is only: the outline, two eyes drawn as small simple dots with NO whites, NO
pupils and NO highlights, two short eyebrows, a tiny nose mark, a small mouth line and an ear.
Hair is a flat shape in two tones with no individual strands. EVERY PERSON IS DRAWN WITH
EXACTLY THE SAME LINE WEIGHT AND SOLIDITY, including the one furthest away. The three people
differ clearly in age, build, hair and clothing colour, and no two faces are alike.

COLOUR. The speciality blue is #4478b5: strongest on THE GLOVES, and giving the screens their
dark blue glass. The line art inside the screens is pale cyan-white (#bcdcff). THE DENTIST'S
SCRUBS UNDER HIS OPEN WHITE COAT ARE PALE WARM GREY (#cfd0cc), NOT BLUE, so the gloves stand
out. The patient wears a warm oatmeal shirt (#d8cdbb) over dark slate trousers; the nurse pale
slate blue-grey. The picture is a BRIGHT WHITE ROOM WITH DARK GLOWING SCREENS IN IT, not a
dark blue picture.

THE ROOM STILL HAS TO BE THERE BEHIND THE SCREENS - keep it simple but never blank: a TALL
WINDOW filling the left edge with three simple frame bars and the pale flat silhouettes of two
low rooftops beyond it; a LONG COUNTER crossing behind him at about two thirds of the picture
height with two large drawer lines; the round stool; and a pale clinic floor with six or seven
long horizontal reflection strokes, meeting the wall in a continuous coved skirting. NO
posters, NO framed pictures, NO plants, NO instrument trays, NO dental chair, NO desktop
monitors, NO keyboards.

THERE MUST BE NO LARGE EMPTY AREA ANYWHERE IN THE PICTURE. Every part of the image below the
top strip carries either a person, a screen with marks on it, or a piece of clearly drawn
furniture. At the same time nothing is cramped: the screens keep visible gaps between them.

THE TOP SIXTH OF THE PICTURE IS PLAIN WALL AND NOTHING ELSE - every screen, every glow, both
people's heads, the window frame and the nurse's screens ALL STAY BELOW THAT STRIP. The
highest screen's upper edge sits at about one fifth of the way down the picture.

TEXTURE AND FINISH - fine even paper grain over the whole image, flat colour areas with a few
loose hand-drawn shading strokes, gentle and warm-hearted, the look of a printed editorial
illustration even though the subject is futuristic.

AVOID: a 3D render, a photorealistic image, neon cyberpunk, a dark or night-time room, blue
light washing over the room or the faces; letters, numbers, words, labels or anything
resembling writing on the screens; menus, buttons, sliders, bar charts, keyboards, dense
dashboards; a neat grid or straight row of equal screens; more than eight screens around the
dentist; a solid three-dimensional tooth floating in the air; the dentist pulling anything out
of a screen; teeth with faces, eyes or limbs; an arch of perfectly tidy teeth; a skull, a
skeleton or an X-ray photograph; anything in the patient's hands; a floating hand or a forearm
with no body; a dental chair with someone being treated in it; anyone waving or looking at the
camera; a faded or ghostly background figure; lens flare, sparkles, particles, floating dust,
light beams; cream, beige or tan walls; a large blank area of wall or floor.
```

⚠ 回圖要量：① **邊緣密度 ≥ 22%**（v7 只有 14.2，這一版靠八張螢幕的邊框與符號拉）
② 頂 17% 的 B ≥ 212、安靜區 < 5% ③ 螢幕上有沒有長出「像字的東西」
④ 有沒有還在畫一顆浮空的實體牙齒（使用者已經否決）

---

## 五之十三、Ⓗ v2 的量測 ＋ 使用者的三件（2026-08-24）

存檔 `drafts/og-topic-ortho-v8-screens.jpg`。使用者：

> 「**有三張圖跟牙齒沒什麼關聯　還是要換一下　可以是局部牙齒移動的示意圖**　附圖參考
> 　另外目前**人物風格和網站常用風格有點落差　還是要在網站插圖風格內**
> 　醫師穿**藍色刷手服 ＋ 白色醫師長袍**　現在圖片是短袍」

參考圖三張已進版控：`drafts/ortho-ref-loop-mechanics.png`、`-sliding-mechanics.png`、
`-malocclusion-types.png`。

### ⚠⚠ 「風格落差」是六件具體的事，不是感覺 —— 對照 `assets/og-topic-endo.jpg` 逐項看出來的

| | 站上的風格（顯微根管那張） | v8 |
| --- | --- | --- |
| 線 | **深褐色**墨線，粗細不均，手繪感重 | 細硬的黑灰線 |
| 臉 | **兩頰有柔和的粉色腮紅**，眼睛是小黑點 | 沒有腮紅，臉偏白 |
| 頭身 | 頭略大、身體圓潤（約 6 頭身） | 寫實比例、修長 |
| 白袍 | **過膝的長袍** | 到腰的短袍 |
| 房間 | **木頭色櫃子、暖白牆、暖光** | 整張冷藍白 |
| 質感 | 明顯紙紋 ＋ 手繪陰影筆觸 | 乾淨平塗 |

⚠⚠ **這一輪的張力**：站上是暖色調，但這張要冷藍的科技螢幕。
**處理方式是分工：房間回到暖色（木櫃、暖白牆、暖光），冷藍只給螢幕與手套。**
⚠ 暖白牆不影響帶子 —— endo 那張的頂 17% 量到 B=246。
**但一定要是「很亮的暖白」（#f0ece4，B=228），不是米黃（#efe0c0 只有 B=192，會讓帶子追不上套色）。**

### 量測

| | v7 | v8 | 已上線四張的原圖 |
| --- | --- | --- | --- |
| 邊緣密度 | 14.2% | **14.9%** ❌ | 21.9 ~ 32.4（endo 22.1） |
| 無彩空白 | 0.4% | 2.2% ✅ | — |
| 頂 17% B | 240 ✅ | 243 ✅ | — |

⚠ 密度仍然不到。這一版的解法**同時也是風格解法**：木頭櫃子（門縫、把手、木紋）、
暖色地板、手繪陰影筆觸 —— endo 那張的密度就是這樣來的。

### 刷手服與手套都藍，怎麼分開

使用者指定刷手服要藍，而手套也是主題色藍。**同色系靠明度分開**：
手套 **`#4478b5`（套色，最飽和）**、刷手服 **`#6a94c4`（淺一階）**，中間隔著白色長袍。
⚠ 這推翻了我上一輪「刷手服改淺灰」的做法 —— 那是為了讓手套跳出來，
現在改用明度差達成同一件事。

## 五之十四、Ⓗ v3 的提示詞（回到站上的風格 ＋ 局部牙齒移動示意圖，2026-08-24）

**參考圖三組，用途分開標：**
① `assets/og-topic-endo.jpg` → **整體風格**：深褐色手繪墨線的顏色與粗細、腮紅、
   頭身比例、白色長袍的長度、木頭家具、暖色調、紙紋。**不要**參考它的構圖、顯微鏡、
   擬人牙齒、磚紅色。
② `drafts/ortho-ref-loop-mechanics.png` ＋ `-sliding-mechanics.png` → **托槽與弓線的形狀**
   （小方塊＋一條穿過去的線＋線上的 U/T 形彎曲）與**力箭頭怎麼畫**。
   **不要**參考照片感、顏色、上面的文字、真實口內照。
③ `drafts/ortho-ref-malocclusion-types.png` → **側面咬合的形狀**（上排突出、深咬、開咬、擁擠）。
   **不要**參考粉紅色牙齦、色塊標籤、上面的文字。

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, one single continuous scene, no comic-strip panels.

THE STYLE IS THE MOST IMPORTANT INSTRUCTION AND IT COMES FROM THE REFERENCE ILLUSTRATION OF
THE FEMALE DENTIST WITH THE MICROSCOPE. Match it exactly:
• EVERY OUTLINE IS DRAWN IN WARM DARK BROWN INK (#4a3a30), NEVER BLACK and never cold grey.
  The line is clearly hand-drawn: its thickness varies, it wobbles slightly and it breaks in
  places. It is NOT a thin even vector line.
• EVERY FACE HAS SOFT PINK BLUSH ON BOTH CHEEKS. Eyes are small dark dots with no whites and
  no pupils; eyebrows are two short strokes; the nose is one small mark; the mouth is one
  short line. Hair is a flat shape in two tones.
• THE PEOPLE ARE GENTLY STYLISED, NOT REALISTIC: the head is a little large for the body
  (about six heads tall), shoulders and limbs are rounded and soft, hands are simplified.
• THE WHOLE IMAGE CARRIES A FINE PAPER GRAIN, flat colour areas, and loose hand-drawn
  hatching for shadows.
Draw a futuristic subject in that warm, hand-drawn storybook style. NOT a 3D render, NOT a
photo, NOT neon cyberpunk, NOT a clean vector infographic.

THE ROOM IS A WARM, BRIGHT DENTAL CLINIC BY DAY - the same kind of room as the reference
illustration: PALE WARM OFF-WHITE WALLS (#f0ece4 - very light, warm, but never yellow, never
beige, never tan and never cream), LIGHT WOOD CABINETS with visible door lines and simple
handles along the back, and a pale warm floor. THE ROOM IS WARM AND THE ONLY COLD BLUE THINGS
IN THE PICTURE ARE THE FLOATING SCREENS AND THE DENTIST'S GLOVES. Do not darken the room, do
not flood it with blue light.

THE PICTURE IN ONE LINE: A CONFIDENT ORTHODONTIST STANDS INSIDE A CURVED CLOUD OF GLOWING
SCREENS SHOWING HOW TEETH ARE MOVED, GUIDING ONE TOOTH INTO ITS PLACE WITH HIS GLOVED FINGERS,
WHILE HIS PATIENT WATCHES AND LOOKS FORWARD TO IT.

THE SEVEN THINGS THAT MATTER MOST, IN ORDER:

1. THE SCREENS ARE MANY AND THEY CURVE AROUND HIM - ONE BIG, SEVEN SMALL. Panels of dark
   translucent blue glass (#2b4a72, the room faintly visible through them), each with ONE
   thin bright cyan-white edge line (#bcdcff), softly rounded corners and a gentle halo.
   They hang at different depths and angles, SWEEPING IN A WIDE ARC FROM HIS UPPER LEFT,
   ACROSS IN FRONT OF HIM, DOWN TO HIS LOWER RIGHT, so he stands INSIDE his own workspace.
   THE MAIN SCREEN is in front of him, slightly left of centre, about FIFTY PER CENT of the
   picture height. THE SEVEN SMALL ONES are between a fifth and two fifths of its size, ALL
   DIFFERENT SIZES, tilted at different angles, some overlapping, the further ones smaller
   and fainter. No neat grid, no straight row, no two the same size.

2. EVERY SCREEN SHOWS TEETH - AND MOST OF THEM SHOW HOW TEETH ARE MOVED. All screen content
   is GLOWING PALE CYAN-WHITE LINE ART (#bcdcff), big and simple:
   • MAIN SCREEN: A DENTAL ARCH SEEN FROM ABOVE - a horseshoe of TWELVE teeth. THREE ARE
     PLAINLY OUT OF LINE: one rotated sideways, one tipped over, one pushed inwards out of
     the horseshoe. TWO THICK SHORT CURVED ARROWS show which way two of them must travel.
   • THREE OF THE SMALL SCREENS ARE CLOSE-UP DIAGRAMS OF TEETH BEING MOVED, drawn from the
     reference photographs of orthodontic mechanics: a row of FOUR OR FIVE LARGE TEETH seen
     from the front or the side, each carrying a SMALL SQUARE BRACKET on its surface, with A
     SINGLE WIRE RUNNING THROUGH ALL THE BRACKETS. On one screen that wire has A CLEAR
     U-SHAPED OR T-SHAPED LOOP bent into it between two teeth; on another, ONE TOOTH IS
     SLIDING ALONG THE WIRE with a straight arrow behind it showing the direction; on the
     third, TWO CURVED ARROWS around a single tooth show it being rotated and uprighted.
     Draw the brackets and the wire LARGE AND SIMPLE - a bracket is just a small rounded
     square with a slot, the wire is one continuous line.
   • THE OTHER FOUR SMALL SCREENS: a SIDE PROFILE of a face in glowing line (no skull, no eye
     socket); UPPER AND LOWER TEETH SEEN FROM THE SIDE WITH THE UPPER FRONT TEETH CLEARLY
     STICKING OUT BEYOND THE LOWER ONES; a SECOND DENTAL ARCH, simpler and smaller; and ONE
     SINGLE TOOTH in three-quarter view with a curved rotation arrow around it.
   ⚠ Any extra marks are PURELY GEOMETRIC - short tick marks, small round nodes, one angle
   with an arc, corner brackets at the screen corners. THERE ARE NO LETTERS, NO NUMBERS, NO
   WORDS AND NOTHING THAT RESEMBLES WRITING ANYWHERE, in any language. Every screen has teeth
   or a face on it; NO screen carries only abstract symbols.

3. HE IS MOVING A TOOTH INTO PLACE, AND IT STAYS ON THE SCREEN. On the main screen, the thumb
   and index finger of his near hand are PINCHING ONE OF THE CROOKED TEETH and sliding it
   along the horseshoe towards the empty place where it belongs; his fingers are ON the glass
   and a SHORT SOFT TRAIL OF LIGHT follows behind that tooth. THE TOOTH REMAINS PART OF THE
   GLOWING LINE ART - he does NOT pull anything out, and NO solid tooth floats anywhere.

4. THE DENTIST - CONFIDENT, IN COMMAND, AND DRESSED LIKE THE CLINIC'S DENTISTS. A Taiwanese
   man in his late thirties, short black hair, clean-shaven, gently stylised as described
   above, with soft pink blush on his cheeks. HE WEARS A LONG WHITE DOCTOR'S COAT THAT REACHES
   BELOW HIS KNEES, open at the front, over BLUE SCRUBS (#6a94c4) - a V-neck scrub top and
   matching scrub trousers - and CLOSE-FITTING GLOVES IN THE DEEPER CLINIC BLUE (#4478b5)
   with a soft rolled cuff. THE GLOVES ARE MORE SATURATED THAN THE SCRUBS so the hands read
   clearly; the gloves are plain, with no pattern, no wires, no sensors and no lights.
   He stands on the LEFT, turned three-quarters towards the screens, feet planted, shoulders
   level and open, calm and unhurried - a specialist doing what he does every day. NOT a
   magician, NOT a performer. BOTH ARMS ARE RAISED IN FRONT OF HIM, ELBOWS BENT, BOTH FULLY
   VISIBLE AND BOTH CONNECTED TO HIS BODY: the near hand pinches and slides the tooth; THE
   OTHER HAND IS OPEN, fingers spread, palm towards a smaller screen beside him, PUSHING IT
   GENTLY ASIDE so it tilts away.
   AT HIS FINGERTIPS THERE IS A SMALL BRIGHT GLOW: a soft cyan-white halo (#dff0ff) no bigger
   than a fingertip, brightest where his fingers touch the glass. THE GLOW IS ONLY AT THE
   FINGERTIPS AND ON THE SCREENS - no beams, no sparkles, no floating dust, no light on the
   walls or floor, and his face keeps its warm daylight colour and its blush, never blue.
   HIS EYES ARE ON THE TOOTH HE IS MOVING; his mouth is slightly open, explaining quietly.
   THERE IS NO HAND ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A VISIBLE PERSON.

5. THE PATIENT IS SITTING ON A ROUND STOOL ON THE RIGHT, LOOKING UP AT THE SCREENS, AND HE IS
   LOOKING FORWARD TO THIS. A Taiwanese man of about forty - an ordinary working adult, NOT a
   teenager and NOT a student - in a warm oatmeal shirt (#d8cdbb) and dark slate trousers,
   with the same blush and the same rounded, stylised drawing. He sits with his body tipped
   slightly forward, ONE HAND ON HIS KNEE AND THE OTHER LOOSELY ON THE EDGE OF THE STOOL; HIS
   HANDS ARE COMPLETELY EMPTY. HIS CHIN IS LIFTED AND HIS EYES ARE ON THE MAIN SCREEN. HIS
   EXPRESSION IS HOPEFUL AND TRUSTING - eyebrows lifted a little at the inner ends, eyes open
   and soft, the beginning of a small closed smile, shoulders relaxed. He is NOT anxious, NOT
   frowning, NOT astonished, NOT laughing and NOT touching his own mouth.

6. THE NURSE IS FAR BACK ON THE RIGHT, SORTING THE RECORDS, AND SHE IS NOT LOOKING AT THEM.
   Small because she is distant, but drawn with exactly the same line weight and the same
   blush: she stands side-on in front of HER OWN CLUSTER OF FOUR SMALL FLOATING SCREENS - one
   hand sliding one screen aside, the other resting on a second, two more already stacked
   behind them like filed cards. She wears the same blue scrubs. She is absorbed in her own
   task. TWO SEPARATE PIECES OF WORK HAPPENING AT ONCE, nobody posing for us.

7. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

EVERYONE LOOKS DIFFERENT: the three people differ clearly in age, build, hair and clothing,
and no two faces are alike. EVERY PERSON IS DRAWN WITH EXACTLY THE SAME LINE WEIGHT AND
SOLIDITY, including the one furthest away.

THE ROOM MUST BE FURNISHED, NOT BLANK - this is where the picture gets its density: a run of
LIGHT WOOD CABINETS across the back wall with clear door lines and simple handles; a TALL
WINDOW at the left edge with three frame bars and the pale silhouettes of two low rooftops
beyond it; the round stool; a warm pale floor with a few long horizontal reflection strokes
and a continuous coved skirting. Add loose hand-drawn hatching where shadows fall - under the
cabinets, beneath the stool, along the floor. NO posters, NO framed pictures, NO plants, NO
instrument trays, NO dental chair, NO desktop monitors, NO keyboards.

THERE MUST BE NO LARGE EMPTY AREA ANYWHERE BELOW THE TOP STRIP - every part of the picture
carries a person, a screen with teeth on it, or clearly drawn furniture. Nothing is cramped:
the screens keep visible gaps between them.

THE TOP SIXTH OF THE PICTURE IS PLAIN WARM OFF-WHITE WALL AND NOTHING ELSE - every screen,
every glow, both people's heads, the window frame, the cabinets and the nurse's screens ALL
STAY BELOW THAT STRIP. The highest screen's upper edge sits at about one fifth of the way
down the picture.

AVOID: black outlines; thin even vector lines; realistic adult body proportions; faces
without blush; a short waist-length white coat; a cold blue or grey room; a dark or
night-time room; blue light on faces or walls; a 3D render, a photograph, neon cyberpunk;
letters, numbers, words or anything resembling writing on the screens; menus, buttons,
sliders, bar charts, dense dashboards; a screen with only abstract symbols and no teeth; a
neat grid or straight row of equal screens; more than eight screens around the dentist; a
solid three-dimensional tooth floating in the air; teeth with faces, eyes or limbs; an arch
of perfectly tidy teeth; pink gums or a photographic mouth; a skull, a skeleton or an X-ray
photograph; anything in the patient's hands; a floating hand or a forearm with no body; a
dental chair with someone being treated in it; anyone waving or looking at the camera; lens
flare, sparkles, particles, floating dust, light beams; a large blank area of wall or floor.
```

⚠ 回圖要量：① **邊緣密度 ≥ 22%**（木櫃與陰影筆觸是這一版的主要來源）
② 頂 17% 的 B ≥ 212（暖白牆要夠亮，米黃就過不了）③ 螢幕上每一張都有牙齒
④ 線是不是深褐色、臉上有沒有腮紅、白袍是不是過膝

---

## 五之十五、Ⓗ v3 被退回：臉的規格是我寫壞的（2026-08-24）

存檔 `drafts/og-topic-ortho-v9-warm.jpg`。使用者：
> 「插圖風格和之前的還是有很大的問題　**現在的鼻子和腮紅太明顯**」

⚠⚠ **兩件都是我的提示詞寫出來的**（同第十一之一節那三條的性質）：

| 我寫的 | 模型畫出來的 | 站上實際是什麼樣 |
| --- | --- | --- |
| `EVERY FACE HAS SOFT PINK BLUSH ON BOTH CHEEKS` | **兩個橘紅色的圓塊**，邊界清楚 | 淡到**縮圖上看不出來**的一層暖色，沒有邊界 |
| `the nose is one small mark` | 有鼻頭、有鼻翼、有陰影的鼻子 | **一條比眼睛還短的細線**，沒有鼻孔鼻翼 |
| `about six heads tall` | 頭偏大的卡通比例 | **成人的正常比例**，只是簡化 |

> **記起來：「有腮紅」和「腮紅要淡到看不出來」是兩句不同的話。**
> 凡是站上「幾乎看不見」的特徵，提示詞要寫**上限**（不可以超過多少），
> 不能只寫「有這個東西」——模型一定會把它畫清楚，那是它的預設行為。

**判準要寫成可驗收的**：把圖縮到 250px，**看不出臉上有腮紅**、**看不出鼻子的形狀**才算對。

### 量測（密度終於在動）

| | v7 | v8 | **v9** | 已上線四張的原圖 |
| --- | --- | --- | --- | --- |
| 邊緣密度 | 14.2% | 14.9% | **20.9%** | 21.9 ~ 32.4 |
| 頂 17% B | 240 | 243 | 235 ✅ | — |
| 安靜區被佔 | 3.31% | 2.02% | 2.99% | 1.5 ~ 8.4 |

**木櫃 ＋ 暖房間 ＋ 手繪陰影把密度從 14.9 拉到 20.9**（＝上一輪的判斷是對的），
只差 1 個百分點就進入已上線那四張的區間。

### 風格參考圖改成四張（之前只給一張，這是風格跑掉的主因）

| # | 檔案 | 只參考 | 不要參考 |
| --- | --- | --- | --- |
| ① | `assets/og-topic-kids.jpg` | **臉的畫法**（五官有多少筆、腮紅有多淡、鼻子多小）、線的顏色與粗細、人物比例 | 構圖、診療椅、兒牙的暖黃色 |
| ② | `assets/og-topic-endo.jpg` | **同上**，另加木頭家具與暖白牆的畫法 | 構圖、顯微鏡、擬人牙齒、磚紅 |
| ③ | `drafts/ortho-ref-loop-mechanics.png` | 托槽與弓線的形狀、線上的 U/T 形彎曲 | 照片感、顏色、文字 |
| ④ | `drafts/ortho-ref-sliding-mechanics.png` | 牙齒滑動的力箭頭怎麼畫 | 照片感、顏色、文字、粉紅牙齦 |

⚠ 使用者這一輪另外給的兩張站上舊圖也存進版控當風格庫：
`drafts/style-ref-perio-full.jpg`、`drafts/style-ref-waiting-room.jpg`。

## 五之十六、Ⓗ v4 的提示詞（臉改成「最少的筆畫」，2026-08-24）

⚠ 這一版**只改臉的規格與比例**，其餘和 v3 相同（螢幕、動作、房間、密度那幾段一個字沒動）。

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, one single continuous scene, no comic-strip panels.

THE STYLE COMES FROM THE TWO REFERENCE ILLUSTRATIONS OF THE CLINIC (the kneeling dentist with
the child, and the woman at the microscope). Match them exactly:

• LINE: every outline is drawn in WARM DARK BROWN INK (#4a3a30), NEVER black, never cold grey.
  The line is clearly hand-drawn - its thickness varies, it wobbles slightly, it breaks in
  places. It is NOT a thin even vector line.

• FACES ARE DRAWN WITH AS FEW MARKS AS POSSIBLE. THIS IS THE HARDEST PART - KEEP IT MINIMAL:
  - THE NOSE IS ONE VERY SHORT, VERY THIN LINE, SHORTER THAN AN EYE IS WIDE. It has NO
    nostrils, NO wings, NO tip, NO shading and NO colour of its own. On a three-quarter face
    it may be nothing more than a tiny notch on the profile. IF IN DOUBT, DRAW LESS NOSE.
  - BLUSH IS ALMOST INVISIBLE: at most a soft edgeless warm tint over the cheekbone, only a
    shade warmer than the skin itself. IT MUST NOT BE VISIBLE WHEN THE PICTURE IS SHRUNK TO
    250 PIXELS WIDE. It is NEVER a circle, NEVER a defined patch, NEVER orange, NEVER red,
    and NEVER two round dots. IF IN DOUBT, LEAVE THE BLUSH OUT ALTOGETHER.
  - Eyes are small dark dots with no whites and no pupils. Eyebrows are two short strokes.
    The mouth is one short line. Ears are one small curve.
  - SKIN IS A VERY PALE WARM IVORY (#f6e6d8), FLAT, with no modelling, no cheekbones, no jaw
    shadow, no chin line and no neck shading.
  - COUNT THE MARKS: a whole face is two eye dots, two eyebrow strokes, one tiny nose line,
    one mouth line and one ear curve. NOTHING ELSE.

• PEOPLE HAVE NORMAL ADULT PROPORTIONS, gently simplified - about seven heads tall, shoulders
  and hands softened and simplified. They are NOT chibi, NOT big-headed, NOT children-shaped.

• The whole image carries a FINE PAPER GRAIN, flat colour areas, and loose hand-drawn hatching
  for shadows.

Draw a futuristic subject in that warm, hand-drawn storybook style. NOT a 3D render, NOT a
photo, NOT neon cyberpunk, NOT a clean vector infographic.

THE ROOM IS A WARM, BRIGHT DENTAL CLINIC BY DAY - the same kind of room as the reference
illustrations: PALE WARM OFF-WHITE WALLS (#f0ece4 - very light and warm, but never yellow,
never beige, never tan, never cream), LIGHT WOOD CABINETS with visible door lines and simple
handles along the back wall, and a pale warm floor. THE ROOM IS WARM; THE ONLY COLD BLUE
THINGS IN THE PICTURE ARE THE FLOATING SCREENS AND THE DENTIST'S GLOVES. Do not darken the
room and do not flood it with blue light.

THE PICTURE IN ONE LINE: A CONFIDENT ORTHODONTIST STANDS INSIDE A CURVED CLOUD OF GLOWING
SCREENS SHOWING HOW TEETH ARE MOVED, GUIDING ONE TOOTH INTO ITS PLACE WITH HIS GLOVED FINGERS,
WHILE HIS PATIENT WATCHES AND LOOKS FORWARD TO IT.

THE SEVEN THINGS THAT MATTER MOST, IN ORDER:

1. THE SCREENS ARE MANY AND THEY CURVE AROUND HIM - ONE BIG, SEVEN SMALL. Panels of dark
   translucent blue glass (#2b4a72, the room faintly visible through them), each with ONE thin
   bright cyan-white edge line (#bcdcff), softly rounded corners and a gentle halo. They hang
   at different depths and angles, SWEEPING IN A WIDE ARC FROM HIS UPPER LEFT, ACROSS IN FRONT
   OF HIM, DOWN TO HIS LOWER RIGHT, so he stands INSIDE his own workspace. THE MAIN SCREEN is
   in front of him, slightly left of centre, about FIFTY PER CENT of the picture height. THE
   SEVEN SMALL ONES are between a fifth and two fifths of its size, ALL DIFFERENT SIZES,
   tilted at different angles, some overlapping, the further ones smaller and fainter. No neat
   grid, no straight row, no two the same size.

2. EVERY SCREEN SHOWS TEETH - AND MOST OF THEM SHOW HOW TEETH ARE MOVED. All screen content is
   GLOWING PALE CYAN-WHITE LINE ART (#bcdcff), big and simple:
   • MAIN SCREEN: A DENTAL ARCH SEEN FROM ABOVE - a horseshoe of TWELVE teeth. THREE ARE
     PLAINLY OUT OF LINE: one rotated sideways, one tipped over, one pushed inwards out of the
     horseshoe. TWO THICK SHORT CURVED ARROWS show which way two of them must travel.
   • THREE OF THE SMALL SCREENS ARE CLOSE-UP DIAGRAMS OF TEETH BEING MOVED, drawn from the
     reference photographs of orthodontic mechanics: a row of FOUR OR FIVE LARGE TEETH seen
     from the front or the side, each carrying a SMALL SQUARE BRACKET on its surface, with A
     SINGLE WIRE RUNNING THROUGH ALL THE BRACKETS. On one screen that wire has A CLEAR
     U-SHAPED OR T-SHAPED LOOP bent into it between two teeth; on another, ONE TOOTH IS
     SLIDING ALONG THE WIRE with a straight arrow behind it; on the third, TWO CURVED ARROWS
     around a single tooth show it being rotated and uprighted. Draw the brackets and the wire
     LARGE AND SIMPLE - a bracket is a small rounded square with a slot, the wire is one
     continuous line.
   • THE OTHER FOUR SMALL SCREENS: a SIDE PROFILE of a face in glowing line (no skull, no eye
     socket); UPPER AND LOWER TEETH SEEN FROM THE SIDE WITH THE UPPER FRONT TEETH CLEARLY
     STICKING OUT BEYOND THE LOWER ONES; a SECOND DENTAL ARCH, simpler and smaller; and ONE
     SINGLE TOOTH in three-quarter view with a curved rotation arrow around it.
   ⚠ Any extra marks are PURELY GEOMETRIC - short tick marks, small round nodes, one angle
   with an arc, corner brackets at the screen corners. THERE ARE NO LETTERS, NO NUMBERS, NO
   WORDS AND NOTHING THAT RESEMBLES WRITING ANYWHERE, in any language. Every screen has teeth
   or a face on it; NO screen carries only abstract symbols.

3. HE IS MOVING A TOOTH INTO PLACE, AND IT STAYS ON THE SCREEN. On the main screen, the thumb
   and index finger of his near hand are PINCHING ONE OF THE CROOKED TEETH and sliding it along
   the horseshoe towards the empty place where it belongs; his fingers are ON the glass and a
   SHORT SOFT TRAIL OF LIGHT follows behind that tooth. THE TOOTH REMAINS PART OF THE GLOWING
   LINE ART - he does NOT pull anything out, and NO solid tooth floats anywhere.

4. THE DENTIST - CONFIDENT, IN COMMAND, AND DRESSED LIKE THE CLINIC'S DENTISTS. A Taiwanese man
   in his late thirties, short black hair, clean-shaven, drawn with the minimal face described
   above. HE WEARS A LONG WHITE DOCTOR'S COAT THAT REACHES BELOW HIS KNEES, open at the front,
   over BLUE SCRUBS (#6a94c4) - a V-neck scrub top and matching trousers - and CLOSE-FITTING
   GLOVES IN THE DEEPER CLINIC BLUE (#4478b5) with a soft rolled cuff. THE GLOVES ARE MORE
   SATURATED THAN THE SCRUBS so the hands read clearly; the gloves are plain, with no pattern,
   no wires, no sensors and no lights. He stands on the LEFT, turned three-quarters towards the
   screens, feet planted, shoulders level and open, calm and unhurried - a specialist doing
   what he does every day. NOT a magician, NOT a performer. BOTH ARMS ARE RAISED IN FRONT OF
   HIM, ELBOWS BENT, BOTH FULLY VISIBLE AND BOTH CONNECTED TO HIS BODY: the near hand pinches
   and slides the tooth; THE OTHER HAND IS OPEN, fingers spread, palm towards a smaller screen
   beside him, PUSHING IT GENTLY ASIDE so it tilts away.
   AT HIS FINGERTIPS THERE IS A SMALL BRIGHT GLOW: a soft cyan-white halo (#dff0ff) no bigger
   than a fingertip, brightest where his fingers touch the glass. THE GLOW IS ONLY AT THE
   FINGERTIPS AND ON THE SCREENS - no beams, no sparkles, no floating dust, no light on the
   walls or floor, and his face keeps its warm daylight colour, never blue.
   HIS EYES ARE ON THE TOOTH HE IS MOVING; his mouth is slightly open, explaining quietly.
   THERE IS NO HAND ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A VISIBLE PERSON.

5. THE PATIENT IS SITTING ON A ROUND STOOL ON THE RIGHT, LOOKING UP AT THE SCREENS, AND HE IS
   LOOKING FORWARD TO THIS. A Taiwanese man of about forty - an ordinary working adult, NOT a
   teenager and NOT a student - in a warm oatmeal shirt (#d8cdbb) and dark slate trousers,
   drawn with the same minimal face. He sits with his body tipped slightly forward, ONE HAND ON
   HIS KNEE AND THE OTHER LOOSELY ON THE EDGE OF THE STOOL; HIS HANDS ARE COMPLETELY EMPTY. HIS
   CHIN IS LIFTED AND HIS EYES ARE ON THE MAIN SCREEN. HIS EXPRESSION IS HOPEFUL AND TRUSTING -
   eyebrows lifted a little at the inner ends, eyes open and soft, the beginning of a small
   closed smile, shoulders relaxed. He is NOT anxious, NOT frowning, NOT astonished, NOT
   laughing and NOT touching his own mouth.

6. THE NURSE IS FAR BACK ON THE RIGHT, SORTING THE RECORDS, AND SHE IS NOT LOOKING AT THEM.
   Small because she is distant, but drawn with exactly the same line weight and the same
   minimal face: she stands side-on in front of HER OWN CLUSTER OF FOUR SMALL FLOATING SCREENS
   - one hand sliding one screen aside, the other resting on a second, two more already stacked
   behind them like filed cards. She wears the same blue scrubs. She is absorbed in her own
   task. TWO SEPARATE PIECES OF WORK HAPPENING AT ONCE, nobody posing for us.

7. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

EVERYONE LOOKS DIFFERENT: the three people differ clearly in age, build, hair and clothing, and
no two faces are alike. EVERY PERSON IS DRAWN WITH EXACTLY THE SAME LINE WEIGHT AND SOLIDITY,
including the one furthest away.

THE ROOM MUST BE FURNISHED, NOT BLANK - this is where the picture gets its density: a run of
LIGHT WOOD CABINETS across the back wall with clear door lines and simple handles; a TALL
WINDOW at the left edge with three frame bars and the pale silhouettes of two low rooftops
beyond it; the round stool; a warm pale floor with a few long horizontal reflection strokes and
a continuous coved skirting. Add loose hand-drawn hatching where shadows fall - under the
cabinets, beneath the stool, along the floor. NO posters, NO framed pictures, NO plants, NO
instrument trays, NO dental chair, NO desktop monitors, NO keyboards.

THERE MUST BE NO LARGE EMPTY AREA ANYWHERE BELOW THE TOP STRIP - every part of the picture
carries a person, a screen with teeth on it, or clearly drawn furniture. Nothing is cramped:
the screens keep visible gaps between them.

THE TOP SIXTH OF THE PICTURE IS PLAIN WARM OFF-WHITE WALL AND NOTHING ELSE - every screen,
every glow, both people's heads, the window frame, the cabinets and the nurse's screens ALL
STAY BELOW THAT STRIP. The highest screen's upper edge sits at about one fifth of the way down
the picture.

AVOID: a visible blush; round, pink, orange or red cheeks; a nose with nostrils, wings, a tip
or shading; a large or detailed nose; face shading, cheekbones, a jaw line or a chin line;
big-headed chibi proportions; black outlines; thin even vector lines; a short waist-length
white coat; a cold blue or grey room; a dark room; blue light on faces or walls; a 3D render,
a photograph, neon cyberpunk; letters, numbers, words or anything resembling writing on the
screens; menus, buttons, sliders, bar charts, dashboards; a screen with only abstract symbols
and no teeth; a neat grid or straight row of equal screens; more than eight screens; a solid
three-dimensional tooth floating in the air; teeth with faces, eyes or limbs; an arch of
perfectly tidy teeth; pink gums or a photographic mouth; a skull, a skeleton or an X-ray
photograph; anything in the patient's hands; a floating hand or a forearm with no body; a
dental chair with someone being treated in it; anyone waving or looking at the camera; lens
flare, sparkles, particles, floating dust, light beams; a large blank area of wall or floor.
```

⚠ 這一版的驗收多一條，而且要**縮到 250px 再看**：
**看不出臉上有腮紅、看不出鼻子的形狀** —— 看得出來就是還沒對。

---

## 五之十七、Ⓗ v4 過關，使用者的五件 ＋ 兩件量出來的（2026-08-24）

存檔 `drafts/og-topic-ortho-v10-minimalface.jpg`。使用者：「**蠻不錯的**」——
臉那一關過了（腮紅與鼻子縮到 250px 都看不出來）。要改五件：

> 「醫師**頭髮可以稍微帶一點藍**　參考根管醫師頭髮有一點紅
> 　醫師**另一隻手…還是一起有動作　操弄螢幕比較有感**
> 　另外**病患的動作表情穿著都太單調**
> 　**診間環境是之前常用的木質感　跟現在的科技未來感不搭**　調整一下
> 　另外**畫面還是偏安靜靜態　加一些像流星或是光電訊號在醫師周圍環繞**」

### ⚠⚠ 另外兩件是量出來的，使用者沒提但一定要修

| | v9 | **v10** | 門檻 |
| --- | --- | --- | --- |
| 頂 17% 的 B 通道 | 235 | **212 ⚠ 剛好卡在懸崖邊** | ≥ 212（算出來的補償色 B 已經到 255） |
| 邊緣密度 | 20.9% | **18.7% ↓** | ≥ 22（已上線四張的下限） |

・**牆被畫成偏黃的米色**（我寫 `#f0ece4`，模型往黃的方向走），B 通道掉到剛好及格。
  **下一版要寫更亮、更中性的暖白 `#f4f2ee`（B=238）**，並明講「不可以偏黃」。
・密度掉回去是因為房間變空（木櫃少了一大半）。**剛好使用者要換掉木質感** ——
  新的密度來源改成：**牆上的凹槽燈帶、櫃體下方的長溝縫、地板的長反光、更多螢幕、光弧**。
  ⚠ **不要再靠木櫃的門縫與把手**，那和這一版的科技感互斥（使用者指出的正是這件）。

### 五件怎麼落實（都寫成幾何，不寫形容詞）

| 使用者的話 | 提示詞怎麼寫 |
| --- | --- |
| 頭髮帶一點藍 | 深藍黑 `#2a3342`，亮處 `#3c4a5e`（＝根管那位「深褐帶紅」的同一個做法，換色相） |
| 另一隻手也要有動作 | 兩隻手都在操作：近手在主螢幕上捏著牙移動，**遠手張開放在旁邊一張小螢幕上，正把它往上撥開** |
| 病患太單調 | ① 穿著加層次（**赭色針織外套 ＋ 淺色襯衫**，不是單一件米色）② 動作：一手撐膝、**另一手抬起指著螢幕上那顆牙**（他在問「這顆嗎」）③ 表情：嘴微張、眉稍揚，是**參與**不是旁觀 |
| 木質感不搭 | 換成**無把手的淺色櫃體＋霧面石白檯面＋牆上一條凹槽燈帶**，木紋完全拿掉 |
| 加流星／光電訊號 | **三到四條長光弧**（亮頭＋漸淡的尾），繞在他與螢幕之間，**長度至少一個頭寬**。⚠ 少而大，不是一片小點 —— 小點在 212px 上就是雜訊 |

## 五之十八、Ⓗ v5 的提示詞（頭髮帶藍、雙手都在操作、病患有戲、現代診間、光弧，2026-08-24）

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, one single continuous scene, no comic-strip panels.

THE STYLE COMES FROM THE REFERENCE ILLUSTRATIONS OF THE CLINIC. Match them exactly:
• LINE: every outline is drawn in WARM DARK BROWN INK (#4a3a30), NEVER black, never cold grey.
  Clearly hand-drawn - the thickness varies, it wobbles slightly, it breaks in places. NOT a
  thin even vector line.
• FACES ARE DRAWN WITH AS FEW MARKS AS POSSIBLE:
  - THE NOSE IS ONE VERY SHORT, VERY THIN LINE, SHORTER THAN AN EYE IS WIDE - no nostrils, no
    wings, no tip, no shading, no colour. IF IN DOUBT, DRAW LESS NOSE.
  - BLUSH IS ALMOST INVISIBLE: at most an edgeless warm tint on the cheekbone, a shade warmer
    than the skin. IT MUST NOT BE VISIBLE AT 250 PIXELS WIDE. Never a circle, never orange,
    never two round dots. IF IN DOUBT, LEAVE IT OUT.
  - Eyes are small dark dots, no whites, no pupils. Eyebrows two short strokes. Mouth one
    short line. Ear one small curve. Skin a flat pale warm ivory (#f6e6d8) with no modelling,
    no cheekbones, no jaw line.
  - COUNT THE MARKS: two eye dots, two eyebrow strokes, one tiny nose line, one mouth line,
    one ear curve. NOTHING ELSE.
• PEOPLE HAVE NORMAL ADULT PROPORTIONS, gently simplified - about seven heads tall. NOT chibi,
  NOT big-headed.
• FINE PAPER GRAIN over the whole image, flat colour, loose hand-drawn hatching for shadows.
Draw a futuristic subject in that warm, hand-drawn storybook style. NOT a 3D render, NOT a
photo, NOT neon cyberpunk, NOT a clean vector infographic.

THE ROOM IS A MODERN, CALM CONSULTING ROOM - BRIGHT AND WARM BUT WITHOUT ANY WOOD.
THE BACK WALL IS A VERY LIGHT, ALMOST NEUTRAL WARM WHITE (#f4f2ee) - bright, barely warm,
and NEVER yellow, NEVER cream, NEVER beige, NEVER tan, NEVER sand. Along the back runs a
SEAMLESS PALE CABINET RUN WITH NO HANDLES AND NO DOOR PANELS - just one long dark shadow gap
beneath a matt stone-white countertop. ONE RECESSED LIGHT LINE glows softly along the wall
above the counter. The floor is a pale seamless resin floor with LONG SOFT HORIZONTAL
REFLECTIONS, meeting the wall in a continuous coved skirting. NO wood grain, NO wooden doors,
NO cabinet handles, NO panelled furniture, NO plants, NO posters, NO dental chair.
The room stays bright: the only cold blue things are the floating screens and the gloves.

THE PICTURE IN ONE LINE: A CONFIDENT ORTHODONTIST STANDS INSIDE A CURVED CLOUD OF GLOWING
SCREENS SHOWING HOW TEETH ARE MOVED, WORKING TWO SCREENS AT ONCE WITH BOTH GLOVED HANDS,
WHILE HIS PATIENT LEANS IN, POINTS AT ONE TOOTH AND ASKS ABOUT IT.

THE EIGHT THINGS THAT MATTER MOST, IN ORDER:

1. THE SCREENS ARE MANY AND THEY CURVE AROUND HIM - ONE BIG, SEVEN SMALL. Panels of dark
   translucent blue glass (#2b4a72, the room faintly visible through them), each with ONE thin
   bright cyan-white edge line (#bcdcff), softly rounded corners and a gentle halo. They hang
   at different depths and angles, SWEEPING IN A WIDE ARC FROM HIS UPPER LEFT, ACROSS IN FRONT
   OF HIM, DOWN TO HIS LOWER RIGHT, so he stands INSIDE his own workspace. THE MAIN SCREEN is
   in front of him, slightly left of centre, about FIFTY PER CENT of the picture height. THE
   SEVEN SMALL ONES are between a fifth and two fifths of its size, ALL DIFFERENT SIZES, tilted
   at different angles, some overlapping, the further ones smaller and fainter. No neat grid,
   no straight row, no two the same size.

2. EVERY SCREEN SHOWS TEETH - AND MOST SHOW HOW TEETH ARE MOVED. All screen content is GLOWING
   PALE CYAN-WHITE LINE ART (#bcdcff), big and simple:
   • MAIN SCREEN: A DENTAL ARCH SEEN FROM ABOVE - a horseshoe of TWELVE teeth. THREE ARE
     PLAINLY OUT OF LINE: one rotated sideways, one tipped over, one pushed inwards. TWO THICK
     SHORT CURVED ARROWS show which way two of them must travel.
   • THREE SMALL SCREENS ARE CLOSE-UP DIAGRAMS OF TEETH BEING MOVED, from the reference
     photographs of orthodontic mechanics: FOUR OR FIVE LARGE TEETH in a row, each with a SMALL
     SQUARE BRACKET on it and A SINGLE WIRE THROUGH ALL THE BRACKETS. On one, the wire has a
     clear U-SHAPED OR T-SHAPED LOOP bent into it; on another, ONE TOOTH SLIDES ALONG THE WIRE
     with a straight arrow behind it; on the third, TWO CURVED ARROWS around one tooth show it
     rotating upright. Brackets and wire drawn LARGE AND SIMPLE.
   • THE OTHER FOUR: a SIDE PROFILE of a face in glowing line (no skull, no eye socket); UPPER
     AND LOWER TEETH FROM THE SIDE with the upper front teeth clearly sticking out; a SECOND
     ARCH, simpler; and ONE SINGLE TOOTH in three-quarter view with a rotation arrow.
   ⚠ Extra marks are PURELY GEOMETRIC - tick marks, small round nodes, one angle with an arc,
   corner brackets. THERE ARE NO LETTERS, NO NUMBERS, NO WORDS AND NOTHING RESEMBLING WRITING
   ANYWHERE. Every screen has teeth or a face on it.

3. SLOW LIGHT STREAKS CURVE THROUGH THE AIR AROUND HIM - THIS IS WHAT MAKES THE PICTURE MOVE.
   THREE OR FOUR long, thin streaks of cyan-white light (#dff0ff) arc slowly between the
   screens and around his shoulders, passing behind and in front of him: each streak is a small
   bright head with a SOFT TAPERING TAIL AT LEAST AS LONG AS HIS HEAD IS WIDE, curving gently
   like a slow comet. THEY ARE FEW, LARGE AND CALM - never a spray of dots, never sparkles,
   never confetti, never glitter, never a cloud of particles, and they never cross his face.

4. HE IS WORKING TWO SCREENS AT ONCE, WITH BOTH GLOVED HANDS. BOTH ARMS ARE RAISED, ELBOWS
   BENT, BOTH FULLY VISIBLE AND BOTH CONNECTED TO HIS BODY - NEITHER ARM HANGS IDLE AT HIS
   SIDE:
   • HIS NEAR HAND is on the main screen: thumb and index finger PINCHING one of the crooked
     teeth and sliding it along the horseshoe towards the empty place where it belongs, with a
     SHORT SOFT TRAIL OF LIGHT behind that tooth. The tooth REMAINS part of the glowing line
     art - he does NOT pull anything out and NO solid tooth floats anywhere.
   • HIS OTHER HAND is on a smaller screen beside him, fingers spread and palm flat against the
     glass, SWEEPING IT UPWARDS AND ASIDE so that screen tilts and lifts away from him.
   AT HIS FINGERTIPS THERE IS A SMALL BRIGHT GLOW: a soft cyan-white halo (#dff0ff) no bigger
   than a fingertip on both hands, brightest where the fingers touch the glass. THE GLOW IS
   ONLY AT THE FINGERTIPS, ON THE SCREENS AND IN THE STREAKS - no beams, no light on the walls
   or floor, and his face keeps its warm daylight colour, never blue.

5. THE DENTIST - CONFIDENT AND IN COMMAND. A Taiwanese man in his late thirties, clean-shaven,
   drawn with the minimal face above. HIS SHORT HAIR IS A DEEP BLUE-BLACK (#2a3342, catching
   light at #3c4a5e) - clearly cool and blue-tinged, the same way the endodontist in the
   reference illustration has a warm red-brown tint in hers. HE WEARS A LONG WHITE DOCTOR\'S
   COAT REACHING BELOW HIS KNEES, open at the front, over BLUE SCRUBS (#6a94c4) - V-neck top
   and matching trousers - and CLOSE-FITTING GLOVES IN THE DEEPER CLINIC BLUE (#4478b5) with a
   soft rolled cuff; the gloves are MORE SATURATED than the scrubs, plain, with no pattern, no
   wires, no sensors, no lights. He stands on the LEFT, turned three-quarters towards the
   screens, feet planted, shoulders level and open, calm and unhurried - a specialist doing
   what he does every day. NOT a magician, NOT a performer. HIS EYES ARE ON THE TOOTH HE IS
   MOVING; his mouth is slightly open, explaining as he works.
   THERE IS NO HAND ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A VISIBLE PERSON.

6. THE PATIENT IS TAKING PART, NOT JUST WATCHING. A Taiwanese man of about forty - an ordinary
   working adult, NOT a teenager - sitting on a round stool on the RIGHT, LEANING WELL FORWARD
   FROM THE HIPS towards the screens. ONE FOREARM RESTS ON HIS THIGH taking his weight; HIS
   OTHER ARM IS RAISED AND HIS INDEX FINGER POINTS UP AT ONE PARTICULAR TOOTH on the main
   screen - he is in the middle of asking about that tooth. HIS MOUTH IS SLIGHTLY OPEN, his
   eyebrows lifted a little, his eyes bright and on the screen: interested, hopeful, taking
   part. He is NOT anxious, NOT frowning, NOT astonished, NOT laughing.
   HIS CLOTHES HAVE LAYERS AND COLOUR: an OPEN OCHRE-TERRACOTTA KNITTED CARDIGAN (#b6764a)
   over a PALE SAGE COLLARED SHIRT (#d8ddd0), with DARK SLATE TROUSERS and simple shoes. He is
   NOT dressed in one flat beige colour.

7. THE NURSE IS FAR BACK ON THE RIGHT, SORTING THE RECORDS, AND SHE IS NOT LOOKING AT THEM.
   Small because she is distant, but drawn with exactly the same line weight and the same
   minimal face: she stands side-on in front of HER OWN CLUSTER OF FOUR SMALL FLOATING SCREENS
   - one hand sliding one screen aside, the other resting on a second, two more already stacked
   behind them like filed cards. She wears the same blue scrubs. Absorbed in her own task. TWO
   SEPARATE PIECES OF WORK HAPPENING AT ONCE, nobody posing for us.

8. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

EVERYONE LOOKS DIFFERENT: the three people differ clearly in age, build, hair and clothing, and
no two faces are alike. EVERY PERSON IS DRAWN WITH EXACTLY THE SAME LINE WEIGHT AND SOLIDITY,
including the one furthest away.

THERE MUST BE NO LARGE EMPTY AREA ANYWHERE BELOW THE TOP STRIP - every part of the picture
carries a person, a screen with teeth on it, a light streak, or clearly drawn furniture (the
counter, its shadow gap, the light line, the floor reflections). Nothing is cramped: the
screens keep visible gaps between them.

THE TOP SIXTH OF THE PICTURE IS PLAIN BRIGHT WALL AND NOTHING ELSE - every screen, every glow,
every light streak, both people\'s heads, the counter, the light line and the nurse\'s screens
ALL STAY BELOW THAT STRIP. That strip must stay light and neutral-warm, never yellow.

AVOID: wood, wood grain, wooden cabinets, cabinet handles, panelled doors; a yellow, cream,
beige, tan or sandy wall; a visible blush; round, pink, orange or red cheeks; a nose with
nostrils, wings, a tip or shading; face shading, cheekbones or a jaw line; big-headed chibi
proportions; black outlines; thin even vector lines; a short waist-length white coat; one arm
hanging idle at his side; a patient sitting still with both hands on his knees; a patient
dressed in a single flat colour; sparkles, glitter, confetti, a spray of dots, a cloud of
particles, lens flare, light beams across the room; a dark or night-time room; blue light on
faces or walls; a 3D render, a photograph, neon cyberpunk; letters, numbers, words or anything
resembling writing on the screens; menus, buttons, sliders, charts, dashboards; a screen with
only abstract symbols and no teeth; a neat grid or straight row of equal screens; more than
eight screens; a solid three-dimensional tooth floating in the air; teeth with faces, eyes or
limbs; an arch of perfectly tidy teeth; pink gums; a skull, a skeleton or an X-ray photograph;
anything in the patient\'s hands; a floating hand or a forearm with no body; anyone waving or
looking at the camera; a large blank area of wall or floor.
```

⚠ 回圖要量四件：① 頂 17% 的 **B ≥ 225**（v10 只有 212，剛好在懸崖邊；牆偏黃就會掉）
② **邊緣密度 ≥ 22%**（這一版靠凹槽燈帶、長溝縫、地板反光、光弧撐）
③ 光弧是不是**三到四條大的**，不是一片小點 ④ 病患有沒有在指螢幕、衣服有沒有層次

---

## 五之十九、Ⓗ v5：使用者只剩一件（2026-08-24）

存檔 `drafts/og-topic-ortho-v11-streaks.jpg`。使用者：
> 「**差不多了**　病患坐在**有主題色（淡一點）診療椅**上　比較像牙科診間」

### 量測：兩件過關，只剩密度

| | v9 | v10 | **v11** | 門檻 |
| --- | --- | --- | --- | --- |
| 頂 17% 的 B | 235 | 212 ⚠ | **235** ✅ | ≥ 212（目標 ≥225） |
| 安靜區被佔 | 2.99% | 2.64% | **0.32%** ✅ | < 5% |
| 邊緣密度 | 20.9% | 18.7% | **18.2%** ❌ | ≥ 22 |

**診療椅正好是密度的解**：大件、有頭靠與扶手的分件輪廓、有底座與管件，
比圓凳多出十幾條線，而且它就在畫面右半那塊最空的地方。

### 椅子的顏色是算出來的，不是挑的

「淡一點的主題色」＝ 站上套色 `#4478b5` 往紙色 `#f4f4f5` 混：

| 混紙色 | 值 |
| --- | --- |
| 45% | `#93b0d2` |
| **55%（定）** | **`#a5bcd8`** |
| 65% | `#b6c9df` |

取 55% 那一格：夠淡（不會和深藍螢幕搶）、又留得住色相（一眼看得出和標籤同一支藍）。

⚠ **這一版要放寬一條自己下的 AVOID**：前幾版寫著 `NO dental chair`
（怕變成「治療中」的畫面）。現在改成**允許診療椅，但限定「椅背豎起、病人坐著、
不是在治療」** —— 沒有器械、沒有躺平、沒有圍兜、沒有燈照在嘴上。

## 五之二十、Ⓗ v6 的提示詞（診療椅版，2026-08-24）

⚠ 只改兩處：病患的座位換成**淡主題色的診療椅**，以及 AVOID 裡那條 `NO dental chair`
改成「不可以是治療中的畫面」。其餘一個字沒動。

```
Editorial illustration, landscape 1.91:1 (1200 x 628), for a small social-media preview
card. It will be seen at about 250 pixels wide, so everything must read at thumbnail size:
big simple shapes, one single continuous scene, no comic-strip panels.

THE STYLE COMES FROM THE REFERENCE ILLUSTRATIONS OF THE CLINIC. Match them exactly:
• LINE: every outline is drawn in WARM DARK BROWN INK (#4a3a30), NEVER black, never cold grey.
  Clearly hand-drawn - the thickness varies, it wobbles slightly, it breaks in places. NOT a
  thin even vector line.
• FACES ARE DRAWN WITH AS FEW MARKS AS POSSIBLE:
  - THE NOSE IS ONE VERY SHORT, VERY THIN LINE, SHORTER THAN AN EYE IS WIDE - no nostrils, no
    wings, no tip, no shading, no colour. IF IN DOUBT, DRAW LESS NOSE.
  - BLUSH IS ALMOST INVISIBLE: at most an edgeless warm tint on the cheekbone, a shade warmer
    than the skin. IT MUST NOT BE VISIBLE AT 250 PIXELS WIDE. Never a circle, never orange,
    never two round dots. IF IN DOUBT, LEAVE IT OUT.
  - Eyes are small dark dots, no whites, no pupils. Eyebrows two short strokes. Mouth one
    short line. Ear one small curve. Skin a flat pale warm ivory (#f6e6d8) with no modelling,
    no cheekbones, no jaw line.
  - COUNT THE MARKS: two eye dots, two eyebrow strokes, one tiny nose line, one mouth line,
    one ear curve. NOTHING ELSE.
• PEOPLE HAVE NORMAL ADULT PROPORTIONS, gently simplified - about seven heads tall. NOT chibi,
  NOT big-headed.
• FINE PAPER GRAIN over the whole image, flat colour, loose hand-drawn hatching for shadows.
Draw a futuristic subject in that warm, hand-drawn storybook style. NOT a 3D render, NOT a
photo, NOT neon cyberpunk, NOT a clean vector infographic.

THE ROOM IS A MODERN, CALM DENTAL CONSULTING ROOM - BRIGHT AND WARM BUT WITHOUT ANY WOOD.
THE BACK WALL IS A VERY LIGHT, ALMOST NEUTRAL WARM WHITE (#f4f2ee) - bright, barely warm, and
NEVER yellow, NEVER cream, NEVER beige, NEVER tan, NEVER sand. Along the back runs a SEAMLESS
PALE CABINET RUN WITH NO HANDLES AND NO DOOR PANELS - just one long dark shadow gap beneath a
matt stone-white countertop. ONE RECESSED LIGHT LINE glows softly along the wall above the
counter. The floor is a pale seamless resin floor with LONG SOFT HORIZONTAL REFLECTIONS,
meeting the wall in a continuous coved skirting. NO wood grain, NO wooden doors, NO cabinet
handles, NO panelled furniture, NO plants, NO posters. The room stays bright: the only cold
blue things are the floating screens and the gloves.

THE PICTURE IN ONE LINE: A CONFIDENT ORTHODONTIST STANDS INSIDE A CURVED CLOUD OF GLOWING
SCREENS SHOWING HOW TEETH ARE MOVED, WORKING TWO SCREENS AT ONCE WITH BOTH GLOVED HANDS, WHILE
HIS PATIENT SITS UP IN THE DENTAL CHAIR, LEANS IN, POINTS AT ONE TOOTH AND ASKS ABOUT IT.

THE EIGHT THINGS THAT MATTER MOST, IN ORDER:

1. THE SCREENS ARE MANY AND THEY CURVE AROUND HIM - ONE BIG, SEVEN SMALL. Panels of dark
   translucent blue glass (#2b4a72, the room faintly visible through them), each with ONE thin
   bright cyan-white edge line (#bcdcff), softly rounded corners and a gentle halo. They hang
   at different depths and angles, SWEEPING IN A WIDE ARC FROM HIS UPPER LEFT, ACROSS IN FRONT
   OF HIM, DOWN TO HIS LOWER RIGHT, so he stands INSIDE his own workspace. THE MAIN SCREEN is
   in front of him, slightly left of centre, about FIFTY PER CENT of the picture height. THE
   SEVEN SMALL ONES are between a fifth and two fifths of its size, ALL DIFFERENT SIZES, tilted
   at different angles, some overlapping, the further ones smaller and fainter. No neat grid,
   no straight row, no two the same size.

2. EVERY SCREEN SHOWS TEETH - AND MOST SHOW HOW TEETH ARE MOVED. All screen content is GLOWING
   PALE CYAN-WHITE LINE ART (#bcdcff), big and simple:
   • MAIN SCREEN: A DENTAL ARCH SEEN FROM ABOVE - a horseshoe of TWELVE teeth. THREE ARE
     PLAINLY OUT OF LINE: one rotated sideways, one tipped over, one pushed inwards. TWO THICK
     SHORT CURVED ARROWS show which way two of them must travel.
   • THREE SMALL SCREENS ARE CLOSE-UP DIAGRAMS OF TEETH BEING MOVED, from the reference
     photographs of orthodontic mechanics: FOUR OR FIVE LARGE TEETH in a row, each with a SMALL
     SQUARE BRACKET on it and A SINGLE WIRE THROUGH ALL THE BRACKETS. On one, the wire has a
     clear U-SHAPED OR T-SHAPED LOOP bent into it; on another, ONE TOOTH SLIDES ALONG THE WIRE
     with a straight arrow behind it; on the third, TWO CURVED ARROWS around one tooth show it
     rotating upright. Brackets and wire drawn LARGE AND SIMPLE.
   • THE OTHER FOUR: a SIDE PROFILE of a face in glowing line (no skull, no eye socket); UPPER
     AND LOWER TEETH FROM THE SIDE with the upper front teeth clearly sticking out; a SECOND
     ARCH, simpler; and ONE SINGLE TOOTH in three-quarter view with a rotation arrow.
   ⚠ Extra marks are PURELY GEOMETRIC - tick marks, small round nodes, one angle with an arc,
   corner brackets. THERE ARE NO LETTERS, NO NUMBERS, NO WORDS AND NOTHING RESEMBLING WRITING
   ANYWHERE. Every screen has teeth or a face on it.

3. SLOW LIGHT STREAKS CURVE THROUGH THE AIR AROUND HIM - THIS IS WHAT MAKES THE PICTURE MOVE.
   THREE OR FOUR long, thin streaks of cyan-white light (#dff0ff) arc slowly between the
   screens and around his shoulders, passing behind and in front of him: each streak is a small
   bright head with a SOFT TAPERING TAIL AT LEAST AS LONG AS HIS HEAD IS WIDE, curving gently
   like a slow comet. THEY ARE FEW, LARGE AND CALM - never a spray of dots, never sparkles,
   never confetti, never glitter, never a cloud of particles, and they never cross his face.

4. HE IS WORKING TWO SCREENS AT ONCE, WITH BOTH GLOVED HANDS. BOTH ARMS ARE RAISED, ELBOWS
   BENT, BOTH FULLY VISIBLE AND BOTH CONNECTED TO HIS BODY - NEITHER ARM HANGS IDLE AT HIS SIDE:
   • HIS NEAR HAND is on the main screen: thumb and index finger PINCHING one of the crooked
     teeth and sliding it along the horseshoe towards the empty place where it belongs, with a
     SHORT SOFT TRAIL OF LIGHT behind that tooth. The tooth REMAINS part of the glowing line
     art - he does NOT pull anything out and NO solid tooth floats anywhere.
   • HIS OTHER HAND is on a smaller screen beside him, fingers spread and palm flat against the
     glass, SWEEPING IT UPWARDS AND ASIDE so that screen tilts and lifts away from him.
   AT HIS FINGERTIPS THERE IS A SMALL BRIGHT GLOW: a soft cyan-white halo (#dff0ff) no bigger
   than a fingertip on both hands, brightest where the fingers touch the glass. THE GLOW IS
   ONLY AT THE FINGERTIPS, ON THE SCREENS AND IN THE STREAKS - no beams, no light on the walls
   or floor, and his face keeps its warm daylight colour, never blue.

5. THE DENTIST - CONFIDENT AND IN COMMAND. A Taiwanese man in his late thirties, clean-shaven,
   drawn with the minimal face above. HIS SHORT HAIR IS A DEEP BLUE-BLACK (#2a3342, catching
   light at #3c4a5e) - clearly cool and blue-tinged, the same way the endodontist in the
   reference illustration has a warm red-brown tint in hers. HE WEARS A LONG WHITE DOCTOR\'S COAT
   REACHING BELOW HIS KNEES, open at the front, over BLUE SCRUBS (#6a94c4) - V-neck top and
   matching trousers - and CLOSE-FITTING GLOVES IN THE DEEPER CLINIC BLUE (#4478b5) with a soft
   rolled cuff; the gloves are MORE SATURATED than the scrubs, plain, with no pattern, no
   wires, no sensors, no lights. He stands on the LEFT, turned three-quarters towards the
   screens, feet planted, shoulders level and open, calm and unhurried - a specialist doing
   what he does every day. NOT a magician, NOT a performer. HIS EYES ARE ON THE TOOTH HE IS
   MOVING; his mouth is slightly open, explaining as he works.
   THERE IS NO HAND ANYWHERE IN THIS PICTURE THAT DOES NOT BELONG TO A VISIBLE PERSON.

6. THE PATIENT IS SITTING UP IN A DENTAL CHAIR ON THE RIGHT, AND HE IS TAKING PART. THE CHAIR
   IS THE ONE PIECE OF FURNITURE THAT SAYS "DENTAL CLINIC": a proper dental treatment chair
   seen three-quarters from the front, ITS BACKREST RAISED UPRIGHT so he is sitting, not lying.
   ITS UPHOLSTERY IS A PALE, SOFT VERSION OF THE CLINIC BLUE (#a5bcd8) - clearly the same
   family as the gloves but much lighter - with a separate HEADREST, two low ARMRESTS, a
   rounded seat, a slim chrome-grey column and a wide flat base on the floor. Draw those parts
   as clearly separate shapes.
   THE PATIENT IS NOT BEING TREATED: no bib, no instruments, no suction tube, no operating
   light over him, nothing in his mouth, nobody working on him.
   A Taiwanese man of about forty - an ordinary working adult, NOT a teenager - sitting well
   forward on the chair, LEANING IN towards the screens. ONE FOREARM RESTS ON THE ARMREST
   taking his weight; HIS OTHER ARM IS RAISED AND HIS INDEX FINGER POINTS UP AT ONE PARTICULAR
   TOOTH on the main screen - he is in the middle of asking about that tooth. HIS MOUTH IS
   SLIGHTLY OPEN, eyebrows lifted a little, eyes bright and on the screen: interested, hopeful,
   taking part. He is NOT anxious, NOT frowning, NOT astonished, NOT laughing.
   HIS CLOTHES HAVE LAYERS AND COLOUR: an OPEN OCHRE-TERRACOTTA KNITTED CARDIGAN (#b6764a) over
   a PALE SAGE COLLARED SHIRT (#d8ddd0), with DARK SLATE TROUSERS and simple shoes. He is NOT
   dressed in one flat beige colour.

7. THE NURSE IS FAR BACK ON THE RIGHT, SORTING THE RECORDS, AND SHE IS NOT LOOKING AT THEM.
   Small because she is distant, but drawn with exactly the same line weight and the same
   minimal face: she stands side-on in front of HER OWN CLUSTER OF FOUR SMALL FLOATING SCREENS
   - one hand sliding one screen aside, the other resting on a second, two more already stacked
   behind them like filed cards. She wears the same blue scrubs. Absorbed in her own task. TWO
   SEPARATE PIECES OF WORK HAPPENING AT ONCE, nobody posing for us.

8. NO WRITING ANYWHERE in the image, in any language, and NO NUMBERS.

EVERYONE LOOKS DIFFERENT: the three people differ clearly in age, build, hair and clothing, and
no two faces are alike. EVERY PERSON IS DRAWN WITH EXACTLY THE SAME LINE WEIGHT AND SOLIDITY,
including the one furthest away.

THERE MUST BE NO LARGE EMPTY AREA ANYWHERE BELOW THE TOP STRIP - every part of the picture
carries a person, a screen with teeth on it, a light streak, the dental chair, or clearly drawn
furniture (the counter, its shadow gap, the light line, the floor reflections). Nothing is
cramped: the screens keep visible gaps between them.

THE TOP SIXTH OF THE PICTURE IS PLAIN BRIGHT WALL AND NOTHING ELSE - every screen, every glow,
every light streak, both people\'s heads, the chair, the counter, the light line and the
nurse\'s screens ALL STAY BELOW THAT STRIP. That strip must stay light and neutral-warm, never
yellow.

AVOID: a patient lying back being treated; a bib, suction tube, dental instruments or an
operating light pointed at anyone; wood, wood grain, wooden cabinets, cabinet handles, panelled
doors; a yellow, cream, beige, tan or sandy wall; a visible blush; round, pink, orange or red
cheeks; a nose with nostrils, wings, a tip or shading; face shading, cheekbones or a jaw line;
big-headed chibi proportions; black outlines; thin even vector lines; a short waist-length
white coat; one arm hanging idle at his side; a patient sitting still with both hands on his
knees; a patient dressed in a single flat colour; a bright saturated blue chair; sparkles,
glitter, confetti, a spray of dots, a cloud of particles, lens flare, light beams across the
room; a dark or night-time room; blue light on faces or walls; a 3D render, a photograph, neon
cyberpunk; letters, numbers, words or anything resembling writing on the screens; menus,
buttons, sliders, charts, dashboards; a screen with only abstract symbols and no teeth; a neat
grid or straight row of equal screens; more than eight screens; a solid three-dimensional tooth
floating in the air; teeth with faces, eyes or limbs; an arch of perfectly tidy teeth; pink
gums; a skull, a skeleton or an X-ray photograph; anything in the patient\'s hands; a floating
hand or a forearm with no body; anyone waving or looking at the camera; a large blank area of
wall or floor.
```

⚠ 回圖要量：① **邊緣密度 ≥ 22%**（椅子的頭靠、扶手、柱子、底座是這一版新增的線）
② 頂 17% 的 **B ≥ 225** ③ 椅子是不是**豎起椅背、病人坐著、不是治療中**
④ 椅子的藍**比手套淡**（不可以搶戲）

---

## 六、成品做出來了（2026-08-24）

使用者選了 **v11 那一張**（`drafts/og-topic-ortho-src.jpg`，圓凳版，不是後來加診療椅那版），
指定「做成 LINE 訊息顯示圖示」。成品 **`assets/og-topic-ortho.jpg`**，
LINE 模擬在 `drafts/ortho-line-mock-paper.png`（紙色字）與 `-white.png`（純白字）。

### 管線（這一張實際跑的三行）

    node drafts/og-topfill.mjs drafts/og-topic-ortho-src.jpg drafts/og-topic-ortho-src-filled.jpg \
      --rows 120 --color 235,234,228
    node tools/og-resize.mjs drafts/og-topic-ortho-src-filled.jpg ortho
    node tools/og-plate.mjs ortho --blend multiply --tintcolor "#5095ed" \
      --ink 0.18 --blur 6 --loc full --locpos stack
    node tools/topics.mjs     # ← og:image 要重產才會指到新圖
    node drafts/line-mock.mjs ortho

### ⚠⚠ 教訓一：上緣的白框要用「填色」不是「外插」

`og-resize` 的白框守門攔下這張（**上緣 118 列純白**）。第一次我用
`drafts/og-topclean.mjs`（顯微根管那次寫的），結果**安靜區被佔從 0.32% 惡化到 38.43%** ——
那一支是**逐欄往上外插**，而這張圖中央那幾欄的下面是**深藍螢幕**，
於是把螢幕的深藍一路外插到頂端。

**分工寫進 `drafts/og-topfill.mjs` 的檔頭**：
・`og-topclean` ＝ 逐欄外插，只有「那一欄下面就是牆」才成立（顯微根管的顯微鏡臂）。
・`og-topfill` ＝ **整條填成同一個牆色**，適用於「上緣本來就是一條均勻的白框」（這一張）。
⚠ 填的列數要小於「帶子換算回原圖的高度」（這張是 123 列，填了 120），接縫才藏得住。

### ⚠ 教訓二：補償色算對了，帶子逐格命中站上的套色

牆填成 `rgb(235,234,228)` → 算出補償色 **`#5095ed`** →
帶子玻璃實測 **rgb(68,120,180)**，站上的套色是 `#4478b5` ＝ rgb(68,120,**181**)，
**只差 1 階**。而且因為牆是填出來的、完全均勻，**這是七科裡最均勻的一條帶子**
（中位與 p95 都是 3.61，其他幾科的中位與最亮處都有落差）。

### 驗收數字

| | 這一張 | 門檻／對照 |
| --- | --- | --- |
| 頂 17% 的 B 通道 | **229** ✅ | ≥ 212 |
| 安靜區被佔 | **0.44%** ✅ | < 5% |
| 無彩空白 | 0.4% ✅ | < 5% |
| 邊緣密度 | **20.3%** ⚠ | 已上線四張是 21.9~32.4 |
| 帶子安全區 | 左右各 160px ✅ | ≥ 160（LINE／iMessage 的裁切） |
| 紙色字對帶子 | **3.61**（中位＝最亮處） | 牙周 4.40／顯微根管 4.06／兒牙白字 4.95 |
| 純白字對帶子 | **4.57** | — |

⚠ **密度 20.3% 比已上線四張的下限低 1.6 個百分點** —— 成因是這張圖的
牆與地板佔了很大面積而且很平。使用者已經指定用這一張，**沒有重生成**，如實記錄。

### 還沒決定的一件

**帶子上的字要紙色（3.61）還是純白（4.57）** —— 兩版的 LINE 模擬都做好了。
判準（ILLUSTRATION.md 第十一節）：暖色底用純白、冷色與深色用紙色 → 矯正是冷色，**預設紙色**；
但 3.61 是七科第二低，所以做成實際的卡片讓使用者挑（同兒牙那次）。
目前 `assets/og-topic-ortho.jpg` 裡是**紙色字**那一版。
