...
// ⚠ Ⓕ 的風格是靠參考圖給的（第三之二節），那一段掉了整案就退回文字描述風格。
{
  ...
  // ⚠⚠⚠ 第七版加的：治「顏色單調」與「窗戶像監獄鐵窗」。前者是第六版自己改壞的。
  if (!/SEVEN BACKGROUNDS ARE SEVEN DIFFERENT COLOURS/.test(f.prompt)) throw new Error("Ⓕ 少了七個底色逐格指名那一段");
  if (!/NEVER AS A PRISON/.test(f.prompt)) throw new Error("Ⓕ 少了『窗戶不可以像監獄』那一段");
  if (!/THE GUTTER IS A LINE, NOT A BAND/.test(f.prompt)) throw new Error("Ⓕ 少了『格線是線不是白帶』那一句");
  if (/barred/.test(f.prompt)) throw new Error("Ⓕ 又出現 barred window（那個字本身就在叫它畫柵欄）");
  if (/creamiest background/.test(f.prompt)) throw new Error("Ⓕ 又出現第六版那句『最米白的底』（會把七個底色統一成米白）");
}
...
const NOTES = {
  ...
  "Ⓕ": {
    ...
    guards: [
      "⚠⚠⚠ <b>第七版（09-05 更晚）治的是「顏色變單調」與「窗戶像監獄鐵窗」，而顏色那一件又是我第六版改壞的</b>：第六版為了讓顏色跟著位置重排，補了一句「<code>and that panel keeps the palest, creamiest background of the seven</code>」——<b>那是整份提示詞裡唯一一句真的講到「背景是什麼顏色」的話，而它說的是 cream</b>。在那之前背景只有一句很虛的「各給一格一個淡淡的色調」，七個底色<b>從來沒有被指名過</b>；第五版有七個不同的底，是靠那一串<b>衣服</b>的顏色順帶帶開的。⚠⚠ <b>通則：一份規格裡「唯一具體的那一條」會變成全場的規則</b> —— 一個維度只有一句虛的描述時它其實沒有被指定，是靠別的東西順帶撐著的。第七版把<b>底色與衣服拆成兩份清單</b>、七格逐格指名，並收一句判準：<b>有人瞄一眼，要能單靠顏色分出這一格和隔壁不是同一格。</b>",
      "⚠⚠⚠ <b>窗戶：用字本身就在叫它畫監獄</b> —— 兩處寫的都是 <code>a barred Taiwanese window</code>，而 <code>barred</code> 就是「有柵欄的」；第六版還把其中一扇搬進了<b>最大的中間那一格</b>（孕婦本來在左上，窗戶跟著她搬過去），於是被畫得又大又深。新增一整段：<b>淺色、細、和其他家具同一個線寬、簡單的方格或曲線花樣、只用幾筆</b>，四條紅線（不可以粗／深色／一排沒有橫向連接的直條／一整片密格子），並明寫<b>先看到的是透進來的日光</b>。⚠ <b>通則：一個名詞如果它的字面意思就是那個壞結果，換掉那個名詞，不要在旁邊加形容詞。</b>順手還修了第三件（使用者沒提）：<b>格線從細的深色線變成很寬的白帶</b>，也在把整張圖洗淡 —— 補上「格線是一條線不是一條帶」。",
...
<p><b>2026-09-05 更晚：Ⓕ 第七版</b> —— 治「顏色變單調」與「窗戶像監獄鐵窗」。
⚠ 第六版那張出圖沒有存進容器裡，<b>所以這一輪沒辦法逐像素量它</b>；
成因是<b>逐行 diff 提示詞</b>找出來的，不是從畫面猜的。</p>
<p><b>① 顏色那一件，又是我第六版改壞的。</b>為了讓顏色跟著位置重排，
我把那一句改成「…<code>and that panel keeps the palest, creamiest background of the seven</code>」——
<b>那是整份提示詞裡唯一一句真的講到「背景是什麼顏色」的話，而它說的是 cream</b>。
在那之前背景只有一句很虛的「各給一格一個淡淡的色調」，
七個底色<b>從來沒有被指名過</b>；第五版能有七個不同的底，是靠那一串<b>衣服</b>的顏色
順帶把每一格帶開的。我把那一串改成純粹的衣服清單、又補了一句具體的「最米白」，
<b>唯一具體的那句就變成全場的規則</b>。</p>
<p>⚠⚠ <b>通則：一份規格裡「唯一具體的那一條」會變成全場的規則。</b>
一個維度如果只有一句虛的描述，它其實沒有被指定 —— 它是靠別的東西順帶撐著的，
哪天那個東西被改了就會整組垮掉。<b>要嘛逐格寫死，要嘛不要在旁邊擺一句具體的。</b>
第七版把<b>底色與衣服拆成兩份清單</b>，七格逐格指名（中間淡玫瑰／左上淡芥末金／
右上暖奶油黃／右淡岩藍／右下柔鼠尾草綠／左下夜色藍灰／左淡青綠），
並收一句判準：<b>有人瞄一眼，要能單靠顏色分出這一格和隔壁不是同一格。</b></p>
<p><b>② 窗戶 —— 用字本身就在叫它畫監獄。</b>兩處寫的都是 <code>a barred Taiwanese window</code>，
而 <code>barred</code> 就是「有柵欄的」；第六版還把其中一扇搬進了<b>最大的中間那一格</b>
（孕婦本來在左上，窗戶跟著她一起搬過去），於是被畫得又大又深。
新增一整段規格：<b>淺色、細、和其他家具同一個線寬、簡單的方格或曲線花樣、只用幾筆</b>，
四條紅線 —— <b>不可以粗、不可以深色、不可以是一排沒有橫向連接的直條、不可以是一整片密格子</b>。
⚠ <b>通則：一個名詞如果它的字面意思就是那個壞結果，換掉那個名詞，不要在旁邊加形容詞。</b></p>
<p><b>③ 順手找到第三件（你沒有提，是我看畫面看到的）</b>：
格線從細的深色線變成<b>很寬的白帶</b>，那幾條白帶自己就佔掉不少面積，也在把整張圖洗淡。
原本只寫「細的手繪格線」，沒有說<b>它是一條線不是一條帶</b> ——
補上：格線是一條深色的手繪線，粗細不超過人物的輪廓線，永遠不是一條白色的寬縫。</p>
<p>Ⓐ~Ⓔ 留著沒有動，往下捲還在。</p>
</div>
