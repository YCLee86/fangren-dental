...
字色改成**純白 ＋ 地名 .9**，並把四個候選放進 LINE 訊息裡讓使用者選
（`node drafts/line-mock.mjs kids`，卡片 212px、圖照 LINE 的裁法只留中央 89.7%）：

| | 帶子 | 字 | 主名／地名 | |
| --- | --- | --- | --- | --- |
| Ⓐ | 深階 `#9e6301` | 紙色 | 3.91／2.49 | ＝「灰灰的」那張 |
| Ⓑ | 深階 `#9e6301` | 純白 | 4.95／4.33 | 我推薦的 |
| **Ⓒ** | **套色 `#c28229`** | **純白** | **3.23／2.93** | **← 使用者選這個並指定上線** |
| Ⓓ | 更深 `#8a5601` | 純白 | 6.23／5.35 | 最好讀但偏褐 |

**定案：Ⓒ —— 帶子落在站上兒牙標籤那顆套色 `#c28229`，字純白。**

    node tools/og-resize.mjs drafts/og-topic-kids-src.jpg kids
    node tools/og-plate.mjs kids --blend multiply --tintcolor '#f1a331' --ink 0.18 --blur 6 \
      --loc full --locpos stack --fg '#ffffff' --locop 0.9

⚠⚠ **已知取捨，不要拿對比度來「訂正」它**（同牙周那張的 4.40、兒牙填色上的白字 3.22）：
使用者看過四格的實際樣子之後選了 Ⓒ，理由是**帶子的顏色要等於站上那顆標籤**。
主名 3.23、地名 2.93 都在站上的 4.5 之下 —— 這是他知情之下的選擇，不是漏看。
⚠ 純白字這一項仍然成立：Ⓒ 如果用紙色字只有 2.53，白字把它拉到 3.23。
