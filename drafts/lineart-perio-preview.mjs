...
[data-topic="perio"] .tp-intro::before {
  ...
  /* 「裁右」是用 CSS 模擬的：框變窄、圖照高度撐滿、靠左對齊 →
     右邊那一段被框切掉，**比例尺不變**，所以人物只是往右移，不會變大。
     定案時是真的重裁 PNG（--crop 的 x 往右挪），頁面這一側就回到單純的 contain。 */
  width: calc(var(--pv-w, min(76%, 360px)) * var(--pv-k, 1));
  aspect-ratio: calc(1024 - var(--pv-cut, 0)) / 755;
...
<div class="pv-bar">
  ...
  <div class="pv-row pv-fine" data-k="w" hidden>
  ...
  function meas() {
    ...
    if (imgOK && fw > 0) {
      try {
        ...
        /* 和 CSS 同一個做法：照高度撐滿、靠左，右邊被框切掉 */
        g.drawImage(img, 0, 0, cv.height * img.naturalWidth / img.naturalHeight, cv.height);
    ...
    var txt = "視窗 " + Math.round(innerWidth) + "×" + Math.round(innerHeight) +
      ...
      "　裁右 " + cutPx + "（往右 " + shift.toFixed(0) + "px）" +
      "　濃度 " + al.toFixed(2) +
      "\\n壓到線的字 " + hit + "／" + lines + " 行";
    if (hit) {
