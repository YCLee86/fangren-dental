...
| **`drafts/facade-side-ref.jpg`**（1600×1384，側拍） | 三層樓＋頂上一層鋼構斜屋頂；整排是連棟 | 側面，正面拍不到 |
| **`drafts/facade-3f-ref.jpg`**（1600×1200，**施工期間拍的正面**） | **三樓立面的唯一依據** | 三層一模一樣的柱樑格子＋紅磚嵌板＋每格一扇窗；⚠ 那是還沒完工的樣子，**只取立面關係，不要照抄裸露的洞與鋼筋** |
...
## 十四、第九輪（2026-08-20）—— 往後退，把房子兩邊帶回來

第八輪使用者說「好很多了」，剩一件：

> 「你看第一版，房子的兩邊是保留比較多的，但是到這一版房子兩邊被裁切掉更多，
> 　可不可以把那兩邊帶進來一些，不要裁切那麼多。」

### 1. ⚠ 每一輪都在悄悄推近 —— 因為沒有人在管取景範圍

對照第一輪與第八輪那兩張：門的寬度從**畫面寬的一成**長到**一成三**，
左右各少掉一個開間。成因是**前八輪的提示詞從來沒有寫「畫面裡要看得到多寬」** ——
`COMPOSITION` 只管上下（三樓一條、底下五分之一空路面），左右是模型自由發揮，
而每一輪我又不斷加人、加互動、加細節，模型就自動往前推近好把它們畫清楚。

⚠ **通則：上下有規格、左右沒有，畫面就會一輪比一輪近。** 取景要**四邊都寫**。

### 2. 用「數得出來的開間」寫，不要用「退遠一點」

第九輪新增一段 `FRAMING`，把整排從左到右**逐項列出來讓它數**：

> 遮陽棚｜熱水器與電表箱｜窗｜深色窄門｜窗｜**木門**｜窗｜深色窄門｜窗｜隔壁單元的開頭

再補兩個比例當保險：**木門只佔畫面寬度的十分之一**、
**人行道上最高的大人約佔畫面高度的三分之一**。

⚠ 連帶要放寬一條：**三樓那條帶子從「上面七分之一」改成「最多五分之一」** ——
平立面往後退是等比例的，橫向多看到多少，縱向就會多看到多少，
不放寬的話這兩條會互相打架（就是第八輪騎士那個「近景 vs 貼著緣石」的同一種錯）。

### 3. 這一輪的取捨

人會變小（最高的大人從畫面高度的四成多降到約三分之一）。**這是使用者要的** ——
他要的是「這棟房子」，人是讓它活起來的東西。臉還是看得清楚，
因為手機上是滿寬顯示、電腦上也只裁上下不裁左右。

### 提示詞（第九輪完整版）

⚠ 餵三張：`facade-yongle-ref.jpg`、`facade-side-ref.jpg`、**第八輪那張圖**
（「所有東西都對了，**只改取景：往後退一點，把房子左右兩邊帶回來**」）。
...
FRAMING — STAND FURTHER BACK AND SHOW MORE OF THE BUILDING. The whole width of the
photographed frontage must fit inside the picture with room to spare at both sides. Reading
from left to right you should be able to count all of this: the neighbour's striped awning at
the far left edge, then the boxy water heater and meter boxes, a window, a narrow dark metal
door, a window, THE WOODEN ENTRANCE, a window, a second narrow dark metal door, a window, and
the start of the next unit running off the right edge. Upstairs the matching row of brick
panels and windows spans that same width, with the slid-open window directly above the
entrance. Because the view is this wide, THE OPEN WOODEN ENTRANCE IS ONLY ABOUT ONE TENTH OF
THE IMAGE WIDTH — it is the focus because of the warm light and the people around it, not
because it is large — and the tallest adult standing on the pavement is only about one third
of the image height. Do NOT crop in tight around the doorway, and do NOT fill the frame with
only three or four window bays.

COMPOSITION — the open wooden doorway sits SLIGHTLY RIGHT OF CENTRE. This banner will be
cropped along the top and the bottom, so:
  • the TOP BAND holds the cut-off third floor and nothing else that matters — now that the
    view is wider it may be up to a fifth of the height deep;
  • the middle holds everything important: the open upstairs window and the two people at
    it, the open door, every face, the whole of every person and every vehicle;
  • THE LOWEST FIFTH OF THE IMAGE IS EMPTY ASPHALT ROAD — nothing in it but road surface and
    soft shadows. THIS EMPTY BAND IS DELIBERATE; do not fill it, do not put anybody in it,
...
AVOID — cropping in close so that only three or four window bays are visible; the wooden
entrance taking up more than an eighth of the image width; a zoomed-in view that loses the
water heater on the left or the far window on the right; two people leaning out of the same
window; anybody's head, shoulder or arm passing
through a closed pane of glass; an upstairs window drawn with no glass in it at all; an
upstairs floor of identical dark empty windows with nobody in them; the people at the window
facing the viewer, waving at us, or drawn as a posed portrait; a detailed interior or any
equipment visible through the windows; clinic staff gathered together, lined up, posing, or
...
22. **取景**：從左到右數得出十個開間嗎（遮陽棚｜熱水器｜窗｜窄門｜窗｜木門｜窗｜窄門｜窗｜隔壁）、
    木門是不是只佔畫面寬的十分之一。


---

## 十五、第十輪（2026-08-20）—— 退不遠的真正原因，與三樓的正面依據

使用者：「這個提示詞我畫兩次都沒有把視角再拉遠，然後三樓的立面反而還跑掉了。
我現在放另外一張有三樓的立面，但這是施工期間拍的，所以只是表現出那個立面感而已。」

### 1. ⚠⚠ 退不遠的原因很可能是**我叫他餵上一張插畫**

第九輪要他附三張，其中一張是**上一輪的成品**。那張的用途本來是「維持畫風與人物」，
但**圖生圖的參考圖同時也帶著構圖** —— 模型看到一張已經完成的、取景很近的圖，
文字說「退遠一點」、圖片說「就是這樣」，**圖片會贏**。

**兩個解法，先試第一個：**

1. **提示詞開頭新增一段 `HOW TO USE THE ATTACHED PICTURES`**，明講：
   照片決定建築**與取景寬度**；如果有附插畫，**只取畫風、顏色與人物，不要抄它的取景**，
   而且**這一張一定要比那張退得更遠**。
2. 還是退不遠的話：**這一輪不要附上一張插畫**，只附兩張照片。
   代價是畫風與人物可能會變，但取景一定會照照片走。

⚠ 通則：**參考圖會同時帶走「畫風」和「構圖」兩件事，只想要其中一件就要明講。**

### 2. 取景改成「照著照片數一遍」

第九輪寫的是「往後退、木門佔十分之一」—— 那是**要模型自己判斷**。
第十輪改成**逐項清點、而且錨在那張照片上**：

> 你的畫面至少要涵蓋那張照片拍到的全部：遮陽棚｜熱水器與電表｜窗｜窄門｜窗｜
> **木門**｜窗｜窄門｜窗｜隔壁的開頭；二樓在同一段寬度裡要數得出**四扇大窗**
> ＋門正上方那扇**小窗**。**數不完就是站太近，再退。**

### 3. 三樓終於有正面依據了

使用者提供施工期間的正面照，存成 `drafts/facade-3f-ref.jpg`。
看得到的是**三層完全一樣的柱樑格子**：淡灰混凝土柱與深樑、之間嵌紅磚、每格一扇窗，
上下樓層的窗**對齊**。第十輪把這段寫死，並註明**那是還沒完工的樣子**——
只取立面關係，不要照抄裸露的洞、鋼筋與污漬。

### 4. 順手修一件我在圖上看到的

第九輪其中一張的白線變成**一條橫跨整張的大彩虹拱**，那不是氛圍線是裝飾拱。
新增規格：**每一條長弧都要接近水平、只是橫向飄過去，不准隆成拱形或彩虹，
也不准有任何一條從畫面一角拉到另一角。**
另外那張的女學生旁邊長出一個「…」的對話泡泡 —— `AVOID` 補上
「用點或破折號代表說話」。

### 提示詞（第十輪完整版）

⚠ 餵**兩張照片**：`facade-yongle-ref.jpg`（正面立面）、**`facade-3f-ref.jpg`（三樓立面）**。
`facade-side-ref.jpg` 這一輪可以不附（側面容易讓它畫出透視）。
**第三張（上一輪的插畫）先附**，並在對話裡加一句
「只參考畫風與人物，取景要比它更遠」；如果出來的還是一樣近，**第二次就把插畫拿掉**。
...
HOW TO USE THE ATTACHED PICTURES — the PHOTOGRAPHS decide the building and, above all, HOW
WIDE THE VIEW IS. If an earlier illustration is also attached, take from it ONLY the drawing
style, the colours and the characters: DO NOT copy its framing. THIS NEW PICTURE MUST BE
DRAWN FROM FURTHER BACK THAN THAT ILLUSTRATION — noticeably more of the building must fit
inside the frame at BOTH sides, and everything in it is correspondingly smaller.
...
seen almost straight on, flat elevation, no dramatic wide-angle perspective.
  ...
  • this is a terraced row: the same facade repeats in the neighbouring units and runs off
    both edges of the picture.
THE BUILDING IS THREE STOREYS TALL, and the third floor is IDENTICAL to the second, as the
second attached photograph (taken while the building was being renovated) shows: the same
grid of pale grey concrete columns and deep beams, the same panels of red brick filling the
wall between them, and the same dark aluminium window in each bay, lined up exactly above the
windows below. The bay above the entrance carries a brick panel and a small window, just like
...
FRAMING — MATCH THE WIDTH OF THE REFERENCE PHOTOGRAPH, THEN GO A LITTLE WIDER. The
photograph of the frontage is a wide, straight-on elevation, and your picture must take in AT
LEAST everything it shows. Count them off as you draw, left to right:
  the neighbour's striped awning at the far left edge;
  the tall boxy water heater and the meter boxes on the wall;
  a ground-floor window;
  a narrow dark metal door;
  a ground-floor window;
  THE WOODEN ENTRANCE, standing open;
  a ground-floor window;
  a second narrow dark metal door;
  a ground-floor window;
  the beginning of the next unit running off the right edge.
Upstairs, across that same width, you must be able to count FOUR large windows with brick
panels between them PLUS the small central window directly above the entrance. If you cannot
fit all of that in, you are standing too close — step back further.
Because the view is this wide, THE OPEN WOODEN ENTRANCE IS ONLY ABOUT ONE TENTH OF THE IMAGE
WIDTH, and the tallest adult standing on the pavement is only about ONE THIRD of the image
height. The entrance is the focus because of its warm light and the people around it, not
because it is large.
...
people. They vary in length and weight; some run almost the full width, some are short, some
break into a dashed run of flecks. They travel the same general direction, never cross, never
loop, never close into a shape, and never read as smoke, steam or a spirit. Each arc is
SHALLOW — nearly horizontal, drifting sideways across the picture. NOT ONE of them rises into
a tall dome, a rainbow or an arch over the building, and no single line spans the whole
picture from corner to corner. None of them crosses the open upstairs window or the two
people at it.
(B) SMALL MOTION AND SOUND MARKS — the light shorthand a comic uses for movement and
cheerful noise, at exactly these points:
  • beside the waving assistant at the open half of the upstairs window: a small cluster of
...
AVOID — copying the framing of the attached illustration instead of stepping back; a view
that shows fewer windows and doors than the reference photograph; cropping in close so that
only three or four window bays are visible; the wooden
entrance taking up more than an eighth of the image width; a zoomed-in view that loses the
water heater on the left or the far window on the right; two people leaning out of the same
window; anybody's head, shoulder or arm passing
through a closed pane of glass; an upstairs window drawn with no glass in it at all; an
...
crowd, a queue, a market with stalls, or more people than the ones listed; a complete third
floor, a roofline, a parapet, a steel roof canopy, a rooftop water tank or sky above the
building; speech balloons, thought bubbles, dots or dashes standing for speech, written sound effects,
stars, hearts or impact bursts; a tall arching white line like a rainbow over the building; comic panel borders; blood, pain, illness or distress; sunset, night, neon, heavy
shadow; rain; a Western or European street; wrinkles and heavy facial shading; smoke, steam
or spirit-like lines; text of any kind.
```
...
## 十七、標誌是**後製貼上去的**，不要叫模型畫（2026-08-20）

使用者：「用這個把診所 logo 加在插圖裡，一樓雙開大門左邊的綠牌子上，做成白色的。」

**做法：不生成，直接合成。** 理由是 ILLUSTRATION.md 第十之一節那一條的極端版 ——
標誌用文字描述失敗過三四輪，用參考圖才中；而這裡它只有 **28×14 px**，
模型在這個尺寸下幾乎一定畫歪，而且每重生成一次全圖都會再骰一次。
**後製貼上去的是原始向量的形狀，不會歪，也不會動到圖上任何其他像素。**

    node drafts/banner-logo-stamp.mjs <插圖> <輸出.png> <牌子中心x> <牌子中心y> <標誌寬度>
    例：node drafts/banner-logo-stamp.mjs 第十一輪.jpg out.png 546 466 28

・形狀來源是 `assets/logo.png`（**index.html 頁首那條原始向量算出來的**，
  不是重畫的 —— 見 CLAUDE.md 第十節第 2 項），只取它的 alpha 當遮罩、填白。
・4× 超取樣再降下來，邊緣才不會有鋸齒。
・牌子的位置是**自動找出來的**：掃偏綠的像素、取連通元件，
  篩出門左邊那一塊 37×24（第十一輪那張圖是 x528-564／y455-478）。
  換一張圖就重跑一次那段偵測，不要沿用座標。

### 輸出

    drafts/fb-banner-1640x924.jpg     ← 可以直接上傳 FB 的成品
    drafts/fb-banner-logo-zoom.png    ← 牌子放大 8 倍，給使用者確認

⚠ 生成的原圖是 1376×768，縮到 1640×924 是**放大 1.19 倍**（先中央裁成 16:9、
左右各去 6px）。平塗插畫撐得住，但下一次生成能給更大就給更大。
...
## 十九、收尾（2026-08-20）—— 成品與最後兩個判斷

第十二輪的圖：**安全帽有了**（騎士戴著開放式半罩、左邊那位助理手上也拿著一頂），
**牌子上的標誌模型自己畫了一個**，形狀接近但不精確（左右兩瓣不對稱、牙洞變成一顆帶框的小點）。

### 1. 標誌仍然換成原始向量的

品牌標誌**形狀要精確**，這不是風格問題。做法是新腳本
`drafts/banner-logo-replace.mjs`：**先把牌子內部抹回它自己的綠**（取邊環的**眾數**，
不是平均 —— CLAUDE.md 第九節第 11 條），加一點顆粒免得變成死板的色塊，
**再貼上 `assets/logo.png` 的形狀**（ink 模式：暖白、不透明度 .90、顆粒、邊緣再軟一階）。

    node drafts/banner-logo-replace.mjs <圖> <輸出> <牌子x0> <y0> <x1> <y1> <標誌寬>
    這一張：527 455 563 476 27　→　牌子綠量到 rgb(92,100,84)

### 2. ⚠ 「把畫面往上搬讓騎士不被切」試過了，**不要做**

騎士的輪子在電腦版的裁切線下面（93%）。試過把整張往上搬 95px、下面用柏油接長：
騎士完整了，**但二樓窗邊那兩個人被切掉一半** —— 而那條「樓上揮手／樓下回應」
是整張圖的核心。

**判斷：寧可切騎士的輪子，也不要切窗邊那兩位。** 電腦版看到的是一台機車騎過畫面下緣，
本來就讀得通；窗邊的人被切掉一半就是壞掉。

> 通則：**裁切要保的是「意義」不是「完整」** —— 先問哪一塊被切會讓畫面失去意思。

### 成品

    drafts/fb-banner-final-1640x924.jpg      ← 可以直接上傳 FB
    drafts/fb-banner-final-plaque-zoom.png   ← 牌子放大 10 倍
