...
## 五之十、~~第六版的提示詞~~ ✅ **過了** —— 現行的是第五之十六節（加水的細節）
...
## 五之十二、~~第七版的提示詞~~ ✅ **水有了，但屋簷的水滴像冰柱** —— 現行的是第五之十六節
...
## 五之十三、✅ 第七版：水有了 —— **但屋簷的水滴像冰柱**（2026-09-05）
...
| | 冰柱 | 水滴 |
| --- | --- | --- |
| 哪一端重 | **上重下尖**（黏在屋簷那端最粗，往下收成點） | **下重上細**（下端是圓的一團，上面拖一條細尾巴） |
| 黏不黏著 | 一定黏在邊上 | **多數已經脫離**，是空中的一顆一顆，離邊有距離 |
| 排列 | 等距、等長，讀得出節奏 | 間距不一、大小不一、高度不一，**中間還有整段是空的** |
| 方向 | 垂直 | **被風吹斜**，和雨同一個角度 |

**第七版的提示詞其實已經寫對了三件**（「separate drips, spaced apart」「never a continuous
curtain」「every falling drip is pulled sideways」），唯一沒寫的就是**哪一端重**——
而那一句寫的是 `short tapered strokes`，**`tapered` 這個字本身就是「往下收成尖端」**，
等於在叫它畫冰柱。

> ⚠⚠⚠ **通則：同一個輪廓家族要靠「哪一端重」分辨，不是靠長度或數量。**
> 水滴、冰柱、鐘乳石、火焰、葉子……外框都是「一頭寬一頭尖的長條」，
> 差別只在**寬的那一端在上面還是下面**。只寫「短短的、分開的、不要連成一片」
> 治得了密度，治不了那件事 —— 要寫死**下端是圓的、上端是細的**，
> 並補一條否定式（**這張圖裡沒有任何東西是「貼著邊那端最粗、往下收成尖」的**）。

> ⚠⚠ **通則的第二半：一排等距等長的東西，一定會被讀成人造物或結晶。**
> 天氣畫出來的東西沒有一項是規律的。所以規則要寫成「**間距全都不一樣、
> 中間有整段是空的、沒有兩顆一樣大**」，並直接給判準：
> **看得出節奏就是錯的**。（同第十五節第 1 條那個做法：規則不要綁在數量上，
> 要給一個看得出來就算輸的判準。）

### 這一輪只改一件

第七版是「加一層水」，第八版**一個東西都不加、也不拿掉**，只改那一排水滴的形狀。
所以抬頭與第 6 段的口氣要整個換掉 —— 第七版第 6 段開頭寫著
「**At the moment the rain is falling but the building is not shedding any of it**」，
那句話在這一輪就變成「叫它把已經畫好的水再畫一次」（＝第六節第 13 個坑，
這已經是第二次靠 grep 才擋下來）。第 6 段現在改成：
**先逐項列出「保留、不要重畫」的六樣水**，再講那一件要改的。

---

## 五之十四、~~第八版的提示詞~~ ⚠ **形狀對了，但水滴又大又滿全圖** —— 現行的是第五之十六節

⚠⚠ **參考圖照舊五張**，說法換成：
「**第七版那張圖幾乎全部正確** —— 建築、鐵門、騎樓、光、天空、雨、風的線條、樹、盆栽、
三個人、窗框的流水、牆上的濕痕、地上的水圈、路邊的積水**全部原封不動**；
**這一輪什麼都不加也不拿掉，只改一件事：屋簷那一排水滴的形狀**（第 6 段）。」
...
TWO OF THE PROVIDED PICTURES ARE INSTRUCTIONS, NOT SCENES TO COPY WHOLE:
  • ONE IS AN EARLIER ATTEMPT AT THIS EXACT SCENE AND EVERYTHING IN IT IS NOW CORRECT EXCEPT ONE
    SMALL THING. Copy its framing, its building, its covered walkway, its closed shutter, its dark
    stormy light, its sky, its wet road, THE ANGLE AND DIRECTION OF ITS RAIN, its wind streaks, its
    bent tree, its broken pot, the wet streaks and puddles it has already got, and the position,
    size, posture and facing of its three people, exactly as they are. Do not mirror anything. Do
    not improve anything. The one small thing is the SHAPE of the drips hanging from its eaves -
    see section 6.
  • THE OTHER IS A PLAIN DIAGRAM OF THE RAIN: a flat grey rectangle covered in pale diagonal
    streaks and nothing else. The rain in the earlier attempt already matches it; use it only to
    confirm the angle. Do not copy its colour, do not use it as a background, do not draw a grey
    panel anywhere.
  THE EARLIER ATTEMPT IS CORRECT, INCLUDING ITS WIND STREAKS AND INCLUDING THE WATER ON THE
  BUILDING. Keep its framing, its building, its shutter, its covered walkway, its light, its sky,
  its rain angle and direction, its wind streaks, its bent tree, its broken pot, its wet window
  boxes, its wet streaks, its splash rings, its puddles and the position, posture and facing of its
  three people EXACTLY as they are. Do not mirror anything, do not re-compose anything, do not
  re-draw anything. NOTHING IS BEING ADDED THIS ROUND AND NOTHING IS BEING TAKEN AWAY. THE ONLY
  CHANGE IS THE SHAPE OF THE DRIPS ALONG THE EAVES AND UNDER THE WINDOW LIPS - see section 6.
...
6. THE WATER ON THE BUILDING IS ALREADY THERE AND ALMOST ALL OF IT IS RIGHT. KEEP EXACTLY AS THEY
   ARE, AND DO NOT RE-DRAW THEM: the bright highlights along the top edge and bottom lip of each
   dark window box; the bands of darker, wet concrete running down the wall below them; the bright
   concentric rings and the little upward ticks of splash on the wet ground where the water lands;
   the longer, quieter pool along the low step at the kerb with its broken vertical reflections of
   the shutter and columns and its one broken smear of the woman's yellow; the ripples on that pool
   at the angle of the wind; the covered walkway floor staying mostly dry with only its outer edge
   wet; and the one thicker thread of water running off the roofline at the corner of the building.
   THE ONE THING TO CHANGE IS THE SHAPE OF THE DRIPS. In the earlier attempt the drips hanging from
   the front edge of the canopy are evenly spaced, all the same length, all vertical, and each one
   is fixed to the edge and tapers to a point at the bottom. THAT IS THE SHAPE OF AN ICICLE, and at
   a glance the clinic looks like it is hung with ice. A falling drop of water is the exact opposite
   of an icicle. Re-draw every drip - along the front edge of the canopy, along the tile ends of the
   low roof of the old house at the left, and under the lip of each window box - by these four
   rules, and change nothing else in the picture:
     • A DROP IS HEAVY AT THE BOTTOM AND THIN AT THE TOP. Each one is a small rounded blob with a
       fine tail trailing up behind it, wide and soft at its lower end. NOTHING hanging from any
       edge in this picture may be thick where it meets the edge and taper to a point below it -
       that shape is ice, not water.
     • MOST OF THEM HAVE ALREADY LET GO. The majority are separate marks in the air below the edge,
       with clear empty space between them and the edge, each at a different distance below it.
       Only two or three are still attached and swelling at the edge, and each of those is shorter
       than the canopy is thick.
     • NOTHING ABOUT THE ROW IS REGULAR. The gaps between them are all different, there are long
       stretches of edge with nothing hanging from them at all, and no two drops are the same size
       or at the same height. If you can see a rhythm, an even fringe or a repeating pattern along
       the edge, it is wrong. Fewer drops, more unevenly placed, is always better.
     • THEY ARE ALL BLOWN SIDEWAYS. Every falling drop leans at the same angle as the rain and the
       wind. Nothing hangs straight down in this wind.
   Under the lip of each window box, the same thing: three or four uneven, separated drops of
   different sizes, never a row of matching teeth along the lip.
   ALL OF THIS IS STILL CLEAN RAINWATER ON A WELL-KEPT BUILDING, and at thumbnail size the drips
   and wet streaks are still a fine, quiet texture: they must not turn the wall or the canopy into
   stripes, a fringe or a frill, and must not out-shout the bright yellow poncho.
...
AVOID: rust-coloured, orange, green or black streaks running down from the windows; mould, algae,
dirt trails, old water stains or discoloured render; a continuous curtain, sheet or waterfall of
water off the canopy instead of separate drips; ICICLES ANYWHERE; anything hanging from an eave,
a canopy or a window lip that is thick where it meets the edge and tapers to a point below it; a
row of evenly spaced drips all the same length; an even fringe, frill, lace or row of matching
teeth along any edge; every drip still attached to the edge with none of them fallen clear; drips
falling straight down as if there were no wind; a building that stays completely dry in heavy rain; a soaking wet covered walkway floor right
up to the shutter; so many wet streaks that the wall reads as stripes; the shutter drawn across the
front of the building instead of deep inside the covered
walkway; a shutter that touches the outer edge of the canopy or comes down onto the pavement;
columns flattened against the shutter with no air, floor or shadow between them; a missing covered
...
## 五之十五、⚠ 第八版：冰柱治好了，**但水滴又大又滿全圖**（2026-09-05）

使用者：**「我必須說這樣沒有比較好　你們好好檢查一下　怎麼回事　或是讓積水和滴水在
診所右側屋簷　和一兩個窗框就好　每個都有很奇怪」**

### 逐項對第八版那四條規則看，只有一條真的照做

| 第八版寫的 | 畫出來 |
| --- | --- |
| 下重上細 | ✅ 做到了（不再是冰柱） |
| 多數已脫離邊緣 | ✅ 大致做到 |
| 間距與大小都不一樣 | ❌ **仍然是一排一排等距等大** |
| 被風吹斜 | ❌ **全部垂直** |

**而真正的問題是那兩條規則根本沒寫到的兩件事：**

1. ⚠⚠⚠ **尺寸** —— 第八版從頭到尾**沒有給任何一個大小的錨點**（只寫「a small rounded
   blob」，`small` 是形容詞）。畫出來每一顆大約和人的眼睛一樣大，2000px 的圖上有四五十顆。
2. ⚠⚠⚠ **數量與位置** —— 第八版寫的是
   「along the front edge of the canopy, along the tile ends of the low roof at the left,
   **and under the lip of each window box**」。那是**一條套用在「每一個這種邊緣」上的通則**，
   於是它套滿全圖：兩棟樓、每一層、每一扇窗、連右邊鄰棟都掛滿。

> ⚠⚠⚠ **通則：把天氣的細節寫成「凡是某一類邊緣都要如何」，它就會被套滿整張圖。**
> 細節要**指名地點**（哪一段屋簷、哪一兩扇窗），不要定義一個類別。
> 這是第十四節那條「規則不要綁在數量上」的**空間版**：那一條說不要綁在「兩塊」，
> 這一條說也不要綁在「每一塊」—— 兩種寫法都會讓射程和你想的不一樣。

> ⚠⚠ **通則：形狀治好了不代表畫對了。** 一個細節有**三個彼此獨立的量**：
> **形狀／尺寸／數量與位置**。第七版錯在形狀、第八版把形狀治好了，
> 卻因為另外兩個從來沒寫過而更糟（同第九節第 17 條那條「淡出有三個獨立的量」）。
> **每加一種細節，三個量都要各給一個量得出來的錨點。**

> ⚠ **第三件：水滴被畫成「符號」不是「觀察到的水」** —— 平滑對稱的淚滴外框、
> 裡面還有一點白色高光，那是水滴的**圖示**。要寫死「不是對稱的淚滴輪廓、
> 裡面沒有白色高光」。

### 這一輪只做減法

使用者自己給了解法，而且是對的：**只留診所右側那一段屋簷 ＋ 一兩個窗框**。
所以第九版**不加任何東西、也不改形狀**，只做兩件：**擦掉**其餘全部的水滴、
留下來的那幾顆**縮到和一道雨差不多細**。

⚠ **路面的積水、倒影、雨打在地上的圈圈全部保留** —— 那是路面本來就有的東西，
使用者說的「積水」指的是跟著水滴一起冒出來的那些落點。

⚠ 抬頭與第 6 段的口氣因此又要整個換掉（第八版寫的是「只改水滴的形狀」，
留著就會叫它再畫一次）—— **這是那條通則的第四次現場**，一樣是靠 grep 上一版的病名擋下來的。

---

## 五之十六、⭐ 第九版的提示詞 —— ✅ **這一版定稿**（逐字，留著給下次換圖用）

⚠⚠ **參考圖照舊五張**，說法換成：
「**第八版那張圖除了一件事以外全部正確** —— 建築、鐵門、騎樓、光、天空、雨、風的線條、樹、
盆栽、三個人、牆上的濕痕、路面的積水與水圈**全部原封不動**；
**這一輪什麼都不加、只做減法：把掛在屋簷與窗框上的水滴幾乎全部擦掉，
只留診所右側那一段屋簷和一兩個窗框，而且要小很多**（第 6 段）。」
...
TWO OF THE PROVIDED PICTURES ARE INSTRUCTIONS, NOT SCENES TO COPY WHOLE:
  • ONE IS AN EARLIER ATTEMPT AT THIS EXACT SCENE AND EVERYTHING IN IT IS CORRECT EXCEPT FOR ONE
    THING THAT HAS TO BE TAKEN OUT. Copy its framing, its building, its covered walkway, its closed
    shutter, its dark stormy light, its sky, its wet road, THE ANGLE AND DIRECTION OF ITS RAIN, its
    wind streaks, its bent tree, its broken pot, its wet walls, its puddles and ripples, and the
    position, size, posture and facing of its three people, exactly as they are. Do not mirror
    anything. Do not improve anything. THE ONE THING TO CHANGE: it has big teardrop-shaped drops
    hanging from every eave, every roof and every window in the picture. Almost all of them are
    deleted this round - see section 6.
  • THE OTHER IS A PLAIN DIAGRAM OF THE RAIN: a flat grey rectangle covered in pale diagonal
    streaks and nothing else. The rain in the earlier attempt already matches it; use it only to
    confirm the angle. Do not copy its colour, do not use it as a background, do not draw a grey
    panel anywhere.
  THE EARLIER ATTEMPT IS CORRECT IN EVERY OTHER RESPECT. Keep its framing, its building, its
  shutter, its covered walkway, its light, its sky, its rain angle and direction, its wind streaks,
  its bent tree, its broken pot, its wet walls, its wet road, its puddles, its ripple rings and the
  position, posture and facing of its three people EXACTLY as they are. Do not mirror anything, do
  not re-compose anything, do not re-draw anything, do not add anything. THIS ROUND ONLY TAKES
  SOMETHING AWAY: the hanging drops. Nearly all of them are erased and the few that stay become
  very small - see section 6.
...
6. THIS ROUND ONLY ERASES THE HANGING DROPS. In the earlier attempt, large pale teardrop shapes
   hang from the front canopy, from the tiled roof of the old house at the left, from the eaves at
   the corner, from the neighbouring building at the right and from under every single window on
   every facade. There are far too many of them, they are far too big, and each one is drawn as a
   smooth symmetrical teardrop symbol. Water on a building does not look like that.
   KEEP EXACTLY AS THEY ARE AND DO NOT RE-DRAW THEM: the wet road with its broken reflections; the
   shallow puddles and the ring-shaped ripples on them; the pool along the low step at the kerb; the
   bright highlights along the top edge and bottom lip of each dark window box; the bands of darker,
   wet concrete running down the wall; and the covered walkway floor staying mostly dry with only
   its outer edge wet.
   ERASE EVERY HANGING DROP IN THE PICTURE EXCEPT IN TWO SMALL PLACES:
     • ONE STRETCH OF EAVE ONLY: the short run of eave over the side elevation, on the right-hand
       side of the building, past the corner column. A handful of small drops there, plus the one
       thicker thread of water already running off the roofline at that corner.
     • ONE OR TWO WINDOWS ONLY: pick the two window boxes nearest the corner of the building and
       let three or four small drops hang under their lower lips. Every other window in the
       picture, on both buildings, has nothing hanging from it.
     • Nowhere else. No drops along the front canopy above the shutter. None along the tiled roof
       of the old house at the left. None anywhere on the neighbouring building at the right. Those
       edges are simply wet and dark, with no hanging water at all.
   THE FEW THAT REMAIN ARE TINY AND UNEVEN:
     • SIZE: each one is about as wide as one of the fine rain strokes and never wider than the
       thin dark frame around a window. It is a small mark, not a shape you would notice on its own.
       It is never as big as an eye, a leaf or a person's fingertip.
     • FORM: a soft, slightly ragged little bead of water - heavier and rounder at its lower end,
       thinning to nothing at the top. It is not a smooth symmetrical teardrop outline, it has no
       white highlight drawn inside it, and it never tapers to a downward point.
     • PLACEMENT: uneven gaps, different sizes, different heights, long empty stretches of edge in
       between. Most have already let go of the edge and are falling clear of it. If you can see a
       rhythm or an even row, it is wrong.
     • They lean at the same angle as the rain. Nothing hangs straight down in this wind.
   Where a drop lands, the little ring and splash tick on the ground below it stays; where the
   drops have been erased, those rings go with them, and only the ordinary rain ripples remain.
   ALL OF THIS IS STILL CLEAN RAINWATER ON A WELL-KEPT BUILDING: the dark is wet, never dirty. At
   thumbnail size the water on the building is a quiet texture that you would barely notice, and it
   must never out-shout the bright yellow poncho.
...
dirt trails, old water stains or discoloured render; a continuous curtain, sheet or waterfall of
water off the canopy instead of separate drips; ICICLES ANYWHERE; anything hanging from an eave,
a canopy or a window lip that is thick where it meets the edge and tapers to a point below it;
LARGE TEARDROP SHAPES HANGING ANYWHERE IN THE PICTURE; a smooth symmetrical teardrop symbol; a
drop with a white highlight drawn inside it; any drop bigger than an eye; drops hanging from more
than one stretch of eave; drops under more than two windows; any drop on the front canopy above
the shutter, on the tiled roof of the old house at the left, or anywhere on the neighbouring
building at the right; a row of evenly spaced drips all the same length; an even fringe, frill,
lace or row of matching teeth along any edge; every drip still attached to the edge with none of
them fallen clear; drips falling straight down as if there were no wind; a building that stays completely dry in heavy rain; a soaking wet covered walkway floor right
up to the shutter; so many wet streaks that the wall reads as stripes; the shutter drawn across the
front of the building instead of deep inside the covered
walkway; a shutter that touches the outer edge of the canopy or comes down onto the pavement;
columns flattened against the shutter with no air, floor or shadow between them; a missing covered
...
## 五之十七、✅ 第九版定稿並接上（2026-09-05）

使用者：**「定稿了　做成圖卡」**

**九版走完**。第八版那四五十顆又大又滿全圖的水滴，第九版**只做減法**之後收乾淨了：
屋簷只剩右側那一段與幾扇窗有零星幾顆、而且細到縮圖上根本不搶戲，
路面的積水、倒影、雨圈全部保留。**構圖、建築、鐵門、光、雨、風、樹、盆栽、
三個人一個像素都沒有再動過。**
...
| | |
| --- | --- |
| 原檔 | `drafts/typhoon-hero-src.jpg`（1376×768，16:9） |
| 成品 | `preview/line-typhoon/hero-typhoon.jpg`（**1024×512**，2:1，180 KB） |
| 裁圖 | `node drafts/channels/typhoon-hero-crop.mjs`（**上 40 / 下 40**） |
| 接在哪 | 提案頁**兩處吃同一個檔**：聊天室那張貼的圖（232×116）、Flex 圖卡的頭圖（268×134） |

### ⚠⚠⚠ 守門第三次不能照抄 —— 這一張整張都是暗的

`remind-hero-crop.mjs` 與 `review-hero-crop.mjs` 的地標都是**深墨**
（<120 或 <70 的線稿），而這一張是暴風天：實測 **768 列裡每一列都有 40 個以上
的 <70 像素**，「深墨離上緣幾列」回的是 `0 / 767`，**完全沒有資訊**。

> **通則（第三次記）：守門的地標要挑「這一張圖才有、而且一定存在」的東西。**
> 這一張挑的是**黃雨衣**（含黃雨鞋）—— 提示詞明文寫著它是全圖最亮的一塊，
> 也正是縮圖上眼睛第一個找到的東西。實測原檔第 **431~678** 列，
> 守門要求它在成品裡上下各留 ≥12 列（實測 289 / 37 ✓）。

⚠ 白框那一道也要改寫法：`bind-done` 那支是拿「邊緣中位亮度 vs **紙底**」比，
這一張四邊全是暗的天空與積水，改成**絕對亮度**（起伏 <6 且中位 >200 才算白框）。
實測四邊起伏 130／93／99／72，一邊都沒有誤報。

### 為什麼是上 40 / 下 40

16:9 收成 2:1 要拿掉 80 列，而**這一張兩端都有餘裕**，所以不必偏心：

- **上面 40 列全是暴風天空與雨** —— 最上面的實體是左邊那棵樹的樹冠（約第 150 列）。
  ⚠ 從上面裁會讓二樓那幾扇窗的上緣跑出畫面，**那是對的**：原檔右半本來就有
  好幾扇窗貼著上緣被切掉，多幾扇一致而已。
- **下面 40 列全是積水與倒影** —— 黃雨鞋收在第 678 列，最低的鞋子在第 700 列上下，
  裁到第 728 列碰不到任何一雙鞋。

### 接上圖之後量到的（五個寬度逐格相同）

聊天室那張 **232×116**、Flex 圖卡 **268×447.8**（頭圖 268×134）、
水平溢出 0、JS 錯誤 0、圖三處都載得到。
⚠ **和佔位框那一版逐格相同** —— 佔位框本來就是照 `aspect-ratio: 2/1` 預留的，
那一輪的預防生效了（同 `line-review` 那一輪）。
⚠ `width`／`height` 兩個屬性 ＋ CSS 的 `height:auto` **三個都要寫**
（呈現屬性會蓋過 `aspect-ratio`，`line-auto-reply` 踩過兩次）。

---

## 六、⚠ 十六個一定會踩的坑（都是這一站踩過的）
...
15. ⚠⚠⚠ **同一個輪廓家族要靠「哪一端重」分辨，不是靠長度、數量或密度**
   （第五之十三節，第七版的屋簷水滴被看成冰柱）。水滴、冰柱、鐘乳石、火焰、葉子，
   外框都是「一頭寬一頭尖的長條」，**差別只在寬的那一端在上面還是下面**。
   那一版寫了「分開的、不要連成一片、要被吹斜」三件都對，
   卻用了 `short tapered strokes` —— **`tapered` 本身就是「往下收成尖端」＝冰柱**。
   ⚠ 要寫死的是**下端是圓的一團、上端拖一條細尾巴**，
   並補一條否定式：**這張圖裡沒有任何東西是「貼著邊那端最粗、往下收成尖」的**。
   ⚠ 第二半：**一排等距等長的東西一定會被讀成人造物或結晶** ——
   天氣畫出來的東西沒有一項是規律的，所以規則要寫成
   「間距全都不一樣、中間有整段是空的、沒有兩顆一樣大」，
   並直接給判準：**看得出節奏就是錯的**。
   ⚠ 第三半：**多數水滴已經脫離邊緣**（是空中的一顆一顆），
   只留兩三顆還黏在邊上 —— 全部黏著就一定像冰柱。

16. ⚠⚠⚠ **天氣的細節不要寫成「凡是某一類邊緣都要如何」，要指名地點；
   而且形狀、尺寸、數量與位置是三個獨立的量，缺一個就會出事**
   （第五之十五節，第八版的水滴又大又滿全圖）。
   第八版寫的是「along the front edge of the canopy、the tile ends of the low roof、
   **and under the lip of each window box**」——**那是一個類別**，
   於是它套滿兩棟樓、每一層、每一扇窗。
   ⚠ 尺寸那一項第八版**根本沒寫**（只有形容詞 `small`），畫出來每顆和眼睛一樣大。
   ⚠ 所以每加一種細節，**三個量都要給量得出來的錨點**：
   形狀（下重上細、不是對稱的淚滴輪廓、裡面沒有白色高光）、
   尺寸（**和一道雨差不多細、不超過窗框那條細線**）、
   數量與位置（**只有右側那一段屋簷 ＋ 一兩扇窗，其餘一顆都沒有**）。
   ⚠ 這是第 14 個坑「規則不要綁在數量上」的**空間版**：
...
3. ~~換掉提案頁那兩個佔位框~~ ✅（聊天室那張與 Flex 頭圖**吃同一個檔**）
4. ~~圖畫好不代表會用到~~ ✅ **2026-09-05 使用者定稿走圖卡（Flex）**
   （「定稿　存起來」）—— 所以「要不要為了有圖，每個人多貼一次」**不必答了**：
   圖卡是**系統送的、貼 0 次**。
   ⚠⚠ 換來的是另一件（規格頁上寫著、守門也在盯）：**圖卡的字是廠商系統裡
