...
## 兩個候選版本 ＋ 提案頁（2026-08-25）

使用者：「**用這兩個版本做手機版著陸頁，做可切換的提案頁給我看。**」
—— 他挑的是 **v1（半身）與 v2（全身）**，也就是治「電風扇」之前的那兩張
（md5 對過，逐位元組相同）。**v3/v4/v5 那三輪的機器修正沒有被選上**，
留在這一份裡是紀錄，不要拿去重做。

| | Ａ 半身 | Ｂ 全身 |
| --- | --- | --- |
| 來源 | `drafts/lineart-surg-v1.jpg` | `drafts/lineart-surg-v2.jpg` |
| 裁切 | `--crop 104,95,816,830`（裁掉底下那條地面線，y≈936 有 518 個墨點） | `--crop 274,42,478,943`（沒有地面線，最後幾列是鞋子） |
| 產出 | `assets/lineart-surg.png` 816×830 | `drafts/lineart-surg-alt.png` 478×943 |
| 線佔 | 10.7% | 8.3% |
| 390 上預設 76% | 275×280 | 275×**543** ⚠ 比介紹區高 133px |

⚠⚠ **兩張的長寬比差很多（0.98 vs 0.51），所以「同一個大小」在畫面上完全不是同一件事**
—— Ｂ 在同一個寬度下高出一倍。**這是顯微根管那一輪那條「近正方或直的圖，卡住的是
高度不是對比度」的第二次出現**：Ｂ 在手機上要收到 **52% 以下**（371px 高，介紹區 409）、
在 744 上要收到 **200px**（240 那一格就比介紹區高 34px）。

### 提案頁的產生器多了「版本」這一把尺（通用，不是口外專用）

    node tools/lineart-preview.mjs surg --alt drafts/lineart-surg-alt.png \
      --main-label "Ａ 半身" --alt-label "Ｂ 全身"

- ⚠ **切版本要連 `aspect-ratio` 一起換，不能只換圖檔** —— 只換圖的話直的那一張會被
  壓進近正方的框裡整個變形。做法：把 `aspect-ratio` 也接成 `var(--la-ar, …)`，
  長寬比直接從 PNG 檔頭（IHDR）讀，不必解碼整張圖。
- 守門那一段跟著多一項（`--la-ar` 沒接上就 throw），同 2026-08-24／25 那兩次
  「按了沒反應、不報錯」的教訓。
- **大小那把尺往下多兩格**（手機 44／52%、≥721 200／240px），理由就是上面那張表。

### 這一輪的驗收

四個視窗（390／375／744／1440）× 兩個版本 × 三個大小：介紹區高度和站上
`topics/surg/index.html` **逐格相同**、都沒有水平捲動、版本按鈕的 `aria-pressed`
與實際吃到的圖檔一致。

⚠ `index.html` 的 `[data-topic="surg"]` 那一段與 `topics/` 的快照**留在工作分支上**，
`main` 只收提案頁與那兩張 PNG（CLAUDE.md 第八節：提案期間可以只把提案頁推上 `main`）。
