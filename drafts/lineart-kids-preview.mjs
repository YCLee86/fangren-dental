...
[data-topic="kids"] .tp-intro::before {
  ...
  background-image: url("../../assets/lineart-kids.png");
  background-repeat: no-repeat; background-size: contain;
  background-position: var(--pv-bp, center) center;
...
<div class="pv-bar">
  ...
  <div class="pv-row" data-k="dx">
    ...
    <button type="button" data-v="0">貼齊</button>
...
(function () {
  ...
  var BLM = { "0": "0px",  "1": "40px", "2": "80px", "3": "120px", "4": "160px" };
  ...
  var BLD = { "0": "0px",  "1": "50px", "2": "100px", "3": "150px", "4": "200px" };
  /* ⚠⚠ 手機與 ≥721 是**兩組獨立的值**（站上本來就分兩段）——
     ...
     **mini 直放 744**，用 834 分會把他那台歸到「手機」那一段，
     他看到的和定案的就不是同一件事。 */
  var st = { wM: "d", opM: "2", wD: "b", opD: "2", dxM: "1", dxD: "1", fig: "on", flip: "on" };
  ...
  function paint() {
    ...
    var bl = (isWide() ? BLD : BLM)[get("dx")] || "0px";
    /* ⚠⚠ 翻轉是 scaleX(-1)：畫面上的「往右」在未翻轉的座標裡是「往左」，
       所以出血的方向要跟著鏡射，不然按下去圖會往反方向跑。 */
    root.style.setProperty("--pv-bp", bl === "0px" ? "center"
      : (st.flip === "on" ? "calc(0% - " + bl + ")" : "calc(100% + " + bl + ")"));
    ...
    dBt.forEach(function (bt) {
      var v = (isWide() ? DXD : DXM)[bt.dataset.v], b = (isWide() ? BLD : BLM)[bt.dataset.v];
      bt.textContent = b === "0px" ? "貼齊" : "＋" + b;
    });
    if (isWide()) {
