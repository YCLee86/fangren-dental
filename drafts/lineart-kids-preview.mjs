...
[data-topic="kids"] .tp-intro::before {
  content: ""; position: absolute; right: calc(-1 * var(--pv-dx, 14px)); bottom: 0;
  width: var(--pv-w, min(85.6%, 419px));
  aspect-ratio: calc(${IMG.w} - var(--pv-cut, 144)) / ${IMG.h};
  /* ⚠ 牙周那一套：**框窄一截 ＋ 圖照高度撐滿靠左**，多出來的被框裁掉。
     比例尺不變（auto 100% 是按高度算的），只是整張往右移。
     ⚠ 翻轉已經烘進圖檔（tools/topic-lineart.mjs --flip），這裡不再用 transform。 */
  background: url("../../assets/lineart-kids.png") left center / auto 100% no-repeat;
  opacity: var(--pv-op, .152); pointer-events: none; z-index: 0;
}
...
(function () {
  ...
  var WMP = { a: 81.25, b: 90, c: 96, d: 100 }, WMX = { a: 390, b: 430, c: 460, d: 490 };
  var WDP = { a: 76, b: 88, c: 96, d: 100 },     WDX = { a: 400, b: 460, c: 520, d: 580 };
  var WDPX = { a: "400", b: "460", c: "520", d: "580" };
  function widthCSS() {
    var cut = +((isWide() ? CUTD : CUTM)[get("dx")] || 144), k = (998 - cut) / 998;
    var P = isWide() ? WDP : WMP, X = isWide() ? WDX : WMX, g = get("w");
    return "min(" + (P[g] * k).toFixed(1) + "%, " + Math.round(X[g] * k) + "px)";
  }
  ...
  /* ⚠⚠ 「往右」＝ 把偽元素推出版心（right 給負值）。
     ...
     ≥721 版心兩側留白很寬，可以推遠一點。 */
  /* ⚠⚠ 「貼齊螢幕邊」之後還要更右，只能讓**圖在框裡往右移**（出血），
     框本身不能再推 —— 再推就凸出螢幕、整頁可以左右拉（CLAUDE.md 記著踩過）。
     出血的部分被偽元素的框裁掉，不會產生水平捲動；等於牙周那張「裁右」的鏡像。 */
  /* 「裁多少」＝ 從圖檔右緣（翻轉後是醫師的背）切掉幾欄原圖。
     ⚠ 框的寬度要跟著等比例縮，**比例尺才不會變** —— 這是牙周那一輪的做法：
       框寬 ＝ 原本的寬 × (998 − 裁量) ÷ 998，圖用 auto 100% 靠左，超出的被框裁掉。
     使用者 2026-08-24 在 375 上選的是「出血 50 rendered px」＝ 原圖 144 欄。 */
  var CUTM = { "0": "96", "1": "144", "2": "192", "3": "240", "4": "288" };
  var DXM = { "0": "14px", "1": "14px", "2": "14px", "3": "14px", "4": "14px" };
  ...
  var CUTD = { "0": "96", "1": "144", "2": "192", "3": "240", "4": "288" };
  var DXD = { "0": "20px", "1": "20px", "2": "20px", "3": "20px", "4": "20px" };
  /* ⚠⚠ 手機與 ≥721 是**兩組獨立的值**（站上本來就分兩段）——
     ...
     **mini 直放 744**，用 834 分會把他那台歸到「手機」那一段，
     他看到的和定案的就不是同一件事。 */
  var st = { wM: "d", opM: "2", wD: "b", opD: "2", dxM: "1", dxD: "1", fig: "on" };
  ...
  function paint() {
    ...
    root.style.setProperty("--pv-w", widthCSS());
    root.style.setProperty("--pv-cut", (isWide() ? CUTD : CUTM)[get("dx")] || "144");
    ...
    dBt.forEach(function (bt) {
      ...
      bt.textContent = (isWide() ? CUTD : CUTM)[bt.dataset.v] + " 欄";
    });
    if (isWide()) {
  ...
  function meas() {
    ...
    if (imgOK && fw > 0) {
      try {
        ...
        /* 和 CSS 那條 auto 100% 靠左 同一個做法：先按高度撐滿，再靠左、超出的不畫 */
        g.drawImage(img, 0, 0, Math.round(fh * ${IMG.w} / ${IMG.h}), Math.round(fh));
    ...
    var txt = "視窗 " + Math.round(innerWidth) + "×" + Math.round(innerHeight) +
      ...
      "　濃度 " + al.toFixed(3) + "　裁 " + ((isWide() ? CUTD : CUTM)[get("dx")]) + " 欄" +
      "\\n壓到線的字 " + hit + "／" + lines + " 行";
    if (hit) {
