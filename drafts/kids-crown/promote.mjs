...
const DESC    = '補了又掉，差別多半不在孩子身上，而在那顆牙當下的條件——洞的大小、剩下的齒質，以及那幾分鐘能不能保持乾燥。什麼時候該從「補」換成整顆包起來，不鏽鋼牙套與全鋯牙套又差在哪裡。';
const OGDESC  = '補了又掉，差別多半不在孩子身上，而在那顆牙當下的條件。什麼時候該從「補」換成整顆包起來。';
// ⚠⚠ 卡片摘要要跟著內文走。這一段從第一版一路留到上線那天，錯了兩處：
//   ①「真正的原因通常在別的地方」和定案的開場「可能說對了一半」**直接矛盾** ——
//     那正是使用者當初抓到的那個事實錯誤，它還活在卡片上。
//   ②「要換的往往不是材料，是做法」是**豪氏牙套那一版**的遺物（豪氏＝做法）；
//     那一節已經整節拿掉，現在文章比較的正好就是材料，這句話變成反的。
//   通則：改內文的那一輪要把 post-meta 的 excerpt 與 <head> 的兩段描述一起唸過 ——
//   它們不在畫面上，改稿時看不到，但它們就是讀者在首頁看到的那一段。
const EXCERPT = '「同一顆牙補了又掉，已經好幾次了。」很多爸媽的第一個念頭是孩子的牙齒特別不好——可能說對了一半。蛀過的那顆牙結構確實被弱化了，但補得住與補不住，差別多半不在孩子身上，而在那顆牙當下的條件。';
