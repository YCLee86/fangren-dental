...
/* ⚠ 「實心」的值叫 fill；?c=blue 是上一版的寫法，留著吃得到 */
...
html[data-pv-d="n"] { --pv-dw: 1.4; --pv-da: none; }
...
/* 空心（2026-09-15 使用者：「藍實心好像怪怪的，做成空心給我看看」）——
...
(function () {
  ...
  /* ⚠⚠ 永樂街：被亮起來的那條點狀路線壓到時，**移位不是消失**（2026-09-15）。
     ⚠ 量的是 viewBox 的使用者單位不是螢幕座標 —— 這個標籤身上掛著 .18s 的
       transform transition，用 getBoundingClientRect 量會量到「動到一半」的位置；
       getBBox() 不吃元素自己的 transform，所以永遠是它的原位。
     ⚠ 三段階梯：往上 → 往下 → 真的沒地方就交還給站上那條「藏起來」。 */
  var yl = svg.querySelector('text.lbl[data-pv-mv="yongle"]');
  function boxOf(el) {                        // 螢幕座標 → viewBox 使用者單位
    var m = svg.getScreenCTM(); if (!m) return null;
    var inv = m.inverse(), r = el.getBoundingClientRect();
    var a = svg.createSVGPoint(), c = svg.createSVGPoint();
    a.x = r.left; a.y = r.top; c.x = r.right; c.y = r.bottom;
    var A = a.matrixTransform(inv), C = c.matrixTransform(inv);
    return { x: A.x, y: A.y, r: C.x, b: C.y };
  }
  function hit(a, dy, o) {
    return !(a.x + a.width < o.x || a.x > o.r || a.y + dy + a.height < o.y || a.y + dy > o.b);
  }
  /* ⚠⚠ 防遞迴：這一段自己會動到 yl 的 class，而它又被 MutationObserver 盯著 ——
     classList.remove() 就算那個 class 本來就不在，也會寫一次 class 屬性、
     再觸發一次觀察者。第一版就是這樣讓整頁卡死（開頁永遠 load 不完，
     而且主控台一個錯都沒有）。所以①只在真的有那個 class 時才拿掉
     ②整段用一個旗標關起來。 */
  var inYL = false;
  function unhide() { if (yl.classList.contains('hide')) yl.classList.remove('hide'); }
  function placeYongle() {
    if (!yl || inYL) return;
    inYL = true;
    try { placeYongle0(); } finally { inYL = false; }
  }
  ...
  /* 站上那支 paintLots() 每次跑都會重新掛 .hide，所以這個標籤自己也要看著 */
  if (yl) new MutationObserver(placeYongle).observe(yl, { attributes: true, attributeFilter: ['class'] });
  ...
  function measure() {
    ...
    p.textContent =
      ...
      '　永樂街 ' + (card.getAttribute('data-yl') === 'up' ? '往上讓開路線'
        : card.getAttribute('data-yl') === 'down' ? '往下讓開路線'
        : (yl && yl.classList.contains('hide')) ? '⚠ 藏起來了（沒地方讓）' : '原位');
  }
...
<\/script>
`;
/* ⚠⚠ 守門：**產生器寫出來的東西要自己驗一次**（CLAUDE.md 第九節第 24 條）。
   ① 那支 inline script 要編得過 —— 模板字串裡的反引號、`*` 加 `/`、`</script>`
      都會讓它壞掉，而症狀是「畫面正常、按了沒反應」。
   ② 那幾支函式只准在 IIFE 的**最外層**各宣告一次（行首兩個空白）——
      2026-09-15 踩過：用 String.replace 插一段程式的時候，錨點
      `chips.forEach(function (b) {` 連**縮排四格的那一個**也吃得到，
      整塊就靜靜地被塞進 paint() 裡面；後果是 measure() 讀不到 yl、
      而且每按一次標籤就多掛一組 MutationObserver。 */
{
  const m = bar.match(/<script>([\s\S]*?)<\/script>/);
  if (!m) throw new Error('抓不到切換條那支 script');
  const body = m[1];
  try { new Function(body); } catch (e) { throw new Error('切換條那支 script 編不過：' + e.message); }
  for (const fn of ['paint', 'measure', 'placeYongle', 'placeYongle0', 'syncDim', 'boxOf', 'offOnLot']) {
    const top = (body.match(new RegExp('\\n  function ' + fn + '\\(', 'g')) || []).length;
    const any = (body.match(new RegExp('\\bfunction ' + fn + '\\(', 'g')) || []).length;
    if (top !== 1 || any !== 1) throw new Error(`${fn}() 應該在最外層宣告一次，實際 最外層 ${top}／全部 ${any}`);
  }
}
