...
## 四、提示詞・第四版 ✅ **通過了**（使用者：「很不錯，就用這個圖吧」）

產出的圖留在 `drafts/bind-done-v4-src.jpg`（1376×768），成品見第七節。
下面這一份是逐字定稿，日後要補畫或重畫就從它開始。
...
1. 裁成 **1024×512（2:1）** → `preview/line-bind-done/hero-bind.jpg`，
   `node drafts/bind-done-hero-crop.mjs`（原檔 1376×768，上下各裁 40 列）。
   ⚠⚠ **不是 1040×520** —— LINE 對 Flex 的 `image` 寫著上限 1024×1024，1040 超過了。
   裁法不是憑感覺：掃過原檔的墨，深墨（線）從第 55 列才開始（那一團人最高的頭），
   底下第 728 列以下只剩椅子底座那兩條細線，**地板線在第 715 列、沒被碰到**。
   產生器三道守門：長寬比、上緣有沒有裁到墨、四邊有沒有烘進白框。
2. 接進提案頁 Ⓑ 那一案（`/preview/line-bind-done/?v=b&h=y`，**預設就是它**），
   另加一排「有頭圖／不放圖」把高度的代價印出來：390×844 上
   **350px（42%）→ 484px（57%）**，頭圖自己 134px（占這張卡 28%）。
3. 順帶修掉提案頁兩個量測錯誤（圖卡其實只有 243.4px 不是 268、泡泡的「可用」
   量到的是它自己）—— 推導在 `drafts/channels/README.md` 第 19-10 節。
